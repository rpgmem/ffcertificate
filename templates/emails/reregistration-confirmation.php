...
return array(
	...
	'body'    => '<h2>' . __( 'Hello, {{user_name}}!', 'ffcertificate' ) . '</h2>'
		...
		. '<div style="margin:8px 0;"><span style="font-weight:600;">' . __( 'Group:', 'ffcertificate' ) . '</span> {{audience_name}}</div>'
		. '<div style="margin:8px 0;"><span style="font-weight:600;">' . __( 'Status:', 'ffcertificate' ) . '</span> {{submission_status}}</div>'
		. '<div style="margin:8px 0;"><span style="font-weight:600;">' . __( 'Verification Code:', 'ffcertificate' ) . '</span> <strong>{{auth_code}}</strong></div>'
		. '</div>'
