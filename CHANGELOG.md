...
### Fixed
- **Short URLs of posts/pages now follow the current permalink** (#888): a short URL created for a post stored the permalink captured at creation, so changing the post's slug (or the site's permalink structure) left it redirecting to the stale URL. The redirect — plus the admin list's *Destination* column and the CSV export — now resolve a post-linked short URL's destination from the post's **current** permalink at read time (falling back to the stored target only if the post is gone). Self-healing, no migration. Manual short URLs (not tied to a post) keep using their stored destination.
- **Certificate layout toolbar buttons had inconsistent sizes** (#865): legacy per-id CSS over-sized the *Import HTML* / *Background Image* / *Load* buttons and broke their icon centering (an `inline-block` + extra-padding override of the unified `.ffc-action-group` toolbar). Removed it so every layout-action button shares one size, border, and dashicon alignment.
- **Certificate layout toolbar showed duplicated icons** (#865): the *Import HTML* and *Background Image* buttons rendered both an inline dashicon and a leftover emoji `::before` rule from an earlier redesign. Removed the three stale emoji rules; the *Load* button gains a matching `dashicons-download` for consistency.
- **Default certificate templates no longer copy shipped images into the Media Library** (#865): a default seeded before the #871 `assets/` path fix kept a stale `html/` image ref, which the *Rewrite html/ Image References* migration then side-loaded into every site's uploads. The migration now **skips shipped defaults** (they're update-safe assets, repaired by re-seeding), and a seeder-version bump refreshes existing default bodies to the `assets/` source on upgrade.
- Internal (#849) — the GitHub-updater's SHA-256 mismatch error was hard-coded in Portuguese; the source string is now English (*"Update package integrity check (SHA-256) failed. Installation aborted."*), so the pt_BR rendering flows through the translation catalog instead of overriding every locale. (The pt_BR entry is regenerated via the maintainer's Loco workflow.)
...
- Internal (#822) — the `maybe_migrate_sibling_instants_to_unix()` datetime→unix safety net now targets the real `ffc_self_scheduling_appointments` table; it previously pointed at a non-existent `ffc_appointments` and silently no-op'd. Behaviour-preserving on every current install (those columns already ship as `BIGINT` on 6.6.0+ and the migration fast-returns on already-int columns) — the fix only restores reach for any legacy `DATETIME` row it was always meant to convert.

### Added
- **URL Shortener — QR Code REST endpoint** (#889): `GET /ffc/v1/short-urls/{code}/qr` returns a short URL's QR Code as base64 (`{ code, short_url, format, mime, data_base64 }`; `?format=png|svg`, `?size=100–1000`) for reuse in external automations (e.g. N8N). Cap-gated by `ffc_view_url_shortener` via an Application Password (never public); reuses the existing generation + `qr_cache` pipeline (PNGs come from the DB cache). A feature-owned controller registered by `UrlShortenerLoader`, so it adds no `API → UrlShortener` module coupling.
- **URL Shortener — edit a manual short URL's destination & title** (#888): the *Short URLs* admin list gains an **Edit** action on manually-created links, opening a modal to change the destination URL and title (`ffc_edit_short_url`, gated by `ffc_manage_url_shortener`). Short URLs tied to a post/page are intentionally **not** editable — their destination follows the page — and the rule is enforced server-side; those rows show a "follows the page" note instead of an Edit button.
- **URL Shortener — expose a page's short URL as its canonical shortlink** (#886): a new per-post-type **Expose** column (Settings → URL Shortener) publishes the short URL as the WordPress `rel="shortlink"` — the `<head>` tag and HTTP `Link:` header (also feeding the editor's "Get Shortlink" button and REST) — via a `pre_get_shortlink` filter. Expose is **opt-in** (off by default) and a strict subset of the shortened types (enforced server-side), and only takes effect once a short URL exists for the page. Adds a **"Generate missing short URLs"** backfill: a keyset-batched AJAX job (cap-gated by `ffc_manage_url_shortener`) that creates short URLs for already-published posts of the shortened types.
- **Certificate template editor — image tools + background round-trip** (#865): editing a template gains **Background Image** + **Insert Image** buttons (the same `wp.media` handlers as the form-editor layout box) and a Background Image URL field. A template's background now follows it into the form on **Load** — new `_ffc_template_bg_image` meta; `ffc_load_template` returns a `{html, bg_image}` payload that populates the form's background field — and **Save as model** captures the form's current background so a model round-trips it. Shipped defaults stay read-only (no image tools; background field read-only).
- **Certificate editor — inline image picker + save-time `html/` linter** (#865, Phase 4): the layout box gains an **"Insert Image"** button (`wp.media`) that inserts an `<img>` for the chosen Media-Library attachment at the cursor, so new images stay update-safe; and saving now surfaces a non-blocking notice listing any layout/background URL still under the plugin's `html/` drop-folder (linking to the Rewrite migration). The marker + URL regex are unified into a shared `Core\LegacyHtmlRefs`. **Completes #865.**
- **Authenticated operator REST API** (#858, #840 Part B): two new `ffc/v1` routes for a field-operator app authenticating via WordPress **Application Passwords** — `POST /operator/certificates` issues a certificate (reusing the same `SubmissionHandler` + field/CPF/email validation as the public form) and `GET /operator/certificates/{id}/pdf` returns the certificate's `pdf_data`. Issuance is gated by `ffc_manage_certificates` (admins via `manage_options`); the PDF route allows the capability **or** ownership of that certificate. Deliberately **omits** the public path's anti-anonymous defenses (captcha/geofence/per-IP+email+CPF rate-limit) — inappropriate for an authenticated, capability-gated caller (many operators share an office IP; authenticated geofencing = the parked A4) — and instead applies a generous **per-`user_id`** rate-limit as a runaway guard, plus an audit-log entry per issuance. Additive: the existing public `POST /forms/{id}/submit` and admin-ajax handlers are untouched. A5 (migrating specific admin-ajax ops to REST) is deferred until a concrete app driver exists.
- **Encryption Key Health card — one-click `FFC_ENCRYPTION_KEY` suggestion** (#851, on #846/S7a): whenever FFC's encryption key still derives from the WordPress salts (`encryption_decoupled === false`, i.e. any status while not decoupled — `ok`/`weak`/`placeholder`/`empty`), the card now offers a ready-to-paste `define( 'FFC_ENCRYPTION_KEY', '…' )` snippet in a read-only field with **"Generate another"** and **Copy** buttons. The 64-char key is generated **client-side** (`crypto.getRandomValues`) over a charset safe for a single-quoted PHP literal (excludes `' " \ $`) and is **never saved or sent** — the plugin never writes wp-config.php. Carries the mandatory rotation caveat (new install only; a site with data needs a planned rotation, S7b). Hidden once already decoupled. New `ffc-encryption-key-suggest.js`, enqueued only on the Advanced tab. **Now also suggests `FFC_HASH_SALT`** (a second field, shown until the salt is decoupled), and each copyable snippet carries a self-identifying phpdoc comment (constant, purpose, FFC version) mirroring the commented WordPress salt block, so the pasted `define` documents itself.
- Internal (#809 Fase 3) — introduced the previously-empty `tests/Integration/` layer (a tracked structural gap): a `PluginBootstrapSmokeTest` boots the **real** plugin object graph (every module loader newing-up its runtime classes) over a stubbed WordPress boundary and asserts it composes without a fatal and lands its bootstrap hooks / crons / roles, in both frontend and admin contexts — exactly the wiring the unit `LoaderTest` deliberately excludes. Adds a reusable `IntegrationTestCase` base and a new `integration` PHPUnit suite; the coverage shards still glob `tests/Unit` only, so the floor is untouched. Tests only, no product change.
- **Certificate branding tokens `{{logo_gov}}` / `{{logo_org}}`** (#865, Phase 2): two **Settings → General → Branding** logo URLs (Media Library picker) resolve into the certificate, ficha and appointment-receipt templates via a shared `BrandingTokens` resolver, falling back to a generic shipped placeholder in `assets/img/certificate-defaults/` when unset. This **de-hardcodes the instance-specific logos** from the default ficha/receipt templates (they now use the tokens instead of `html/Logo_*.png`); the tokens are documented in the in-plugin token reference. Configure your logos under Settings → General.
- **Certificate template pool — management screen** (#865): the `ffc_cert_template` pool is now managed in wp-admin as a **Templates** submenu under Certificate — a native list table with **Type** (Default/Custom) and **Visible** columns, a nonce-protected **Show / Hide** row action (toggles whether a template appears in the form editor's "Load" list), and an edit screen with an **HTML-body editor + visibility** metabox (body sanitized through the same `HtmlPolicy` allowlist as the form layout). The form editor's layout box also gains a **"Save as model"** button that stores the current HTML as a new pool template (`ffc_save_template` AJAX). All writes go through a new `CertTemplateWriter`; shipped defaults are protected from deletion (the seeder re-creates them). Gated by `ffc_manage_forms` (read-only for `ffc_view_forms`).
- **Admin notice for legacy `html/` image references** (#865, Phase 0): a dismissible admin notice warns when any stored certificate content (form layouts, backgrounds, or template-pool bodies) still references an image inside the plugin's update-fragile `html/` folder, and links straight to the Settings → Migrations **Rewrite html/ Image References** card. Detection is one cached `LIKE` existence probe (memoized in a 12h transient); shown to settings managers, one-shot dismiss via the shared `AbstractDismissibleNotice` plumbing.
- **Certificate template pool — sample-data preview** (#865): a **Preview** row action on the Templates list opens the template rendered with **sample data + the configured branding logos** in an overlay iframe. The server (`ffc_preview_cert_template`) reuses the real certificate render pipeline (`PdfHtmlRenderer` + `CertificatePreviewSamples`) so the preview matches production output (logos, relative-URL rewrite, QR/validation placeholders); the JS is a thin script-less-sandboxed viewer, with a no-JS fallback that opens the preview page in a new tab. Read-only (`ffc_view_forms`).
- **Certificate template pool — Duplicate / Export / Restore defaults** (#865): the Templates management screen gains **Duplicate** and **Export (.html)** row actions on every template (a shipped default is customized by Duplicate → an editable user copy, opened straight into the editor), plus a **"Restore defaults"** toolbar button that re-runs the seeder non-destructively (re-adds any missing shipped default and refreshes the shipped body, never touching user templates or a default's chosen visibility). New templates created via **Add New** are now visible by default. All gated by `ffc_manage_forms`; defaults stay delete/rename/edit-protected server-side.
- **Import legacy `html/` templates — migration card** (#865): a new Settings → Migrations card lifts any certificate layouts a site left in the plugin's `html/` drop-folder (basename containing "certificate", excluding the shipped defaults) into the `ffc_cert_template` pool as user templates via `CertTemplateWriter`. Non-destructive and idempotent — each imported basename is recorded in `ffc_imported_legacy_templates` so re-runs never duplicate, and a file that fails to import is still marked handled so the batch runner terminates. Gated by `ffc_manage_settings_dangerzone`; a step toward retiring the `html/` folder.
- **Rewrite legacy `html/` image references → Media Library — migration card** (#865): a new Settings → Migrations card scans stored form layouts (`_ffc_form_config['pdf_layout']`), form backgrounds (`_ffc_form_config['bg_image']` **and** the top-level `_ffc_form_bg` meta the generator reads) and every `ffc_cert_template` pool body for image URLs under `…/ffcertificate/html/` (never `…/assets/`), side-loads each distinct file **once** into the Media Library (local copy → `media_handle_sideload`, cached in `ffc_html_migrated_media`) and rewrites the references to the new attachments. Idempotent (visited posts tracked in `ffc_html_rewrite_processed`); missing files are reported as errors and left untouched. Gated by `ffc_manage_settings_dangerzone`.
- **Certificate template pool — data layer** (#865): a new `ffc_cert_template` CPT (gated by `ffc_manage_forms` / `ffc_view_forms` like `ffc_form`, via `CptCapPolicy`) backs a database-stored pool of reusable certificate templates, with a **non-destructive, versioned seeder** for the shipped defaults (canonical HTML source now lives in `templates/certificate-defaults/`) and a read-side helper for the form editor's "Load" flow. Foundation only — the management UI and the editor rewire land next; the first step toward retiring the update-fragile `html/` drop-folder.
...
### Changed
- Internal (#890, slices 4–5, final) — **inline-style consolidation: recruitment + admin templates**: the recruitment reasons colour swatch and reregistration audience dot now carry only the dynamic colour via `--ffc-color` (chrome in `.ffc-reason-color-swatch`/`.ffc-color-dot`), and a repeated block-radio label moved to `.ffc-radio-option` (both new classes in the universal `ffc-admin.css`). This closes the audit: `includes/frontend` had none; the email templates and the certificate/ficha/receipt HTML (rendered to PDF) keep their inline styles by necessity; JS-toggled `display` states and `data-ffc-confirm-style` data-attributes were left as-is. (One audience-settings colour picker was left inline — it differs from the shared `.ffc-color-input` and migrating would change its look.)
- Internal (#890, slice 3) — **inline-style consolidation across the audience admin pages**: consolidated the colour swatches/dots onto the `--ffc-color` custom property (the `.ffc-color-swatch`/`.ffc-color-dot` classes now supply the chrome; the breadcrumb dots reuse the shared `.ffc-color-dot`) and lifted the environment colour column width/alignment, the colour-input chrome, and the bookings total note into `ffc-audience-admin.css`. No behaviour change; email HTML and JS-toggled `display` states left inline. (Slice 2 — self-scheduling — audited as a no-op: its inline styles are all JS-toggled or PDF/print output.)
- Internal (#890, slice 1) — **inline-style consolidation across the admin/settings views**: lifted 18 static inline `style="…"` attributes into classes — reusable spacing/layout utilities in `ffc-admin-utilities.css` (`ffc-mt-8/12/16`, `ffc-mb-2/4`, `ffc-ml-18`, `ffc-inline`, `ffc-list-disc`) and settings-tab component rules in `ffc-admin-settings.css` (encryption-key blocks, rate-limit signal badge/headings, GPS fallback table, the date/time divergence notice) — and switched the custom-fields colour dot to the `--ffc-color` custom property the class already reads. No behaviour change. Genuinely-dynamic styles (computed colours/widths) and JS-toggled `display` states were deliberately left inline; documentation examples were untouched.
- ⚠ Internal (#883) — **CSS class-name consistency pass**: aligned shorthand admin classes to the plugin's spelled-out `ffc-` kebab convention — `ffc-em-*` → `ffc-email-model-*`, `ffc-sched-exc-*` → `ffc-schedule-exception-*`, the CSV info-table `ffc-j*` → `ffc-info-table-*`, and margin utilities `ffc-mt15`/`ffc-mt20`/`ffc-mb5` → `ffc-mt-15`/`ffc-mt-20`/`ffc-mb-5` (plus a real `.ffc-mt-5` for the previously-dangling `ffc-mt5`). No behaviour change; **breaking only for custom CSS that overrode the old class names.**
- **Default certificate templates carry their background in the dedicated field** (#865): the shipped defaults baked the full-page background into the body as an absolute `<img>`; it now lives in the template's **Background Image** field (seed v3 — upgrades refresh existing defaults), so the field shows it and **Load** carries it into the form's `_ffc_form_bg` like any user template.
- **Shipped default templates are read-only except the visibility toggle** (#865): the edit screen now locks the (non-editable) title and drops the Publish/Update box for defaults, and the sidebar "Show in Load list" toggle **autosaves over AJAX** (gated by `ffc_manage_forms` + a per-action nonce) — a live switch on user templates too.
- **Certificate template "Show in Load list" is now a sidebar toggle** (#865): the visibility checkbox became the shared `.ffc-toggle` switch and moved from under the HTML editor into a WP-idiomatic **Template options** sidebar metabox.
- Internal (#865) — folded a static inline `style` on the submissions filter-button icon into the existing `.ffc-filter-btn .dashicons` CSS rule (part of the inline-style cleanup sweep).
- Internal (#882) — the `develop → testes` deploy workflow now fails fast and legibly instead of hanging on a stalled SSH connection: `-o BatchMode=yes` (an interactive auth prompt errors immediately rather than blocking), a `timeout 300` wrapper around `rsync` (aborts a stalled transfer so the idempotent retry loop can re-try), and a `timeout-minutes: 25` job backstop. Deploy/CI-only; does not change what is shipped.
- **Certificate template edit screen gains the form editor's CodeMirror** (#865): the `ffc_cert_template` HTML box is now the same syntax-highlighted editor as the form-editor layout box (via a shared `AdminAssetsManager::enqueue_code_editor_for()`), rendered read-only for shipped defaults; the form editor's layout action buttons also get consistent leading icons.
- **Ficha & appointment-receipt default templates moved to `templates/documents/`** (#865): the two document templates moved out of the update-fragile `html/` folder to the versioned `templates/documents/`; the `FichaGenerator` and the receipt renderer load them from the new path (and they already use the `{{logo_gov}}`/`{{logo_org}}` branding tokens). Continues emptying `html/` of shipped assets ahead of retiring the folder — only the legacy `html/default_certificate_*` glob fallback remains there.
- **Default certificate images moved to `assets/`** (#865): the shipped background + signature images the default templates use now live in `assets/img/certificate-defaults/` (versioned, update-safe) instead of `html/`, and the seed templates reference them there. A step toward retiring the `html/` folder; the instance-specific logos (`Logo_Cidade_SP`, `Logo_DRE`) stay out of the distributed zip and become the configurable `{{logo_gov}}`/`{{logo_org}}` branding tokens (next). Existing `html/` images are left in place through the deprecation window.
- **Certificate-layout "Load" now reads the DB template pool** (#865): the form editor's layout dropdown + "Load" modal are populated from the `ffc_cert_template` pool by post id (defaults grouped first), and `ffc_load_template` resolves the HTML server-side via `CertTemplateReader` under the existing nonce + `ffc_manage_forms` gate — replacing the unauthenticated direct `fetch()` of `/wp-content/plugins/.../html/`. The legacy `html/` glob (by filename) remains a deprecated fallback for sites whose pool hasn't seeded yet.
- **Encryption Key Rotation now requires *both* decoupling constants** (#857 follow-up): the rotation card (Settings → Migrations) and its `can_run` gate stay hidden/refused until **both** `FFC_ENCRYPTION_KEY` and `FFC_HASH_SALT` are defined in wp-config.php — rotating with only the key set would rebuild the CPF/RF/email search hashes under the still-shared WordPress salt, forcing a second rotation the moment the salt is later decoupled. The **Advanced → Encryption Key Health** panel now surfaces a pending-record count that links to the rotation once both constants are defined (the generator block hides exactly then), closing the adopt→rotate loop.
- Internal (#850) — swept the plugin's ~50 hand-built `<div class="notice …">` admin-notice snippets (across settings tabs/views, admin pages, recruitment, self-scheduling and templates) onto WordPress's canonical **`wp_admin_notice()`** (WP 6.4+, established by #849), so type/dismissibility/extra-class markup is core-escaped and consistent; dynamic and rich-body notices use the `paragraph_wrap => false` + `ob_start()` capture form. Behaviour-preserving.
- Internal (#849) — extracted a shared `AbstractDismissibleNotice` base for the two persistent-dismiss admin notices (device-threshold + encryption key-health), collapsing the duplicated hook / dismiss / capability / enqueue skeleton into one, and adopted WordPress's canonical **`wp_admin_notice()`** (WP 6.4+) for the wrapper markup. **Bumped `Requires at least` 6.2 → 6.4** to use it (the install target already runs 6.5+ — the performant-translations catalog is 6.5-format). Behaviour-preserving; both notices share the generic `ffc-dismissible-notice.js`.
- Internal (#809 Fase 2) — audience coverage push (tests only, no product change): `AudienceNotificationHandler` (~16% → ~96%) now exercises the booking-created / cancelled flows, admin-notification opt-in + recipient parsing/fallback, and the ICS-attachment path; `AudienceAdminBookings::render_page()` (~51% → ~97%) now covers the populated table (active vs cancelled rows, creator-map fallback, export button + environment filter).
...
- Docs (#822) — recorded the user-deletion decision for `ffc_reregistration_submissions`: its `NOT NULL` `user_id` stays an **accepted orphan** (retain the record, PII lives in the `data` body so nulling the FK alone is a half-measure), documented in the reregistration activator. Rewrote CLAUDE.md §4 with the governing *"retain records, drop relationships"* principle + the full plugin-wide inventory of uncovered user references (attribution `*_by` columns, post-promotion candidate, row-body PII), correcting the prior note that implied a separate custom-field value table. No product change.
- Docs — closed documentation-tab gaps for recently-shipped surfaces: new in-plugin pages for **Modules** (#800), the **Activity Log** viewer + its CSV export (#802), **Privacy & LGPD** (WordPress export/erase integration, account-deletion anonymization vs. erasure, PII-at-rest) and **Updates** (GitHub-Releases auto-update, #820); documented the authenticated **operator REST API** (#858) on the Developer page; and added the missing `ffc_export_url_shortener` capability + CSV export to the Short URLs page (#711/#761). Docs only, no product change.

### Deprecated
- **Legacy `html/` certificate-layout glob** (#865): the pre-pool fallback that discovered certificate layouts by globbing the plugin's `html/` folder (the form editor's "Load" list + the `filename` shim in `ffc_load_template`) is **deprecated**. The database-backed template pool is the supported source; a new admin notice flags any stored reference into `html/`, and the Settings → Migrations "Rewrite html/ Image References" card moves those images into the Media Library. On the project's **3-release deprecation cycle** the glob is scheduled for **removal at N+3**, after which the `html/` folder — emptied of shipped assets — is retired. Sites relying on files dropped in `html/` should import them (Settings → Migrations) before then.

### Removed
- Internal (#849) — removed the now-dead `ffc-device-threshold-notice.js` (+ minified): after the `AbstractDismissibleNotice` extraction the device-threshold notice uses the shared `ffc-dismissible-notice.js` (`.ffc-js-dismiss-notice`), so the dedicated handler had no remaining enqueue.
- Internal (#809) — retired the pre-4.6.15 orphan-cron cleanup (the three `wp_clear_scheduled_hook( 'ffc_daily_cleanup_hook' | 'ffc_process_submission_hook' | 'ffc_warm_cache_hook' )` calls in the activator + deactivator/uninstall). Those hooks haven't been registered since 4.6.15; any remaining scheduled entry is inert and harmless. Behaviour-preserving on any install newer than 4.6.15.
...
### Security
- Dev/CI dependency security bumps (#884): PHP `squizlabs/php_codesniffer` 3.13.5 → 3.13.6 (CVE-2026-67434, OS command injection, high) plus the npm build-chain advisories (`brace-expansion`, `js-yaml`, `postcss`) resolved via lockfile updates. Dev/build tooling only — not shipped in the plugin runtime; `composer.json` / `package.json` constraints unchanged.
- **User-deletion integrity — attribution + promoted-candidate coverage** (#834, follow-up to #822): when a WordPress user is deleted, the `deleted_user` hook now also SET-NULLs the **nullable actor-attribution columns** (`edited_by`, `approved_by`/`cancelled_by`, `created_by`/`updated_by`, `reviewed_by` across submissions, self-scheduling, audience-bookings, reregistration, recruitment-call and short-urls) and the **promoted-candidate link** `recruitment_candidate.user_id`, so a deleted admin/operator's id no longer dangles as a stale attribution. This authorship sweep runs on every deletion (an actor/candidate needn't have data-subject footprint), outside the #322 footprint short-circuit. The six `NOT NULL` `created_by` columns stay **accepted orphans** on retained records. **Row-body PII is unchanged by design:** routine account deletion anonymises the *link* only; scrubbing the encrypted `*_encrypted`/`*_hash` body remains the deliberate **manual `PrivacyErasers`** (LGPD erasure) path, so certificates/appointments stay verifiable until a subject actually requests erasure. CLAUDE.md §4 updated with the decision.
- **Encryption key re-keying (FFC→FFC)** (#863, on #857 S7b): `Encryption::decrypt()` gains a **previous-key fallback** — define `FFC_ENCRYPTION_KEY_PREVIOUS` with the prior key and the decrypt chain reads rows still under it while fresh writes use the new `FFC_ENCRYPTION_KEY`, so an admin can rotate one FFC key to another with **zero downtime** (run the Encryption Key Rotation, then remove `_PREVIOUS`). The chain is `active → previous-FFC → WP-derived`, so a WP→FFC adoption and an FFC→FFC re-key can be in flight at once. The **salt needs no previous** (search hashes are rebuilt from plaintext during rotation). Settings → Advanced now shows the previous-key fingerprint, a re-key-aware rotation notice (in-progress *vs.* complete → "remove `FFC_ENCRYPTION_KEY_PREVIOUS`"), and a "Rotate to a new key" helper that generates the new key + the step-by-step wp-config edits. Invariant (single `_PREVIOUS` slot): finish a rotation before starting the next.
- **Encryption key rotation** (#857, #840 S7b, on #846/S7a): a new **Encryption Key Rotation** migration (Settings → Migrations, gated by `ffc_manage_settings_dangerzone`) re-encrypts submissions + appointments PII under a strong `FFC_ENCRYPTION_KEY` and rebuilds the CPF/RF/email search hashes under `FFC_HASH_SALT`. `Encryption::decrypt()` gains a **WP-derived-key fallback** so existing rows stay readable the instant the constant is defined (zero downtime) until the migration re-encrypts them; the migration is keyed on a non-reversible key **fingerprint** that re-arms it if the key changes (never silently "complete"), and any row it cannot decrypt is reported as an error and left untouched — never orphaned. Run during low traffic (hash-based lookups may transiently miss un-migrated rows). Only submissions + appointments rotate today; other encrypted stores keep reading via the permanent fallback, so leave the WordPress salts in place.
- Encryption key/salt health surface (#846, #839 S7a): a new `Encryption::key_health()` (`ok`/`weak`/`placeholder`/`empty`) inspects the *active* secret material — the WordPress salts (or the `FFC_ENCRYPTION_KEY`/`FFC_HASH_SALT` decoupling constants) that derive the encryption key + search-hash salt — and flags missing/short/`put your unique phrase here` placeholder secrets that would make the key predictable. Surfaced in three non-blocking layers: a dismissible **global admin notice** (re-shown if the state worsens or the key rotates, via a status+fingerprint signature), a **status panel** on Settings → Advanced (source, decoupled?, non-reversible key fingerprint), and an **optional** "decouple with `FFC_ENCRYPTION_KEY`" advisory; `get_info()` now carries `key_health`. Never disables encryption (`is_configured()` is unchanged — that would store PII in plaintext). Absorbs the old S6 (search-hash reversal) — no hash-algorithm migration.
- Rate-limit hygiene batch (#839): (S5) the `ffc_rate_limits` counter table now **hashes non-IP identifiers** (email/CPF) at rest — a counter only needs a stable equality key — mirroring the sibling `ffc_rate_limit_logs`; IP stays plaintext for the stats "top IPs" view (one-time window reset on upgrade). (S9) the recruitment classifications CSV import now validates **extension + a 10 MB size cap** before reading the upload whole into memory, across all three import entry points. (S11) the no-JS `[ffc_verification]` fallback now applies the **same verification throttle** as the AJAX path, closing an un-throttled auth-code probing hole. (S8) documented the nginx `deny` rule for the CSV-export temp dir (`wp-content/uploads/ffc-tmp/`) in `docs/DEPLOYMENT.md` + a readme FAQ. S7a (key/salt health warning, absorbing the old S6) is tracked separately in #846.
- **⚠ REST PII masking (#838):** the `GET /submissions[/{id}]` and public `POST /verify` endpoints no longer leak unmasked PII. `/verify` now runs the decrypted `data` blob through the same per-key mask as the `/valid` page (cpf/rf/rg/email), instead of returning the full form body to any caller with a valid auth code (S3). `/submissions` masks `email`/`cpf`/`rf`/`data` by default and returns plaintext only to the `ffc_view_certificates_pii` cap / `ffc_certificates_admin` role (super-admins unmasked), auditing each reveal — closing the bypass that let a PII-blind `view` operator bulk-extract certificate PII over REST without a log (S2). **Breaking for external REST consumers** authenticating as a `view`/operator role: grant them `ffc_view_certificates_pii` to keep plaintext. Per-key masking now lives in the shared `DocumentFormatter::mask_field_value()` so the web + REST paths can't drift again.
