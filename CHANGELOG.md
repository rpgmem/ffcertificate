...
### Changed
- **Document-template hub moved under Settings** (#951): the template pool CPT (`ffc_cert_template`) — which holds certificate **and** appointment-receipt models — is now a kind-neutral **“Document Templates”** submenu under **FFC Settings** instead of living under the Certificates menu (where it misleadingly read as certificate-only now that it holds more kinds). Menu/label change only — the list and edit screens, caps and storage are unchanged. First step of the #951 Direction 1 consolidation (one management hub + per-feature selectors).
