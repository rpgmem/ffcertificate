...
- **Guarda: um id que o JavaScript procura tem de ser emitido por alguém** (#1220): três vezes numa sessão um teste passou verde sobre marcação que o produto nunca renderizou — o painel do usuário sem carregar painel (#1204) e o grupo de acúmulo que nunca se ocultava (#1219). `JsSelectorEmitterTest` cruza os 184 ids que o JS busca com quem os emite em PHP e em JS, e cobra que cada um tenha emissor — os 7 que abriram a baseline foram decididos em #1227 e a lista ficou vazia; restam 2 exceções do WordPress.
...
- **As migrações de capability varriam todos os usuários da instalação em cada requisição que ainda não tivesse concluído** (#1254): onze delas pediam `get_users( array( 'fields' => 'ID' ) )` e faziam um `get_userdata()` por usuário, no `plugins_loaded` — podendo cair numa requisição de frontend anônima. E a flag de conclusão só é gravada **depois** da varredura, então um timeout no meio fazia a requisição seguinte recomeçar do zero, indefinidamente. Passam a perguntar só por quem tem alguma capability ou papel `ffc_*` **pessoal**, que é a única coisa que qualquer uma delas decide — o trabalho vira proporcional ao número de operadores, não ao tamanho da base.
- **A varredura diária de tickets expirados nunca rodou** (#1234): o activator agenda o evento, ele dispara todo dia, e o callback era registrado pelo `AdminLoader` — que só é construído dentro de `if ( is_admin() )`. O `wp-cron.php` define `DOING_CRON` e **nunca** `WP_ADMIN`, então em todo contexto que executa o gancho a guarda era falsa. O registro passa para `Loader::define_admin_hooks()`, que apesar do nome roda em toda requisição; o gate do módulo Certificados continua valendo.
- **A varredura de retenção emitia um `DELETE` sem limite dentro da requisição de um visitante** (#1234): ela roda no cron diário e é opt-in duplo, então nunca rodou — e o dia em que um administrador a ligar numa instalação madura é exatamente quando o backlog é maior. Passa a apagar em páginas de 500, no máximo 20 por execução; o resto sai no dia seguinte, sem nenhum statement grande.
- **`SchemaAgreementTest` media 2 dos 6 activators, e a autoverificação não pegava** (#1241): a varredura procurava só `add_columns_if_missing()` no plural (4 ocorrências) e ignorava o singular (40), enquanto o regex do `CREATE TABLE` exigia `{$table_name}` literal e `) {$charset_collate}` sem nada no meio — o que tornava invisíveis os 9 statements do recrutamento (que fecham com `ENGINE=InnoDB`) **nas duas direções da guarda**. A varredura passa a ler o parser compartilhado, e duas asserções novas cobram que nenhum ficheiro nem nenhum statement fique de fora.
- **Doze colunas existiam só na migração, não na declaração da tabela** (#1241): uma instalação nova as recebia porque `maybe_migrate()` roda logo após a ativação, então o defeito era latente — mas o `CREATE TABLE` não descrevia a tabela real, e o portão de idempotência do `dbDelta` media um statement que não correspondia. Declaradas nos `CREATE TABLE` de `ffc_audience_schedules`, `ffc_audience_bookings`, `ffc_audiences`, `ffc_self_scheduling_calendars` e `ffc_short_urls`.
- **Três colunas que a guarda passaria a exigir não podem ser declaradas** (#1241): `submission_date_ts`, `submitted_at_ts` e `called_at_ts` são colunas de **estágio** da migração DATETIME→BIGINT do #249 — preenchidas e depois renomeadas para o nome definitivo. Declará-las criaria uma coluna permanente sobre a qual o `CHANGE` tentaria renomear outra. A guarda passa a excluí-las pelo sinal certo: o nome aparecer como origem de um `CHANGE`, não pelo sufixo `_ts`.
- **A migração de rotação de chaves podia falhar em silêncio sobre candidatos duplicados** (#1236): o docblock afirmava que reconstruir o hash não podia colidir, e a afirmação estava errada — `cpf_hash` e `rf_hash` são UNIQUE sobre o **valor**, não sobre a pessoa, então duas linhas da mesma pessoa criadas sob salts diferentes passam pela restrição. Onde esse par existe, o `UPDATE` bate na UNIQUE e a linha nunca migra. A mensagem de erro passa a carregar o texto do banco, que nomeia a chave e o valor duplicados.
- **O agendamento de cada submissão gravava PII e a credencial do certificado em claro na `wp_options`** (#1248): o payload do cron levava oito argumentos — o `submission_data` inteiro e o `magic_token`, que é a autenticação de `?token=…` — vinte linhas depois de o mesmo método ter cifrado e-mail, CPF e RF para gravar. E a option `cron` é autoloaded, então todo pedido ao site carregava aquilo enquanto o evento estivesse pendente. Agora viaja só o id; o gancho público `ffcertificate_process_submission_hook` continua recebendo os mesmos oito valores, reidratados no disparo.
- **A rotação de chaves passa a cobrir a PII de perfil dos usuários** (#1236): `ffc_user_cpf`, `ffc_user_rf` e `ffc_user_rg` estavam cifradas sob a chave **derivada dos salts do WordPress** — rotacionar `SECURE_AUTH_KEY` / `LOGGED_IN_KEY` / `NONCE_KEY` as tornaria ilegíveis para sempre. A migração agora recifra sob a chave ativa e refaz os hashes de busca de CPF e RF sob `FFC_HASH_SALT`. Não é item de desempenho: é integridade de dados.
- **A semeadura de uma campanha de recadastramento era um SELECT e um INSERT por membro** (#1234): dez mil membros custavam vinte mil consultas na requisição que salva a campanha. Passa a INSERTs de 500 linhas — vinte consultas —, e "pular quem já tem submissão" fica por conta da UNIQUE `(reregistration_id, user_id)`, que também fecha a janela entre ler e inserir que um duplo clique em Salvar abria.
- **Uma falha de decriptação gravava uma linha de log por valor, sem teto** (#1234): com a chave quebrada, exportar 5.000 submissões escrevia 5.000 INSERTs em `ffc_activity_log` — a auditoria custando mais que a leitura que falhou. Agora são cinco linhas por requisição mais uma marca `decrypt_failure_suppressed` dizendo onde cortou; passado o teto o caminho sai antes até de ler as configurações.
- **O `<select>` de filtro do Log de Atividades percorria o índice inteiro a cada render** (#1234): `SELECT DISTINCT action` na tabela que mais cresce, sem cache, para produzir algumas dezenas de valores. Passa a transiente de uma hora — sem invalidação por escrita de propósito: a tabela é append-only e o conjunto de ações muda por deploy, não por uso.
- **As estatísticas de pré-voo puxavam até 5.000 linhas por render do editor** (#1234): `PreflightStatsService` carrega as linhas na memória e faz um `json_decode` por linha, porque o filtro por formulário mora no JSON. Passa a transiente de 5 minutos, com a chave levando **o formulário e a janela em dias** — só o formulário devolveria a contagem de 30 dias para quem pediu 7.
- **`hasEditInfo()` varria a tabela inteira para responder um booleano** (#1234): usava `COUNT(*)` sobre `ffc_submissions` — a maior tabela do plugin, com `edited_at` não indexado — só para comparar o número com zero, a cada renderização da tela que decide se mostra a coluna de edição. Passa a `SELECT 1 … LIMIT 1`, que para na primeira linha que satisfaz.
- **O histórico de chamadas do candidato era um N+1** (#1234): `GET /me/recruitment` consultava uma vez por classificação, dentro de laço aninhado, enquanto o ajudante em lote já existia e já era usado pelo painel. Passa a uma consulta só. A ordem não muda — as duas versões ordenam por `called_at DESC`, e agrupar uma lista ordenada preserva a ordem dentro de cada grupo.
- **`column_exists()` era sondado por linha dentro do lote da rotação de chaves** (#1234): um `SHOW COLUMNS` sem cache por (linha × coluna), com resposta invariante no lote — até 2.500 consultas supérfluas a cada 500 linhas. Resolvido uma vez por lote, e recomputado a cada lote, então uma coluna adicionada entre lotes continua sendo vista.
- **Lembretes de recadastramento saem em lotes de 50** (#1232): a varredura diária roda no wp-cron, isto é, **dentro da requisição de um visitante** — sem limite, uma campanha de milhares fazia esse visitante pagar milhares de `wp_mail()` mais um hash phpass e um `UPDATE wp_users` por pessoa. O primeiro lote sai na hora e o resto é reagendado; campanha de 50 ou menos termina numa execução só, como antes.
- **O lote avança mesmo quando um envio não pode ser feito** (#1232): `user_id` na submissão é `NOT NULL` e órfão aceito (#822), então apagar a conta deixa a linha apontando para um usuário inexistente e ela nunca recebe carimbo. Um loop guiado só por `reminder_sent_at IS NULL` a rebuscaria a cada lote e se reagendaria a cada 60s, sem fim. O cursor keyset em `id` ultrapassa a linha; ela volta a ser tentada na varredura do dia seguinte.
- **O cache de QR do encurtador servia o tamanho errado** (#1233): indexado só pelo `short_code`, ignorava o tamanho pedido — uma chamada REST com `size=1000` recebia o PNG de 200px gravado pela metabox e, com o cache vazio, gravava 1000px lá para a metabox renderizar num espaço de 200. Agora o cache serve um único tamanho e o grava junto do PNG; linha do esquema antigo vira miss na leitura, sem migração.
- **A aba Cache dizia "QR Code Cache" sem dizer qual** (#1233): o cartão governa e limpa apenas o QR de certificado; o QR da URL curta tem cache próprio, sem ajuste e sem limpeza manual — ele se auto-corrige. O texto agora nomeia os dois, e cada mecanismo aponta para o outro no código.
- **O lembrete de recadastramento era reenviado todo dia** (#1232): a consulta de campanhas usa `DATEDIFF(end_date, CURDATE()) <= reminder_days` — uma **janela**, não um dia — e o cron é diário, sem marca por submissão. Com `reminder_days = 7`, cada participante pendente recebia sete e-mails. Agora há `reminder_sent_at`: um lembrete por campanha, mais um a cada extensão de prazo, a mesma regra que o convite aplica desde o #1190. O carimbo é por item logo após o envio, então uma execução interrompida retoma de onde parou em vez de recomeçar.
- **A documentação não dizia quando um lembrete é reenviado** (#1232): a aba Documentação descrevia o lembrete como "enviado quando o prazo está a N dias" e parava aí. Agora diz que é **um por participante por campanha**, nomeia as duas formas de haver um segundo (estender o prazo, ou reenviar à mão) e registra a consequência que não é adivinhável: **um reenvio manual substitui o lembrete automático daquela pessoa, não se soma a ele**.
- **Quatro cadeias de activator sondavam o schema em toda requisição** (#1231): `SelfScheduling`, `Audience` e `UrlShortener` (`maybe_migrate`) mais `Recruitment::create_tables` rodavam no `plugins_loaded` de cada página — frontend anônimo incluído — emitindo 48 consultas DDL (`SHOW TABLES` / `SHOW COLUMNS`, nenhuma cacheada) numa instalação sem nada a migrar. Passam a ser guardadas por `FFC_VERSION`, o mesmo idioma que outras oito chamadas do `Loader` já usavam.
- **Cada `encrypt()`/`decrypt()` refazia duas derivações PBKDF2 de 10.000 rounds** (#1230): as chaves derivadas das constantes do WordPress eram recalculadas a cada chamada, sem memoização. Medido: 11,56 ms por derivação, 23,8 ms por decrypt, **2.384 ms para desenhar a tela de Submissões** — que agora custa 0,7 ms. As entradas são constantes PHP, imutáveis no request, então memoizar não pode divergir. Não afeta instalações com `FFC_ENCRYPTION_KEY`, que já usavam o caminho barato.
- **A prévia da imagem de fundo nunca aparecia** (#1227): o código que a desenha está escrito e correto desde sempre, mas nenhuma das duas telas que montam o botão "Background Image" emitia o contêiner `#ffc_bg_image_preview` — então a busca falhava em silêncio, guardada por uma checagem de comprimento. As duas telas passam a emiti-lo.
- **`RequestInputCastTest` lia prosa como código** (#1220): a varredura abria o arquivo como texto puro, então um comentário que citasse `absint()` sobre um superglobal era contado como um cast de chave dinâmica e reprovava o PR que escrevia o comentário. Passa a remover comentários com `token_get_all()`. Medido: mesmas 54 entradas antes e depois na árvore atual.
...
- **O grupo de acúmulo de cargos nunca se ocultava — a fiação não existia** (#1209): o handler procurava `#ffc_rereg_acumulo` e `.ffc-rereg-acumulo-fields`, que nenhum PHP emite, então operava sobre conjuntos vazios; e mesmo assim só reagia ao `change`, nunca à abertura do formulário. Passa a selecionar por `data-field-key`, a rodar na inicialização, e a mostrar os três campos dependentes só em "Eu acumulo" — a mesma regra que a ficha já aplicava ao descartá-los. O `required` das faixas de horário é levantado enquanto o bloco está oculto, senão trava o envio sem dizer o que falta.
- **"Union" estava traduzido como "Estado", colidindo com o Estado do endereço** (#1209): no recadastramento, o grupo 4 e o campo `sindicato` mostravam a mesma palavra que o `uf` — duas coisas diferentes sob um nome só. E "Acknowledgment" estava como "Agradecimentos", quando é ciência/aceite; o próprio código já chamava a coisa de `termo_ciencia`. Viram **Sindicato** e **Termo de Ciência** — os rótulos de grupo são resolvidos em runtime, então alcançam toda instalação sem migração.
- **Rascunho devolvido voltava com o ciphertext dos campos sensíveis** (#1210): o usuário via o blob no lugar do próprio CPF e não conseguia editar. O degrau do rascunho não passava pelo ajudante de descriptografia que o caminho do perfil e o do admin já usavam. Devolver em texto claro é correto porque quem lê é o titular autenticado editando o próprio dado; a gravação segue criptografada.
...
- **Membro de público filho via a campanha do pai mas nunca era semeado nem convidado** (#1190): a consulta que decide quem *vê* sobe do público do usuário até a raiz; a que decide quem é *afetado* não descia — então quem pertencia só ao filho enxergava o banner, não tinha linha de submissão e não recebia convite. A contagem que o operador lê ao montar a campanha usa o mesmo ajudante, então confirmava a semeadura errada em vez de contradizê-la.
- **A cor dos selos do recrutamento também morava no atributo `style`** (#1193): eram 16 chaves de Settings mais a cor de cada adjutância, escritas em cada `<span>`. Passam a ser regras geradas por `wp_add_inline_style`, anexadas à folha de cada tela — o selo de status sai sem estilo nenhum, e só a adjutância, cuja cor é por linha do banco, leva o valor como propriedade personalizada.
- **Selos sem contorno no modo de alto contraste** (#1193): o bloco `forced-colors` do admin listava quatro nomes de status, então as seis variantes de classificação ficavam de fora — a lacuna que o #1183 registrou; e a folha pública não tinha bloco nenhum. Agora casam a família, então uma variante nova nasce coberta.
- **A forma de todo selo do recrutamento morava num literal PHP, aplicado inline** (#1193): inline vence classe, então nenhuma folha conseguia descrever um selo e a mesma dupla de classes rendia duas formas diferentes conforme o emissor — a célula de status das classificações saía retangular, sem cor e com o enum cru, enquanto as outras seis saíam em pílula. A forma vira `.ffc-pill` na base, e aquela célula passa pelo mesmo ajudante das demais.
- **A folha do shortcode público saía no rodapé** (#1193): ela era enfileirada dentro do `the_content`, depois do `wp_head`, deixando uma janela em que os selos eram pintados sem ela. Passa a ser enfileirada também no `wp_enqueue_scripts` quando o post traz o shortcode, sem substituir o caminho tardio, que ainda cobre widget e bloco.
- **O selo "Definitive" de recrutamento não tinha cor de selo** (#1183): o activator migrou o ENUM e rodou `UPDATE … SET status='definitive' WHERE status='active'`, mas a folha ficou com o nome antigo — então a regra não casava com linha nenhuma e o status que a substituiu não tinha nenhuma, saindo com fundo transparente e texto herdado, inclusive sem borda no alto contraste.
- **O selo de status do edital saía em caixa baixa na página pública** (#1183): ele casava o `.ffc-status-badge` genérico da `ffc-common.css` só por dividir o nome, e era de lá que vinham `text-transform` e `white-space` — as duas únicas propriedades que o estilo inline do `BadgeHtml` não define. A folha pública passa a declará-las.
- **Selos de estado de recrutamento e recadastramento eram pintados por três folhas** (#1183): os dois declaravam `.ffc-status-badge` cru, e `ffc-admin-submissions.css` — cujo portão é `is_ffc_page()`, toda tela `ffc-*` — declarava o mesmo nome. O de recadastramento renderizava `text-transform` e `letter-spacing` que a folha dele nunca declarou. Cada família ganha nome próprio declarando a forma inteira.
- **Quem entrou no público depois da campanha ativa não conseguia se recadastrar** (#1188): o estado `no_submission` foi declarado submissível, o painel desenhava o botão — e os três handlers AJAX exigiam uma linha que ninguém criava, respondendo "Nenhuma submissão encontrada para este usuário". A linha passa a nascer sob demanda, depois de confirmar pela mesma consulta que decidiu o que o usuário vê.
- **O botão de recadastramento dizia que o recadastramento estava completo** (#1188): `Complete Reregistration` é imperativo ("faça"), como os irmãos `Continue`/`Resubmit`, e estava traduzido como o adjetivo "Recadastramento Completo" — anunciando um estado inexistente no painel e no botão do e-mail de convite. Vira "Fazer o Recadastramento".
- **A varredura de emissão dava nove classes como órfãs tendo o emissor no repositório** (#1182): ela não lia três formas — o nome na abertura de um atributo que a concatenação deixa sem fechar (`class="ffc-appointments-table' + …`), várias classes numa string só, e o valor de opção do jQuery UI que vira classe. Quem renomeasse qualquer uma delas confiaria numa guarda que já a tinha dado como sem uso.
- **A linha passada da recadastração ficava abaixo do piso de contraste** (#1170): ela dizia "passado" com `opacity: 0.75`, e desbotamento multiplica texto e fundo juntos — media **3,11:1** no tema claro e **4,23:1** no escuro, contra o mínimo de 4,5:1. As duas tabelas irmãs já tinham trocado isso por cor; agora as três dizem o estado do mesmo jeito, e uma catraca congela toda `opacity` das folhas com a razão de cada uma.
...
- **Cinco rótulos e cinco obrigatoriedades dos campos padrão de recadastramento** (#1209): RF vira "Registro Funcional (RF)", "Apto/Bl" vira "Complemento", os dois de emergência ganham "Nome do"/"Telefone do", e o institucional ganha "(@sme)"; `rf`, `endereco`, `endereco_numero`, `email_institucional` e `sindicato` passam a obrigatórios. Vale para **públicos novos** — os campos moram no banco por público e os existentes seguem editáveis na UI.
- **O recibo de agendamento anunciava `?ver=2.5.1` para um jsPDF que é 4.2.1** (#1203): era o único dos oito enfileiramentos dos bundles de PDF que cravava a versão à mão em vez de ler a constante, e o literal ficou para trás quando o jsPDF subiu. Um navegador que cacheou o bundle por aquela tela guardava chave velha para conteúdo novo. Passa a ler `FFC_JSPDF_VERSION`, como os outros sete.
- **`html2canvas` e `jspdf` ganham a versão no nome do arquivo** (#1203, #1202): `html2canvas-1.4.1.min.js` e `jspdf-4.2.1.umd.min.js`, com o caminho montado a partir da constante como `altcha` e `thumbmark` já faziam. As duas metades deixam de poder divergir — subir a constante sem renomear o arquivo agora dá 404 alto em vez de servir o bundle velho sob chave nova.
- **O fluxo de Dependabot do `CLAUDE.md` mandava um agente fazer algo que ele não consegue** (#1201): o `@dependabot rebase` do retarget sai com separadores injetados na menção e o bot nunca o lê — falha em silêncio, o PR só fica parado. A seção passa a registrar o contorno medido (conferir se o lockfile difere; se o PR está só `behind`, atualizar o branch) e que o plugin não envia nenhuma dependência de runtime gerenciada, enquanto quatro bibliotecas vendorizadas à mão em `libs/js/` embarcam sem estar em manifesto nenhum (#1202).

- **O registro de desbotamento tratava cinco elementos como defeito adiado, e nenhum era** (#1185): a razão de três entradas dizia que a cor não era nossa e que dar cor viria antes de tirar o desbotamento. Medido por render no DOM real, os cinco estão sob raízes que o par de base já nomeia e ficam entre 5,55:1 e 21:1 com o desbotamento incluído. As entradas passam a carregar o número medido.
- **Duas varreduras de guarda liam o fonte errado** (#1193): a que procura testes vazios contava chaves sem consciência de string, então um literal com `}` dentro — como o que prova que uma injeção de CSS é recusada — fechava o corpo do método e escondia as asserções seguintes; e a que acha emissores de classe exigia a palavra `class` por perto, que numa chamada a um ajudante está no nome do parâmetro, não no site. Duas classes estavam sem emissor achável por isso.
- **Regras de selo sem emissor em três folhas** (#1193): `ffc-admin-submissions.css` declarava um `.ffc-status-badge` byte a byte igual ao `.ffc-badge` da base, com variantes `.publish`/`.trash` que ninguém emite desde que a lista migrou; e cinco propriedades de forma na folha de recrutamento nunca venciam o inline. Removidas.
...
- **Trazer as respostas do último recadastramento aprovado para a campanha nova** (#1213): ao entrar numa campanha, quem já teve um ciclo **aprovado** vê um aviso oferecendo importar; nada é buscado até clicar. Só campos que existem na campanha atual voltam, e o que o participante já digitou não é sobrescrito. O endpoint devolve PII em claro, então deriva o usuário de `get_current_user_id()` e exige que ele pertença à campanha.
- **Código de autenticação no rodapé da ficha de recadastramento** (#1211): o rodapé já trazia "Preenchido em"; ganha `Autenticação: R-XXXX-XXXX-XXXX` no mesmo formato do e-mail de convite e do certificado. Só em submissão **aprovada** — o código nasce na transição de status, e imprimir o `S{id}` que serve de nome de arquivo faria um identificador interno passar por código de autenticação.
- **A catraca de âncora de namespace do CSS foi de 41 entradas a 3** (#1184): 48 ocorrências de seletor sobre marcação do WordPress — `.tablenav`, `.column-*`, `.form-table`, `.button`, `.card`, `code` — passam a descer da tela a que pertencem, em vez de alcançar toda tela onde a folha por acaso carrega. Um par que a guarda de posse listava como exceção — `.column-actions` e `.column-status` em duas folhas — passa a ser impossível por estrutura, não por promessa. Zero movimento: 26.082 pares de estilo computado idênticos em 7 telas × 2 temas.
...
- **A segunda escada de espaçamento saiu** (#1176): `--ffc-spacing-5` e `--ffc-spacing-15` eram o resto de uma segunda progressão e foram **deletados**, com as 182 declarações repontadas ao degrau vizinho; os literais `14px`/`18px` — os pontos médios exatos dos dois maiores vãos — encaixaram junto. Movimento medido e limitado: 178 declarações andaram 1px, 63 andaram 2px, nenhuma mais. Nove utilitários nomeados por pixel passaram a ler o degrau.
- **A gaveta de utilitários do admin foi esvaziada** (#1171): nove componentes de tela moravam em `ffc-admin-utilities.css` — `ffc-preflight-badge` sozinho tinha cinco regras, com `a`, `a:hover` e `strong`. Foram para a folha do módulo que os renderiza, **sem mover um pixel**: o conjunto das 2.498 regras é idêntico e cada uma das três telas foi re-renderizada com só as folhas que ela enfileira. `UtilityScopeTest` passa a recusar componente na gaveta.
- **Um idioma só para estado e variante** (#1170): 38 classes nossas eram declaradas sem prefixo — `.value`, `.label`, `.open`, `.remove`, `.selected` — e nenhuma dizia de qual componente era. Todas convertidas, **sem mover um pixel**: cada regra do develop, com o mapa de renomeação aplicado, é idêntica em seletor e declaração à de agora, e os 40 nomes novos têm emissor achável. Seis regras mortas que o nome nu escondia saíram junto.
...
- **Hub de e-mails não oferecia um token que o próprio padrão usa** (#1143): a lista de ajuda por e-mail é texto escrito à mão ao lado de um template mantido em outro lugar, então ela deriva em silêncio — nas duas direções. Guarda nova compara catálogo, allowlist e arquivos: um token usado no corpo padrão e ausente da lista falha o CI.
- **E-mail de aprovação de agendamento não trazia o link de cancelamento** (#1143): num calendário que exige aprovação, o e-mail da reserva só diz "aguardando aprovação, você receberá a confirmação" — a confirmação **é** o e-mail de aprovação, e ele era o único dos cinco e-mails de agendamento sem o link. Quem tinha o agendamento de fato confirmado era justamente quem não conseguia cancelar pela caixa de entrada. A cobertura existente provava o token resolvendo dentro de um override do hub, o que não diz nada sobre o corpo que uma instalação sem override recebe.
...
- **Reativar uma campanha de recadastramento não avisava ninguém** (#1125): ao expirar, as submissões pendentes viram `expired`; a reativação não as reabria — e a semeadura pula quem já tem linha — então o usuário ficava num estado que nem deixava enviar nem desenhava aviso. O convite por e-mail também não saía, porque busca só `pending`. Reativar agora reabre as `expired`, sem tocar em `approved`/`submitted`.
- **Quem entra na audiência depois da ativação não via a campanha** (#1125): a semeadura é evento único na transição para ativa, então esse usuário nunca ganhava linha e caía no mesmo vão mudo. Agora "sem submissão" é um estado que pode enviar — o que também cobre a submissão excluída pelo administrador, devolvendo o usuário ao início.
- **Modo escuro: as 28 folhas passam a pintar pela paleta** (#1126): metade delas não usava token nenhum — calendário, encurtador, recrutamento, recadastramento, submissões, painel de certificados, público-alvo, modelo de e-mail e as demais pintavam com hex fixo, então badges, modais, tabelas e o balão de toast ficavam claros dentro da tela escura. A catraca bloqueia em zero; o que segue literal é decisão com o motivo escrito ao lado.
- **A paleta precisa estar na página, e ela não estava — de quatro formas** (#1126): uma custom property não declarada **invalida a declaração inteira**, não cai para o literal anterior. Dezesseis enfileiramentos liam `var(--ffc-*)` sem declarar `ffc-common`; o dashboard e a lista de editais redeclaravam `--ffc-*` localmente, o que sombreia a paleta para tudo ali dentro; duas regras liam token que nunca existiu, escondidas atrás do valor de reserva; e a página pública de cancelamento monta o próprio documento, então nunca passou pelo enfileiramento — ficou sem fundo, sem cor de texto e com o botão despintado, **nos dois temas**.
- **O interruptor não chegava a quatro telas públicas** (#1126): calendário, público-alvo (dois caminhos) e editais enfileiravam a paleta mas não o script que põe `.ffc-dark-mode` no `<html>`. Sem ele todo `var(--ffc-*)` resolve pelo bloco claro, por mais tokenizada que a folha esteja. A paleta sozinha não é o tema.
- **Texto que não declara cor herdava quase-preto, e controle de formulário nem isso** (#1126): quem não declara `color` herda de fora do plugin — do `body` do wp-admin ou do tema — e cai em **1,28:1** sobre fundo escuro; era a legenda do calendário e o rótulo "Modelo:". Um `<button>`, `<input>`, `<select>` ou `<textarea>` **não herda `color`** e nem a regra de base o alcança: eram 18, incluindo todo campo do formulário público de recadastramento e da edição de perfil. As tarjas do admin precisaram de terceira resposta — nomear os nós de texto, porque qualquer declaração direta vence um valor herdado.
- **`opacity` e `.alternate`: dois defeitos que nenhum token corrigiria** (#1126): a linha passada/cancelada da agenda levava `opacity`, que desbota texto **e** fundo juntos e derruba qualquer contraste que os tokens garantiram — 3,11:1 medido. E `.alternate` é a terceira forma de zebrar linha do WordPress, a que o plugin escreve à mão sete vezes só na aba de Cache.
- **Cor inline vencia a folha tokenizada** (#1126): o aviso de e-mails desligados carregava a paleta inteira num `style=""` do PHP, em cinco telas; badges e faixas de edital fixavam `#333` sobre uma cor que o **administrador escolhe**. Para o segundo caso nenhum token serve — o texto passa a ser calculado por luminância WCAG, com 4,58:1 garantido sobre qualquer cor do cubo RGB.
- **Três rótulos de botão reprovavam o contraste** (#1126): sucesso (3,35:1) e aviso (3,04:1) sobre branco, e `.ffc-text-warning` usando cor de sinal como texto. O verde escureceu para `#00801e`; nasceram os pares `on-success` e `on-warning` — o primeiro inverte entre os temas, o segundo não, porque o aviso é tom médio-claro nos dois.
...
- **Dez pares de cor da paleta reprovavam o contraste mínimo** (#1132): o pior era o rótulo do botão primário no tema escuro — branco sobre a primária clara, 2,52:1, desde que o tema escuro existe. O Material 3 inverte esse par no escuro (a primária clareia, o texto sobre ela escurece) e o plugin só tinha feito metade. Corrigidos também os contornos de campo e card nos dois temas e o anel de foco, que compunha 1,78:1 no claro.
- **Placeholder e indicador de aviso ficaram mais escuros no tema claro** (#1132): `#8c8f94` e `#dba617` vêm da paleta do próprio WordPress e reprovavam 3,24:1 e 2,22:1. A correção afasta um pouco o plugin do visual do wp-admin — foi decisão deliberada, em favor da legibilidade.
- **Cinco dos oito mutadores de audiência não invalidavam o cache de audiências do usuário** (#1127): a chave morava num grupo literal próprio, fora do par Reader/Writer, então cada mutador tinha de lembrar sozinho — e `bulk_add_members()`, o caminho do admin, não lembrava. Decidia errado a campanha de recadastramento no painel e os campos personalizados no perfil. A chave passa a morar no grupo do par, com um só ponto de invalidação e TTL, que antes não havia.
- **Sair de uma audiência não limpava o cache de quem saiu** (#1127): `set_members()` invalidava só a lista que entrava, então quem era removido seguia vendo a audiência. `cascade_self_join()` não invalidava nada — nem por id, nem a versão de consulta — e altera `allow_self_join`, que a lista devolve.

### Changed

- **Cancelar um agendamento pelo painel passa a abrir um diálogo, não um `confirm()`** (#1140): o `confirm()` não dizia **qual** agendamento seria cancelado e não tinha campo de motivo — então um cancelamento pelo painel gravava motivo vazio enquanto o mesmo pelo link do e-mail gravava o que a pessoa escreveu. O endpoint já aceitava e sanitizava `reason`; a lacuna era só do cliente. O diálogo mostra serviço e data/hora, e reaproveita as mesmas strings da página pública.
- **Modal vira componente compartilhado na paleta** (#1140): havia duas cópias — uma no público-alvo, outra no admin de recadastramento — e o painel seria a terceira. A do público-alvo foi para `ffc-common.css`, que toda folha que já a usava declara como dependência. A do admin ficou onde está: seletores sem escopo, outra tela.
- **Entradas numéricas do admin fora das abas de configuração agora exigem valor** (#1117): 19 campos ganharam `required` — auto-agendamento, quiz, recrutamento, rerrematrícula e modelo de e-mail. Oito ficaram de fora, cada um com o motivo travado em teste: em cinco vazio significa "herdar o global" ou "sem limite", um não é campo de formulário, e em dois a obrigatoriedade pertence à definição do campo.
- **`required` passa a acompanhar os blocos que o JS esconde** (#1117, #1120): campo obrigatório escondido trava o envio apontando para um controle que ninguém vê. Fecha junto dois casos já vivos — as linhas de horário de trabalho do editor de calendário, e os dois campos de hora do perfil de usuário, obrigatórios dentro de seções recolhíveis.
...
- **Nenhum status de recadastramento produz mais tela em branco** (#1125): o painel tratava `approved` e `submitted` e caía num `continue` mudo para todo o resto. Ganhou ramo final mostrando o status legível — os dois casos que caíam ali estão corrigidos na origem, e o ramo existe para o próximo status não repetir a história.
- **Modo escuro passa a repintar o que o WordPress desenha** (#1126): o modo escuro era só a troca de tokens, e nada aplicava esses tokens ao chrome do core — então rótulos de `form-table` ficavam escuros sobre fundo escuro e os campos continuavam brancos dentro do card escuro. Rótulos, campos, cards, metaboxes, tabelas de listagem, abas, botões secundários e avisos passam a seguir a paleta. Falta ainda a metade das folhas de admin que não usam token nenhum.
...
- **O guarda de modo escuro passa a medir contraste** (#1132): antes só procurava cor literal — via presença, não legibilidade. Agora calcula a razão WCAG de cada par que o CSS realmente pinta, nos dois temas, e bloqueia em zero. Teria acusado o botão primário do tema escuro no dia em que ele foi escrito.
- **Catraca de cor literal por folha de estilo** (#1126): as sete folhas convertidas bloqueiam em zero; as demais carregam o número medido como orçamento que só encolhe. Junto vai a metade que faltava — toda folha que lê `var(--ffc-*)` precisa declarar `ffc-common` como dependência, que é o que achou os dezesseis enfileiramentos sem paleta.
