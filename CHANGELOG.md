...
**i18n cleanup — recruitment module.** Closes #85. Every `__()` / `esc_html__()` / `esc_attr__()` key in the recruitment module that was authored Portuguese-source is now English-source, per the §2 plan rule (Portuguese flows in via `languages/ffcertificate-pt_BR.po`, never as the source string). Touches the public shortcode (`Edital não informado.` → `Notice attribute is required.`, `Posição` → `Rank`, etc.), the candidate-self dashboard (`Minhas Convocações` → `My Calls`, `Classificação preliminar — sujeita a revisão` → `Preliminary classification — subject to review`, etc.), the admin page (`Código` → `Code`, `Criar novo edital` → `Create new notice`, etc.), and the default email template + subject in Settings. PT translations for every new EN msgid were appended to the bottom of `languages/ffcertificate-pt_BR.po`; user-facing rendering in pt-BR is unchanged. Tests that asserted on the rendered PT strings were flipped to assert on the new EN sources (Brain\Monkey's `__` stub returns the source verbatim during unit runs, which is the same behavior as a missing `.mo` lookup).
