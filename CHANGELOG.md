...
### Added
- **Per-module enable/disable toggles** in the Settings → Modules tab (#800): a switch per feature module (Certificates, Audiences/Scheduling, Self-Scheduling, Reregistration, URL Shortener, Recruitment) that gates the module's bootstrap in `Loader::init_plugin()`. All default ON; disabling hides a module's screens and stops its runtime but never deletes data. Saves instantly via the settings-autosave endpoint.
