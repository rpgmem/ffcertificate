...
- **Reregistration: a read-only auditor view (GAP C, 3-state model).** The Reregistration admin (top-level menu + Campaigns) now opens for `ffc_view_reregistration` as a *só vê* tier — the campaigns list and per-campaign submissions render read-only, while every write still requires `ffc_manage_reregistration`: the campaign editor (`new`/`edit`) is denied to viewers, and the "Add New" button, per-row Edit/Delete, the submission Approve/Reject/Return-to-draft actions and the bulk-actions bar are hidden (a read-only row's title links through to its Submissions view instead of the editor). The AJAX handler and the GET-link action handler keep enforcing the manage cap server-side, so hiding the controls is UX, not the security boundary. First slice of GAP C; audiences, appointments and the recruitment admin UI follow (see `docs/plans/gap-c-readonly-admin.md`).
