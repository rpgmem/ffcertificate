...
### Fixed
- Multi-role capability masking (#739): a user holding **FFC Administrator** *and* **FFC End User** could lose the form/calendar CPT menus (`ffc_view_forms` / `ffc_view_calendars` / `ffc_manage_forms` / `ffc_manage_calendars`). A legacy `ffc_* => false` denial left on the `ffc_end_user` role — carried onto renamed caps by earlier migrations — masked the same capability granted `true` by the aggregator role, because WordPress merges role capability maps with `array_merge()` and a later role's `false` overrides an earlier role's `true`. A one-shot, version-flagged migration (`strip_false_ffc_caps()`) now removes every falsy `ffc_*` entry from all FFC-managed roles, and `upgrade_role()` was hardened to strip `0` / `''` denials (not only a strict `false`); an absent cap already denies for single-role users while letting a peer role's grant survive the merge.
