...
### Added
- **Global email-body override layer — foundation** (#964, phase 1): `Core\EmailTemplates` gains a global-override store (option `ffc_email_bodies`) and a cascade — `body()` now returns the admin's global override when set, else the shipped file default (`shipped_body()`), with `global_body()` / `save_global()` / `clear_global()` for the upcoming hub. Every send path and "Restore Default" already resolves through `body()`, so **audience booking/cancellation emails are immediately globally-overridable** through this change; the remaining emails wire in over follow-up increments. Dormant by default: with the option empty, `body()` is byte-for-byte the previous file-only behaviour — no change on upgrade. Groundwork for the #964 email-body hub.
