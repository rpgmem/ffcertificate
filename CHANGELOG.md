...
- **Recruitment CSV import: the actual cause of the `Duplicate entry '1668-1-1-'` cascade.** `ffc_recruitment_classification.list_type` was declared as `ENUM('preview','definitive')`, but the batched importer (#462) writes `__staging_<uuid>` markers into that column while a job streams batches in. MySQL's default non-strict mode silently coerced every invalid ENUM value to the empty string `''`, so every staging row from every concurrent job collided on the UNIQUE `(candidate_id, adjutancy_id, notice_id, list_type)` index — visible as the misleading 3-component error in `debug.log`. V9 migration widens the column to `VARCHAR(50)` (the two enum strings round-trip cleanly, no data change for live rows) and purges legacy `list_type=''` rows the broken staging strategy left behind. The first import after this deploys will succeed; the `__staging_<job_id>` markers now persist correctly and the existing `cleanup_orphan_staging_rows()` sweep starts seeing them.

- **Recruitment CSV import: silent UNIQUE-constraint duplicates surface as a clear error.** `validate()` only checked duplicate `(cpf + adjutancy)` pairs, but `upsert_candidate()` collapses rows that share ANY of `cpf` / `rf` / `email` onto a single candidate record — so a CSV with two rows sharing the same RF (with different CPFs, or one row with no CPF) used to slip past validation and then trip the `uq_candidate_adjutancy_notice_list` UNIQUE constraint at INSERT time. The atomic single-request flow hid this with a full ROLLBACK; the new batched flow exposed it as a 400 on a mid-import batch. `validate()` now runs a physical-identity pre-pass that groups rows by any shared `cpf` / `rf` / `email` and detects `(logical_candidate + adjutancy)` duplicates, surfacing the offending line numbers before any DB write. The duplicate error code changes from `recruitment_csv_duplicate_cpf_adjutancy` to `recruitment_csv_duplicate_candidate_adjutancy` to reflect the broader rule.
- **Recruitment CSV import: staging rows left behind by killed jobs are now reaped.** When a batched import is interrupted (browser closed, network drop, mid-batch failure) the `__staging_<job_id>` rows it inserted stay in `ffc_recruitment_classification` invisible to the live list (different `list_type`) but consuming index space. `start_job()` now calls a new `cleanup_orphan_staging_rows()` helper that drops every staging marker whose owning transient is gone. Runs at most once per import (no extra cron needed).
