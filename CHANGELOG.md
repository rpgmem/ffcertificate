...
- **The destination account is chosen from a search that says in advance whether the move would be accepted** (#1397): a dialog leads with the account already filing one of the identifiers the records carry — or says plainly that none does — then searches by name, e-mail or account number, and labels each result with the verdict the write would reach: agrees by CPF, would gain the RF, or refused with the reason. A refused account cannot be selected. The verdict comes from the service's own rule rather than a copy, so the dialog and the write cannot disagree. Choosing a destination bars splitting onto a new account, since two destinations for one set of records is not a thing to express. The number field stays, so the screen keeps working without JavaScript.
- **A correction can be checked before it is committed, and a shared number corrected on one login only** (#1397): `Check` says what the typed number would do — how many records it rewrites, or the refusal in the service's own words. The refusal that matters names the account holding the value and links to it, because that case is a merge decision rather than a failed correction. On the two-accounts panel each login gets its own correction, scoped to that login: the same rewrite without a scope would reach the other person's rows, since both carry the number. The isolated panel now names the person, read from the records rather than from an account — a recruitment candidacy has no WordPress user, and a submission-only finding says no name is recorded rather than showing a blank. The field is never pre-filled and no stored identifier is ever sent back.
- ⚠ **A merge is now one pair at a time, previewed and acknowledged** (#1397): `Show what would move` names who keeps the records and who is emptied, counts the records per store, and says which identifier the surviving login gains — read from the merge's own resolution, so what is acknowledged is what runs. It also reports how much each login holds, which is the evidence behind keeping the one in use. An explicit confirmation is a posted field rather than a browser dialog, and its absence refuses instead of doing nothing. **This replaces the batch of checkboxes**: each pair is its own form, nonce and transaction, so an outcome can no longer be part merged and part refused. The emptied login is still never deleted.
- **Records with an identifier and no account are found and can be given one** (#1397): a new panel reads all three stores and both identifiers, and — unlike the query it widens — returns orphans that match no account at all, which is exactly the population needing an account opened rather than a link made. Each finding says which of CPF, RF and e-mail it carries and which it lacks, read as the union over its rows rather than from one of them. Two actions: link it to an existing account, through the same search and the same agreement rule a move uses; or open one, which needs all three identifiers because an account opened from a single one is one the resolver will fail to match on the next record carrying another. Opening goes through the plugin's single account-creation path, so an account that already answers is used rather than duplicated. A record erased at the subject's request cannot appear here — the eraser clears the identifier hashes along with the ciphertexts.
- **The two account tiers on the identity screen are cards, not table rows** (#1407): each finding now says in a sentence what the check digits already decided, names the person rather than an account number, and shows the two identifiers as coloured chips with an arrow from the one that goes away to the one that survives — drawn only where the digits actually decided, since on the tier they could not the hashes are a set and an arrow would assert a choice nobody made. A value that could not be read is drawn as unknown rather than as a failure, which is the distinction the queue itself makes. No form, nonce or field changed. It also repairs a link colour nothing had ever declared: wp-admin's blue on the plugin's dark card measured 2.48:1, and the panel now paints links from the palette — 5.10:1 in dark, unchanged in light.
- **The isolated and orphan panels are cards too, so no list table is left on the screen** (#1407): the number to correct and the records belonging to no login now read the same way as the account tiers — the person's name from the record rather than from an account, the identifier as a chip, the stores and row ids that are the only handle on a record no account names, and the verb in a column of its own. The two cards are deliberately not identical: the isolated tier states how many records carry the value, because its scan actually counts them, while the account tiers state none rather than invent one. On an orphan the card still says what the record lacks as well as what it carries, which is what decides whether the answer is a link or a new account.
- **The identity screen says how much you have resolved, beside how much is left** (#1407): a count of the findings this operator has resolved since the queue was read, from the activity log. The window is the held queue's own rather than a clock's — `N left` counts the list taken at that instant, so the only number that can sit beside it honestly is one measured from the same point, and `Read the queue again` resets both together. It is scoped to the operator, because the held list is theirs: another operator's work does not shrink it. Zero is not rendered.
- **Three small corrections in the test suite, found by auditing it rather than by a failure**: the coverage config excluded `includes/libraries` and `includes/views`, two directories that have never existed in the repository's history — PHPStan dropped them once and the two lists had been out of step since; two guards annotated `@covers` on a `tests/Support/` helper, which attributes coverage to nothing because the scope is `./includes`, and now say `@coversNothing` with the reason; and the sixteen per-file assertions that each tab extends `SettingsTab` became one guard over the directory, which covers all eighteen tab classes — the hand-written convention had missed `TabCaptcha` and `TabIpDiagnostics` — and covers any tab added later.
- **The batched CSV export's contract is asserted once instead of thirty-one times** (#772): six source tests each rewrote the same five or six interface behaviours — the authorize fences, the keyset cursor, the job owner — because each builds its own source and the assertion belongs to the interface. They now inherit one contract test case. The reason it is a base class and not a tidy-up: five of those thirty-six assertions had never been written at all, so `authorize_download`'s user fence went unasserted on the appointment and reregistration sources, `cursor_of` on submissions, and `job_owner_fields` on submissions and the url shortener — the last being the source that decrypts PII onto a temp file. All four implement the behaviour correctly; nobody had copied the test. A seventh source now inherits all six the moment it extends the base.
- **Three things the identity screen showed wrong, found on the testes host** (#1407): a card read `self_scheduling_appointments|user_profiles` — a table name joined by the query's own internal `|`, which exists to survive a `GROUP_CONCAT` and was never meant to be read. Stores are now named in the reader's language and joined the way WordPress joins a list, with the identity index named for what it is rather than listed as if a record lived there. The scan strip carried all three of its counts in one string, so a single stored value read `1 valores distintos verificados`; each count now picks its own plural. And on the tier where an account holds several numbers, the action column rendered two verbs per number with nothing saying which was which — each identifier now opens a group that names it, with the sentence explaining the verbs beside them.
- **The one untested product class the audit could not argue away now has a test, and a rule that produced 101 inert lines was re-measured** (#1053): `CaptchaModeNotice` — the nudge toward the ALTCHA-only captcha mode — shipped in 6.23.0 with no test at all, in the same commit as the two other gaps that audit found. Its three gates are now pinned, the composite mode included: a naive "only nudge the math challenge" gate would skip it, and composite accepts either proof, so it is no stronger than math. The other class with no test, `HttpCsvDownload`, keeps the exemption argued in its own docblock. The audit's second half measured the `class_exists()` pcov preload that 101 test files carry: the attribution gap it was written for no longer reproduces on this toolchain, so new tests stop adding it while the existing ones stay — a line that costs nothing and would be churn to sweep.
- **The 38 findings the identity queue should never have offered a verb for now get none** (#1368): an account whose numbers share one address and are not variants of each other is a shared mailbox, or an account submitting on behalf of other people — the premise every other account-side tier rests on, that an address identifies a person, is exactly what does not hold there. It is now a tier of its own, drawn as a card like its neighbours and with an empty verb column that says why it is empty. It outranks the mechanical tier on purpose, so a two-number case the check digits could have resolved is refused too: refusing a real typo costs a manual correction, while accepting a shared mailbox writes one person's number over another person's records. Both halves of the reading were already measured by the audit and reached no screen.
- **The merge form says what each login holds and when it was last used, beside the choice itself** (#1368): both readings were already measured by the audit and exported in the CSV, and neither reached the form — the record counts arrived only in the preview, one click after the operator had already picked a survivor at the radio. They now sit under each option, read through the same `account_facts()` the export reads so the screen and the CSV cannot disagree. **Nothing is proposed**, deliberately: the data can say which login holds more and when each was last seen, never which is the person's real one. An absent count and an absent date each say so in their own words rather than reading as an empty login or a login never used.
- **The identity screen's buttons say which one commits** (#1421): on cards whose verbs rewrite records and cannot be undone, every control was the same grey rectangle. The verb that writes is now the card's primary where the card has one — and deliberately not on the tier offering two destinations, where promoting either would be the screen answering the question it is asking. The `Move` button drops the hash it repeated from the heading directly above it and keeps it in its accessible name, so the two buttons are still announced apart. Both search buttons gain a magnifier, the writing column a 44px target, and `Previous` / `Next` become the arrows they always meant, labelled for screen readers and by tooltip.
- **The identity-index backfill card reports the accounts it may not decide, and links to the screen that can** (#1368): an account holding two different numbers for one field is left empty on purpose — choosing would destroy the evidence that they disagree — so it was outstanding work the card never mentioned once the walk had finished. It now travels as its own number beside the bar rather than inside it, with no re-run offered: pressing the button moves it by zero, and a card whose number never moves is how an operator learns to ignore it. The link is gated on the identity capability, and an account in conflict on both its CPF and its RF is counted once rather than twice. A count that could not be taken renders nothing, because zero conflicts and an unread scan are different answers. The audit card below it gained the same link, closing a loop that ran one way: the identity screen has offered that card's CSV since 6.28.3 and the card named nothing.
- **The comment-language guard reads the stylesheets, and 44 Portuguese comment blocks left them** (#1260): `CommentLanguageTest` walked PHP and JS and reported the tree clean while 19 of the 28 sheets carried Portuguese prose — its `is_comment_line()` already understood `/*` and `*`, which is the whole of CSS comment syntax, so the sheets were always readable and simply were not read. A guard's reach is a claim about what it looked at, and this one's was narrower than a reader would assume. Nothing in the CSS itself moved: with comments stripped, all 28 sheets are byte-identical to what they were, and the minified bundles are unchanged because `cleancss` already dropped the comments.
- **The identity card whose account holds several numbers stops opening a void beside the record** (#1407): its verb column is 17em wide, so each identifier's controls stacked onto their own lines and two identifiers came to 470px — against four short lines in the column beside them. Measured in Chromium at 1600px, the card was 473px tall over 68px of content, leaving 357px of white; it now measures 164px with 7px of slack, and the groups sit side by side. Scoped to the tier that has more than one group, so the two tiers holding a single control are byte-for-byte what they were.
...
- **The identity screen no longer fatals on a class its view never imported** (#1397): a view is `require`d into the caller's scope but does not inherit its imports, so an unqualified name resolves against the global namespace — and `IdentityQueuePanels` was named there without a `use`, which made the whole screen a fatal from the moment the stepper landed. Nothing saw it: PHPStan and the coverage report both exclude the `views/` directories as markup, a carve-out justified by those files carrying no queries, options, nonces or capability checks — which says nothing about whether a name resolves. A guard now checks every view and template in the plugin, blocking at zero.
- **The identity screen opens with what the scan read and what is left** (#1407): a strip stating how many distinct values were checked, how many could not be read and how many stores were scanned, and whether either cap cut the reading short. Those numbers used to render only when the queue came back empty, so the one state an operator actually works — a queue with findings — never said how much of the data it had seen, and a partial list read as a whole one. Its verdict is three states, not a tick: a scan that read nothing says so however few findings came back. Beside it, one counter per category with its own colour, summed from the same panels the bodies below are drawn from, and the audit CSV for whoever holds the capability that serves it.
- **A tooltip no keyboard could reach** (#1421): the admin sheets style a tooltip on `.button[title]` for hover only, so the label of any button that carries one was available to a pointer and to nothing else — `CLAUDE.md` described the family as having a focus half that was never written. It now shows on `:focus-visible` too, on every FFC admin screen (WCAG 1.4.13).
- **The comment-language guard read 46% of a stylesheet, and the Portuguese was in the other 54%** (#1423): its line filter asked whether a line *started* with `*`, `/*` or `//` — right for PHP and JS, where 99% of comment lines do, and wrong for the sheets, whose continuation lines are indented with no marker. It now tracks the block delimiters across lines and returns only the comment span, which also reads trailing comments for the first time. 135 lines were hiding: 131 across 16 stylesheets and 4 in PHP, one of them a `phpcs:ignore` justification. All are now in English.
...
- **One canonical form per identifier, owned by the hash boundary** (#1313): `SensitiveFieldRegistry` declares how each CPF, RF and e-mail is canonicalised and applies it before encrypting or hashing. The hash function was always uniform; the string fed into it was decided per call site, and several had decided differently.
- **Two migrations that repair what was already stored** (#1313): *Canonicalise Stored Identifiers* rewrites each identifier across submissions, appointments, candidacies and the user profile and rebuilds its hash — idempotent on the plaintext, never on the ciphertext, since a random IV would never converge. *Backfill the Identity Index* then copies the hashes already linked to a user into the index, filling empty columns only and refusing to start until the first completes.
- **The link audit now spans every store and reports the conflict count** (#1313): three checks across submissions, appointments, candidacies and the index — one identifier on two accounts, one account with two identifiers, and identifiers the index lacks. The first is the number a merge policy would have to be designed against; the third reads zero once the backfill finishes. Report-only.
- **Three guards that close the identity shape rather than its instances** (#1313, #1314): no value reaches `Encryption::hash()` outside the registry, nothing but `UserCreator` creates a user, and no file re-implements a length test, a digit strip or a case fold. The first two read the token stream, not lines: a registry description contains the text `Encryption::hash()` inside a translated string, and `wp_create_user_request()` is unrelated.
- **The identity-index invariant, checked against a real database** (#1313): a `fresh-install` step plants an orphaned submission carrying a CPF hash, asks the plugin's own resolver to resolve it, then asserts the row was adopted and the index carries the hash. It plants first because the invariant would otherwise hold over nothing — the one guard of the three no static scan can make.
- **Three guards over the translation catalogues** (#1266, #1284, #1209, #1282): `.po` ↔ `.mo` ↔ `.l10n.php` must agree both ways; every source string must have a translated entry, and every entry a call site. They had disagreed in production for two releases, and stayed internally consistent through the two in which 79 strings had no entry at all.
- **A guard that blocks Portuguese prose in comments and source strings** (#1260): `CommentLanguageTest` flags a line carrying two distinct Portuguese function words, quoted spans removed. Two is what separates a sentence from `example.com` or `DoS`. Tuned against the 160 lines #1278 removed: 59 of their 60 blocks, zero false positives — and it found six more on a tree three passes had called clean.
- **Four guards over uninstall and schema reachability** (#1290, #1291, #1302, #1311): the sweep's precondition is that every capability carries the `ffc_` prefix; every `CREATE TABLE` must resolve to an `ffc_` name and to a declarer the `Loader` reaches; and no FFC capability may sit on a role WordPress owns — that last one resolved at runtime, so no static scan reaches it.
- **A guard that a deprecation cannot outlive its own removal release** (#1309): `DeprecationDueTest` reads `@removal X.Y.Z` on the code that is still there and fails in the release that must act, and requires the tag on every `_deprecated_*()` call so a cycle cannot ship with no end date.
- **A guard that only a refresher may name a captcha field** (#1307, #1305): a script outside the two registered refreshers reaching for `ffc_captcha_ans`, `ffc_captcha_hash` or a `name="altcha"` position fails — the shape a hand-written field list shipped in.
- **A guard that a documentation page listing a capability group lists all of it** (#1214): ten pages hand-write capability lists, and three had fallen behind by one each — two of them the capabilities that decide whether a screen shows somebody's CPF.

### Changed

- **Identity resolution asks one indexed profile first, and every link feeds it** (#1313): `UserCreator` consulted `ffc_submissions` alone while its own orphan-adoption happily claimed appointment rows, so the plugin declined to recognise someone it would adopt a record for moments later. `ffc_user_profiles` gains indexed `cpf_hash` / `rf_hash`; `wp_usermeta` indexes `meta_key` and never `meta_value`, so the question the hash exists to answer was a scan. Submissions stay as the fallback.
- **Certificates, appointments and the audience import all resolve through the index** (#1313, #1321, #1302): the two highest-volume writers were still on the legacy single-hash entry point, which reads one table and feeds no index. The audience CSV gains optional `cpf` / `rf` columns and asks the index before the e-mail — a person registered under another address used to get a second account, or have their row dropped as *User not found*. An imported member now gets the `audience` context.
- **An e-mail is hashed in lowercase everywhere** (#1313): `sanitize_email()` strips invalid characters and does not lowercase, so an appointment booked as `Joao@Escola.gov.br` wrote a hash no other module's lookup could find. The rule ships with the migration that rewrites the stored rows — alone it would canonicalise against rows that did not.
- **Forty-four sites asked the same three questions and each answered for itself** (#1314): *seven digits is an RF* had nine copies, the canonical form of an address had three spellings with two wrong, and `normalize_cpf_rf()` — present since 6.6.1 — had its body inlined at twenty sites rather than called. The four entry validators keep their own length test on purpose: rejecting a length is not routing one.
- **The reregistration campaign gains a CSV import** (#1214): two tables of in-flight state with a TTL, and a new `ffc_import_reregistration` capability carved out of `ffc_manage_reregistration` — loading a campaign's answers writes encrypted PII for people who never opened the form. Seeded onto current holders by bumping the one-shot migration's flag to `_v2`, since a flagged migration never revisits an install.
- **Four phases, and the report exists to be read** (#1214): staging matches the header against one audience's `ffc_custom_fields`, exactly and never fuzzily; validation blocks the whole job on any failure, because promotion is batched across requests and cannot undo an earlier batch; promotion writes through `ReregistrationDataProcessor`, never the `data` JSON; commit deletes the staged cleartext. Two clicks, on purpose.
- **A read-only identity probe beside the resolver that creates** (#1214): `resolve_existing_user()` runs the hash and e-mail steps and stops — the `peek()`/`verify()` split the captcha contract already uses. Both share one lookup, so a probe never finds more than promotion will. Two rows resolving to one account block the job, which is how a shared mailbox presents (#1295).
- **No temp file, and the plan had budgeted for one** (#1214, #772): the PII-on-disk discipline — `ffc-tmp`, an `.htaccess`, a cleanup cron and the nginx caveat — does not apply. Staging reads the upload straight into the database, so the only file is PHP's own upload temp.
- **The import sends no e-mail, and each half had its own reason** (#1214): 500 accounts meant 500 synchronous notifications from inside a batch loop, and the confirmation reads *"we received your reregistration"* to somebody who did nothing. Suppressed by an explicit flag, not the global kill switch. Errors render from a stored code, not a stored sentence, and every phase is fenced to the operator who started the job. Documented in-plugin (#1292).
- **The campaign invitation stops asking for a reregistration that is already complete** (#1300): it selects on `invited_at IS NULL` whatever the status, so it reaches everyone imported by CSV and anyone who used the dashboard banner early. The sentence and the button label now come from the status as two tokens — they move together, since a line saying the record exists under a button reading *Complete Reregistration* contradicts itself.
- **The reregistration `Ficha` is now the `Record`** (#1264): 2 classes, 2 methods, a JS function, a constant, an AJAX action and ~50 on-screen strings. The five public `ffcertificate_ficha_*` filters and the old `ffc_generate_ficha` action ride `apply_filters_deprecated()` until 6.28.0 — a rename there breaks an external integration in silence.
- **Four Portuguese values stay stored, and that is the point** (#1264): the `KIND_RECORD` value, the `ffc_reregistration_ficha_template` option, the seeder's `default_ficha` slug and `FilenameHelper`'s `ficha` type all reach the database or a public filter argument. Renaming the seeder slug alone would have inserted a **second** default template on every existing install. pt_BR reads exactly as before.
- **The codebase now reads in English** (#1260): 39 runtime files, 35 test files, 15 production classes and the three most recent CHANGELOG sections, plus eleven Portuguese identifiers (`get_sexo_options` → `get_gender_options`, …) and a JS file renamed with its handle. Comments and messages only, bar the named renames: every token skeleton is byte-identical. `divisao_setor` and `nome` **stay**, being stored keys.
- **An error message shipped as a Portuguese source string** (#1260): the key-rotation migration reported `'Nao foi possivel decifrar…'`, so a non-Portuguese install had no way to read it and no translation could fix it — a source string is the one place the language is not a preference.
- **The earlier slices' Portuguese scan was too narrow, and 160 comment lines survived it** (#1260): a dictionary of accented function words misses unaccented prose (`asercao`, `nao`, `so`) and tier names it never listed, so 35 files merged as translated still carried a Portuguese clause mid-paragraph.
- **`CLAUDE.md` states the language rule, and its own numbers were audited** (#1260, #1261, #1202, #1171, #1280): every versioned artifact is English while the conversation is not. 517 numeric tokens were traced to whatever owns them — three sentences stated a conclusion that had stopped being true, ten counts had drifted, and four claims were simply wrong. Counts carrying nothing were replaced by the invariant they stood in for.
- **`CLAUDE.md` splits its two overgrown sections** (#1261, #1288): 31 KB of guard stories sat under a heading promising a list of CI gates, and 67% of "Architecture and patterns" was stylesheets. Both split, every subsection body byte-identical. Nine cross-references now name their section instead of numbering it — the renumbering had silently broken six.
- **The removal release is now a tag, not a sentence** (#1309): the first version of the guard read the prose already in the tree and reported two docblocks that say the opposite of a promise — past-tense history about an argument removed in 6.24.0. `until X` is a promise or a finished story depending on a verb several clauses away, so the nine live sites carry `@removal` instead.

### Deprecated

- **`UserManager::get_or_create_user()` and `UserCreator::get_or_create_user()`** (#1313): the single-hash entry point, superseded by `get_or_create_user_dual()`, where argument position carries the identifier kind rather than a separate `$identifier_type` that could disagree with the value. No product caller remains, but both are `public static` on classes another plugin can call: notice in 6.26.0, **removal in 6.28.0** (#1245 / #730).

### Removed

- **A registry key nothing read** (#1313): `requires_column` was declared on all eight migration cards and consulted nowhere; the real prerequisite check has always been each strategy's `can_run()`.

### Fixed

- **A masked CPF hashed its punctuation, in two places** (#1313): `UserProfileService` hashed the value exactly as handed to it, so a reregistration submitting `123.456.789-09` — what the form's own mask produces — wrote a hash no other module could match. Admin submission search had the mirror defect, comparing one hash of the raw term against three columns at once.
- **40 of the 60 live capabilities survived uninstall on every install** (#1290): removal intersected a hand-written list naming 31 of them, so `ffc_manage_settings`, both PII capabilities and 37 others stayed as user-meta and role grants after deletion. Now a prefix sweep over `ffc_*`, on users **and** on roles the plugin does not own.
- **Every install put an FFC capability on WordPress's own `subscriber` role** (#1302): `AudienceActivator` granted `ffc_view_own_audience_bookings` there at activation, reaching every subscriber whether or not they belonged to any audience. It covered a real gap — only the self-join route granted it — so the grant moves to `AudienceWriter::add_member()`, with a migration seeding existing members first.
- **A new WordPress administrator saw "FFC Settings" and none of the six feature menus** (#1302, #747): the move onto `ffc_administrator` had a one-shot back-fill, so an administrator created afterwards got neither tier, and every top-level menu registers against a raw `ffc_view_*` slug with no `manage_options` escape. `ffc_administrator` now follows `administrator` continuously.
- **Table removal no longer depends on somebody remembering the list** (#1291): `uninstall.php` discovers the set with `SHOW TABLES LIKE` and drops the union of that and the declared list, so a table created but never declared cannot be left behind. Nothing was leaking: 34 declared, 34 created, zero either way.
- **The reregistration import shipped with no tables on every upgraded install** (#1311, #1214): `create_tables()` had one caller, `Activator::activate()`, and neither a WordPress update nor the rsync deploy activates anything. The post-deploy smoke reported it on twelve consecutive deploys and nobody read it. `UserDashboardActivator` had the same gap, still latent; both now heal **schema only**, because re-running the rest would resurrect a page an administrator deleted.
- **Under ALTCHA, certificate verification could never succeed** (#1305): the `/valid` handler built its payload from four named inputs, two of them the math captcha's, so the field the widget posts was never sent — the visitor watched the box turn green and was told *"Please complete the verification"*. Two siblings went with it: the widget stayed green holding a spent token after a refusal, and the public CSV form could be hijacked by the verification handler when both shortcodes shared a page.
- **PDF filenames were Portuguese for every install, translatable or not** (#1264): the three `_x()` prefixes shipped `certificado` / `recibo` / `ficha` as the **source** strings, each translating to itself, so an English site downloaded `certificado_123.pdf`. Sources are now `certificate` / `receipt` / `record`, with pt_BR translating back.
- **The shipped `.mo` was stale, so WP 6.4 never received the #1209 label fix** (#1264): `.l10n.php` carried the fix while the `.mo` still said "Estado" and "Agradecimentos". WordPress reads `.l10n.php` only from 6.5 and this plugin's floor is **6.4**, so on the floor the defect #1209 existed to fix was never delivered. The record-template option was also never deleted on uninstall — the fresh-install gate compares what *activation* writes, and an admin saves that one.
- **79 source strings had never reached the catalogue** (#1282, #1284): the password-invite screen, the invitation controls, the captcha-cap fields and the key-rotation errors all rendered in English on pt_BR. The catalogue was complete against itself and against the `.pot`, because nothing compared either with the source. The `.pot` also declared one `msgid` twice, which `msgfmt` rejects outright: every reader keys the entries, and keying makes a duplicate vanish rather than fail.
- **Two documentation pages under-reported who can see decrypted PII** (#1214): the appointments and submissions-list pages omitted their `*_pii` capability, so both read as though managing the screen were enough to unmask a CPF.
- **The import could have written past the form's own sanitizer** (#1214): `process_submission()` encrypts and persists, it does not sanitize — the per-type sanitization lived behind a `$_POST` read. It is now a method both paths call. A concurrent seed would also have aborted a batch (#1234): the question that decides promotion is *is there a row*, not *did my INSERT land*.
- **Two test idioms passed with the code under test deleted** (#1214): a Brain\Monkey `when()` in `setUp()` satisfies the call itself, so a later `expect()` counts zero; and `overload:` intercepts `new`, not a static call, so `shouldReceive()->once()` passes with zero calls. Both found the same way — by deleting the check and watching the test stay green.
- **Loading a certificate template with an empty name threw** (#1309): the success notice read `displayName || filename || ''`, and `filename` belonged to a branch the `html/` removal made unreachable in 6.23.0 — a truthy name hides it, because `||` never evaluates its right side. The modal also still named the `html/` folder the plugin no longer has.

### Security

- **Uninstall left the encrypted CPF/RF/RG in `wp_usermeta`** (#1316): step 7 deleted two meta keys by name while both of its own annotations claimed it swept by prefix, so everything the user profile writes survived an opt-in "Delete all plugin data" — ciphertext whose key material was gone with the rest. It is one prefix sweep now, with the underscore escaped. A list could not have worked: only three `ffc_*` meta keys exist as literals in the source.
- **The per-address submission limit counted nothing for an address typed with a capital** (#1314): `RateLimitRepository` hashed the value as the visitor typed it and compared it against a hash written from the canonical form, so `Joao@…` matched no prior submission of `joao@…` and the daily, weekly and monthly caps all read zero.
