...
### Fixed
- **A reregistration submission list could return null from a method declared `: array`** (#1027): `ReregistrationSubmissionReader::get_by_reregistration()` passed `get_results()` straight through, so a failed query fataled instead of yielding an empty list. Found by narrowing the PHPStan suppression that had been hiding it.
...
### Changed
- Internal (#1035) — every PHPCS suppression now says why, and 373 of them are gone. 50 `phpcs:disable` blocks and 15 duplicate `phpcs:ignore` lines suppressed nothing (measured by neutralising each and re-running), 72 named sniffs that never fire, and the 375 `DirectDatabaseQuery` annotations #1033 spread across 49 repository and activator classes collapsed into one justified file-level disable each. A guard fails CI on a bare, unexplained or self-cancelling annotation.
- Internal (#1035) — fixed four classes where an inner `phpcs:enable` silently ended an enclosing file-level `disable`: PHPCS annotations are a flat on/off switch, not a stack, so the tail of each file was uncovered while its comment claimed otherwise.
- Internal (#1034) — recorded the caching decision for the twelve uncached repository reads the direct-query audit surfaced (comments only, no code change). Tracing the callers overturned the issue's own premise: `UserService::get_user_statistics()` is not on a render path at all, and the per-campaign submission read is never called twice with the same key. One read survives as worth caching; the other eleven now say why they are not.
- Internal (#1030) — the last 22 vacuous tests, closing the register (tests only, no product change). Writing real assertions corrected two wrong beliefs about `Csv`: `close()` deliberately does not release a handle it does not own, and `all()` returns positional rows. One placeholder that exercised nothing was deleted; one is kept with a written reason. Vacuous baseline 23 → 1.
- Internal (#1030) — 12 audience tests now assert what they claim (tests only, no product change). Three of them had passed for the wrong reason — they queued no action, so the handler would have done nothing with or without the guard — and one never reached the permission check it was named after. Vacuous baseline 35 → 23.
- Internal (#1030) — 16 self-scheduling tests now assert what their names claim (tests only, no product change): the enqueue guards assert nothing was enqueued, the CPT sync/cleanup guards assert the calendar repository was never consulted, and the three "registers X" tests assert the registration actually happened. Vacuous baseline 51 → 35.
- Internal (#1030) — another 19 guard tests in the Settings and admin handlers now assert the guarded effect instead of `assertTrue( true )` (tests only, no product change). Two had relied on a stub that throws, so the guarantee lived in the stub rather than the test. Vacuous baseline 70 → 51.
- Internal (#1030) — 25 tests that asserted nothing now assert something (tests only, no product change). They were guard tests: they drove a nonce or capability check and ended in `assertTrue( true )`, so deleting the guard left them green. Each now asserts the guarded effect did not happen, verified by deleting the check and confirming the test fails. Vacuous baseline 95 → 70.
- Internal (#1028) — enabled `WordPress.DB.DirectDatabaseQuery`, the second sniff the audit found had never run. Its 129 sites were classified before being annotated: 25 are transaction control touching no table, 47 writes, 3 schema changes, and of the 54 reads most cannot be cached for a reason (a migration must read live state, a token lookup must not be cached). The dozen where caching is a real question say so at the site.
- Internal (#1028) — enabled `WordPress.Security.ValidatedSanitizedInput`, which is not part of the `WordPress-Extra` standard the ruleset uses and so had never run, while dozens of annotations were written as if it did. All 50 sites it reports were read individually — no unsanitised input was found; each is sanitised in the next statement and now says which call does it. An unsanitised superglobal read now fails CI.
- Internal (#1028) — removed 333 `phpcs:ignore` annotations that suppressed nothing, established by neutralising each one and re-running PHPCS. No code changed (the diff is token-identical). The audit also found the reason most of them were inert: two sniffs they name, `WordPress.DB.DirectDatabaseQuery` and `WordPress.Security.ValidatedSanitizedInput`, are not in the `WordPress-Extra` standard the ruleset uses, so they never ran.
- Internal (#1027) — PHPStan suppressions now say what they silence and why. All 31 used `@phpstan-ignore-next-line <id>`, which ignores *every* error on the line (the identifier is only a comment); they now use the filtering `@phpstan-ignore <id>` form, the 17 placeholder `Description.` docblocks carry real reasons, and 3 were dropped by typing the export WHERE-builder's return `literal-string`. A guard fails CI on a broad, unnamed or unexplained suppression.
