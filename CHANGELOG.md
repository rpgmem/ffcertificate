...
### Added
- **IP Diagnostics settings tab + trusted-proxy config + Cloudflare auto-detection** (#899, #901, phase 2 of 3): a read-only *IP Diagnostics* tab (Settings) shows the current request's `REMOTE_ADDR` vs. masked proxy headers, the environment verdict, the IP each strategy would resolve, and an automated header-injection self-test; a config surface persists the effective strategy (`legacy` default — no behaviour change — or opt-in `secure`) and the trusted-proxy mode (`auto`/`cloudflare`/`custom`/`direct`), which a `Settings` bridge feeds into the phase-1 resolver filters. Cloudflare's published CIDR list is refreshed daily via a new cron (`CloudflareCidrRefresh`, bundled fallback preserved), and a setup guide appears when no CDN is detected. No version bump; no new cross-module edge.
...
### Changed
- Internal (#910, #809 Fase 3) — **`UserDashboardActivator` now has direct unit coverage**: a dedicated `UserDashboardActivatorTest` exercises the role + admin-cap grant, both `dbDelta` tables (create and already-present guards) and the dashboard-page create/reuse/insert-error paths. Carries `@covers` + a `class_exists()` preload so pcov attributes the class, which was previously exercised only indirectly via `ActivatorTest` (the #563 autoload-attribution gotcha). Tests only; no runtime change.
- Internal (#910, #809 Fase 3) — **coverage tail: mis-attributed classes + api/self-scheduling error-branches**: dedicated tests fix the pcov mis-attribution of `AbstractDismissibleNotice` (0→full; each subclass's `@covers` had scoped the shared base out) and cover `EmailTemplateDefaults`; `CertificatesCalendarRestController` (~50→~100%) and `AppointmentHandler` (~55→~95%) gain their uncovered error-branches — appointment cancellation auth/policy failures, the email-notification fan-out, and the CPF/RF user-linking `WP_Error`/exception paths. Tests only; no runtime change.
- Internal (#911, #809 Fase 3) — **integration layer expansion**: two new wiring smokes boot the real plugin graph over the stubbed WordPress boundary — `PluginActivationSmokeTest` (the `Activator::activate()` entry point + module-activator dbDelta fan-out, the boot-time `maybe_migrate_*`/foreign-key upgrade path, and the FK version-guard) and `ModuleWiringSmokeTest` (the admin-ajax boot context + the newer cross-cutting modules' registration: CSV export dispatcher #772, client-IP resolver bridge + Cloudflare CIDR-refresh cron #899, certificate-template-pool CPT #865). Tests only; no runtime change.
- Internal (#910, #809 Fase 3) — **api/self-scheduling coverage-tail sweep**: raised every remaining `api/` REST controller and `self-scheduling/` handler that sat under 90% line coverage to ≥90% (or its pcov guard/multi-line-artifact ceiling) — the uncovered auth/validation/rate-limit/exception branches of `RestController`, `Calendar`/`Appointment`/`UserProfile`/`UserCertificates`/`UserSummary`/`OperatorCertificates`/`Submission`RestController, plus `AppointmentValidator`, `AppointmentsListTable`, `AppointmentAjaxHandler`, `SaveHandler`, `Loader`, `Activator` and `Admin`; also fixed a `$_SERVER['REMOTE_ADDR']` test leak the coverage-shard re-pack surfaced. Tests only; no runtime change.
- Internal (#809 Fase 3) — **JS coverage tail**: cover `ffc-email-model.js` (wp-color-picker init/restore + the logo `wp.media` uploader flow, 82→100%) and close two documented jsdom-unreachable residuals — `ffc-csv-download-flow.js` cleanup-timer path (88.6→100%; the false-positive fake-timer test now flushes the promise chain via `advanceTimersByTimeAsync`) and `ffc-csv-cert-preview.js` `fitPreview` scaling math (90.1→100%; stubbed `clientWidth`/`Height`). Tests only; no runtime change.
