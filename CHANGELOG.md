...
### Changed
- Internal (#809) — the CI coverage sharder (`.github/scripts/shard-tests.php`) now weights each test file by its test-method count and packs the four shards with a greedy longest-processing-time (LPT) bin-packer instead of a flat round-robin, so process-isolated forks (1 process/method) spread evenly. Drops the isolated-method spread across shards ~31% → ~0% (total ~14% → ~0.1%), trimming the `Coverage (PHP 8.3)` straggler-shard wall-clock. Partition stays fully deterministic.
...
- Internal (#809 Fase 1) — covered the two heaviest recruitment renderers: `RecruitmentPublicShortcodeRenderer` (~46% → ~99%; the `render_section` pagination + full `render_row` column matrix + `render_filters_bar`) and `RecruitmentAdminPage` (~39% → ~99%; the `render_page` tab/edit router, 3-state cap gates and `register_menu` wiring). Tests only, no product change.
- Internal (#807 follow-up) — raised unit coverage on three under-tested logic classes (tests only, no product change): recruitment `CsvParser` now directly tests `parse()`/`normalise_id()` (~14% → ~96%), `ScheduleExceptionGuard` gains a dedicated test for the live-token contract (~13% → ~87%), and the per-email rate limiter covers its week/month/allowed branches (~40% → ~98%).
- Internal (#807) — extracted inline HTML from four markup-heavy renderers into `templates/` partials (outside the coverage scope), keeping all logic in the class: `RecruitmentPublicShortcodeRenderer`, `ReregistrationFormRenderer`, `AudienceAdminSettings`, `RecruitmentCandidateEditPage`. Behaviour-preserving; adds a first render test for the previously-untested candidate-edit page and tidies two stale Activity-Log references left by #804.
...
- Internal (#803) — Settings now uses a **per-tab capability model** (#802 Phase A): each tab declares its own view/manage cap (defaulting to `ffc_view_settings` / `ffc_manage_settings`), the page renders only the tabs a user can view, and the read-only lock keys on the active tab's manage cap. The menu registers under a computed `ffc_view_settings_page` meta-cap (`manage_options` ∪ any page-entry cap) so it appears iff ≥1 tab is visible. Behavior-preserving; foundation for embedding the Activity Log as a tab.
...
- The audience/reregistration custom-field type `<select>` now shows **translated** labels instead of a raw `ucfirst()` of the slug (e.g. "Dependent_select" → "Dependent Select"), ordered A→Z with the display-only `acknowledgment` type last. Slugs are unchanged — display-only (#797).
- The **certificate form-editor** field-type `<select>` now shares one canonical translated label map across the server-rendered row select and the JS field builder, alphabetized with Info/Embed/Hidden pinned last, and unifies the two entry paths so **Checkbox** and **Hidden Field** are offered in both (both already valid server-side). Slugs unchanged — display-only, no migration (#798, #797 follow-up).
...
- Internal (#764 / #757) — precursor to #772: introduced the injectable `CsvStreamer` pipeline (now the sync-delivery adapter) and paged the appointment export to bound memory.
- Internal (#739 follow-up) — extracted `AbstractRecruitmentListTable`, a shared fetch→search→sort→paginate→bulk-delete base for the Notices/Adjutancies/Reasons tables (~500 duplicated lines removed), making the bulk-delete cap a required hook. Behaviour-preserving.
- Internal (#739 follow-up) — `RecruitmentPiiAccessPolicy` is now a thin adapter over the shared `Core\PiiAccessPolicy` 3-tier engine — one tiering engine instead of two copies. Behaviour-preserving.
- User Access → "Blocked Roles" is now grouped (FFC end users / WordPress core / third-party), with the FFC admin ladder behind a caveat disclosure that auto-opens when one is blocked (#739 follow-up).
- FFC role display labels normalized to `FFC <Module> - <Tier>` (e.g. "Recruitment Manager" → "FFC Recruitment - Manager"); pt_BR re-keyed. Slugs and capabilities are unchanged, so grants and integrations are unaffected (#739 follow-up).
...
- **Certificates module toggle now also gates its daily cron** (#800 follow-up): the expired-ticket cleanup (`ffc_daily_expired_tickets_cleanup`, a `ffc_form` sweep) ran unconditionally from `AdminLoader` even with Certificates disabled. Its callback is now wired only when the module is on, matching the reregistration/self-scheduling gating. The Modules tab notes that unredeemed ticket codes of ended forms aren't purged while the module is off.
- **Plugin deactivation left an orphan reregistration cron** (#800 follow-up): `Deactivator::deactivate()` cleared five hooks but not `ffcertificate_reregistration_expire_hook` (only `uninstall.php` did), so a stale daily event survived deactivation. Now cleared on deactivate and in `uninstall_cleanup()` (which also unschedules the expired-tickets cron for parity).
