...
- **Índices duplicados em `ffc_reregistration_submissions`** (#1087): activator e migrations indexavam `auth_code` e `magic_token` sob nomes diferentes, então instalações que passaram pelos dois caminhos carregam dois índices em cada coluna — custo de escrita e espaço por nada. A ativação passa a derrubar os legados, sempre condicionada à existência do índice canônico, para nunca deixar a coluna sem índice.
- **Uma instalação nova de `ffc_custom_fields` nascia com 12 das 17 colunas** (#1087): o activator do painel do usuário não declarava `field_group`, `field_source`, `field_profile_key`, `field_mask` e `is_sensitive` — as cinco que todo INSERT de campo personalizado grava. Saía certo só porque uma migration de execução única rodava depois, na mesma ativação. Terceira ocorrência da mesma classe (#1091, #1093), e a primeira entre declarações em arquivos diferentes.
- **Os mesmos índices tinham nomes diferentes em `ffc_reregistration_submissions`** (#1087): activator e migrations indexavam `auth_code` e `magic_token` com nomes distintos, então uma instalação que passou pelos dois caminhos carrega dois índices em cada coluna. As declarações passam a concordar, o que impede novas instalações de herdar o par; remover os duplicados existentes é mudança destrutiva e fica à parte.
- **Dez colunas eram declaradas `json`, tipo que o MariaDB não armazena** (#1087): ele implementa `JSON` como `LONGTEXT` mais um CHECK, então a declaração nunca podia bater e o `dbDelta` queria reescrevê-la a cada execução que a alcançasse. Passam a declarar `longtext` — nenhuma consulta do plugin usa função JSON nativa, toda leitura é `json_decode` em PHP. O gate de idempotência volta a bloquear em zero, sem baseline.
- **Declarações de tabela que discordavam do que o banco guarda** (#1087): o gate de idempotência mediu 14 `CREATE TABLE` que o `dbDelta` queria alterar contra a tabela recém-criada; 7 foram corrigidos. `ffc_reregistrations` declarava `audience_id`, coluna que o activator derruba logo depois; `auth_code` era declarado como índice comum nas duas tabelas em que o código o converte para `UNIQUE`; e 20 colunas inteiras omitiam o display width que o MariaDB grava. Os 7 restantes são colunas `json`, que o MariaDB não armazena como `json` — decisão em aberto.
...
- ⚠ **BREAKING — o fallback de layout `html/` foi removido** (#1087, #865): o seletor de layouts do editor de formulários passa a ser servido **exclusivamente** pelo pool de modelos. Saem o glob `html/*.html`, a carga por nome de arquivo e os três `html/default_certificate_*.html`. Quem ainda dependia de um arquivo solto em `html/` precisa importá-lo pelo pool (Configurações → Migrações → `import_legacy_templates`) antes de atualizar. Os `html/*.png` **continuam**, porque a migração de reescrita de imagens os lê do disco.
...
- **O save do editor de formulários lia `$_POST` sem tipo em cinco métodos** (#1084): `FormEditorSaveHandler` era o maior ofensor de nível 9 do repositório (120 erros), com a mesma assinatura que o save-handler do self-scheduling tinha antes do #1075. Um valor não escalar chegava direto ao sanitizador — a mensagem do limite de dispositivo, por exemplo, era gravada como a string `Array` mais um aviso do PHP —, e a lista de ids de localidade passava por `array_map( 'sanitize_key', … )`, que recebe o que a requisição aninhar ali. As leituras passam pelo `RequestInput` + `ArrayValue`; o arquivo foi a **zero** e o total do repositório caiu de 2705 para 2585.
...
- **O widget ALTCHA saía com quinas retas e fonte do tema, não a do formulário** (#1053): as quinas curvas eram pedidas com `var(--ffc-radius)`, um token que só existe nas folhas de recrutamento e do painel do usuário — que não carregam nessas páginas. Uma variável indefinida invalida a declaração inteira em silêncio, então o widget caía no próprio `0`. A tipografia era herdada do tema (tudo dentro do widget é `inherit`) enquanto os campos do formulário têm tamanho próprio, o que num tema de corpo 20px exibia o captcha numa fonte diferente e maior que a pergunta ao lado. Agora casa com a linha do captcha matemático. `CssTokenReferenceTest` passa a falhar quando um `var(--ffc-*)` sem fallback não resolve.

- **No modo “ALTCHA + matemático”, um visitante sem JavaScript via um aviso dizendo que o formulário não funcionaria** (#1053): o aviso pertence ao modo só-ALTCHA, onde é verdade, e era renderizado também no composto — em vermelho, logo acima do campo matemático que o visitante deveria responder. Junto com ele saiu o painel vazio do widget, que sem JavaScript é uma caixa azul sem conteúdo. O aviso continua no modo só-ALTCHA, onde não há alternativa.
