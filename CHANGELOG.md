...
### Changed
- Internal (#772) — closing docs for the CSV-export consolidation: added the **CSV export architecture** section to `CLAUDE.md` §3 (one source contract — `SyncSourceInterface` / `BatchedExportSourceInterface` — with two delivery adapters `CsvStreamer` / `BatchedCsvExport`, the `SourceRegistry` + `BatchedExportDispatcher`, keyset/frozen-header rules, the shared `ffc-batched-export.js` driver, the pluggable ownership models, and the temp-dir PII-on-disk posture). Documented the **nginx hardening caveat** for the `wp-upload/ffc-tmp` temp dir (the `.htaccess` `Deny from all` is Apache-only) both in `CLAUDE.md` and as a pointer comment in `BatchedCsvExport::ensure_tmp_dir()`, and reframed `CsvStreamer`'s docblock as the contract's synchronous-delivery adapter. Docs/comments only — no behaviour change.
- Internal (#772) — migrated the **Reregistration submissions** CSV export from the synchronous `ReregistrationCsvExporter` (a page-action handler streaming via `CsvStreamer`) to the batched engine, completing the batched-export consolidation. New `Reregistration\ReregistrationExportSource` (`type=reregistration`) + `ReregistrationSubmissionReader::find_by_cursor_for_export()` (keyset `WHERE s.reregistration_id = ? AND s.id < cursor ORDER BY s.id DESC`, joining the user display-name/email like the list query). The export is per-campaign; its dynamic columns are that campaign's custom fields — deterministic from the linked audiences, so they are resolved once in `build_context()` and reused for the header and every row (sensitive values decrypted per row). The source registers with the shared registry from `ReregistrationLoader::init()` (under `is_admin()`, true on admin-ajax), and the "Export CSV" button now drives the unified `ffc_export_*` dispatcher through the shared `ffc-batched-export.js` driver instead of a nonce-signed page-action link. The old `ReregistrationCsvExporter` + its `action=export_csv` page handler are retired. Gated by the same `ffc_export_reregistration` cap; the export still covers the whole campaign (unaffected by the on-screen status/search filter). **Minor behaviour change:** the export now streams in id-DESC keyset order (stable across concurrent writes) rather than `created_at ASC`. (#772)
- Internal (#772) — migrated the **Appointments** CSV export (`ffc-appointments`) from the synchronous `AppointmentCsvExporter` (`admin_post` + `CsvStreamer`) to the batched engine. New `SelfScheduling\AppointmentExportSource` (`type=appointments`) + `AppointmentReader::countForExport()` / keyset `getExportBatch()` / `getExportKeysBatch()` (exposed via the `AppointmentRepository` façade). Like the certificate-submissions source it carries per-row `custom_data` JSON, so the header is the fixed 27 columns plus the union of every matching row's dynamic keys — discovered up front by a lightweight keyset scan of just the JSON columns; PII (email / phone / user IP) is decrypted per row in memory. The source registers with the shared registry from `SelfSchedulingLoader::init()` (under `is_admin()`, true on admin-ajax), and the "Export CSV" button now drives the unified `ffc_export_*` dispatcher through the shared `ffc-batched-export.js` driver instead of an `admin_post` form. The old `AppointmentCsvExporter` + its `admin_post_ffc_export_appointments_csv` endpoint are retired. Gated by the same `ffc_export_appointments` cap; calendar / status / date-window filters are preserved. **Minor behaviour change:** the export now streams in id-DESC keyset order (stable across concurrent writes) rather than `appointment_date DESC, start_time DESC`. (#772)
- Internal (#772) — migrated the **Audience Bookings** CSV export (`ffc-scheduling-bookings`) from the synchronous `AudienceBookingCsvExporter` (`admin_post` + `CsvStreamer`) to the batched engine. New `Audience\AudienceBookingExportSource` (`type=audience_bookings`) + reader `count_for_export()` / keyset `find_by_cursor()` — both JOIN the environment table (like `get_all`) so the `schedule_id` filter and the progress total stay accurate (the shared `count()` can't, as it queries the bookings table alone); export order is `id`-DESC keyset. The source registers with the shared registry from `AudienceLoader::init_admin()` (under `is_admin()`, true on admin-ajax), and the "Export CSV" button now drives the unified `ffc_export_*` dispatcher through the shared `ffc-batched-export.js` driver instead of an `admin_post` form. The old `AudienceBookingCsvExporter` + its `admin_post_ffc_export_audience_bookings_csv` endpoint are retired. Gated by the same `ffc_export_audiences` cap; schedule / environment / status / date-window filters are preserved. **Minor behaviour change:** the export now streams in id-DESC keyset order (stable across concurrent writes) rather than `booking_date DESC`. (#772)
- Internal (#772) — migrated the **Activity Log** CSV export (`ffc-activity-log`) from the synchronous `handle_csv_export` (which materialised every matching row with `limit = 999999`) to the batched engine. New `Admin\ActivityLogExportSource` (`type=activity_log`) + `Core\ActivityLogQuery::find_by_cursor()` (keyset `WHERE id < cursor ORDER BY id DESC`, `context` decrypted per row, mirroring `get_activities`); the source registers with the shared registry from the `AdminActivityLogPage` constructor (so it is reachable on admin-ajax, where `admin_menu` never fires), and the "Export CSV" button now drives the unified `ffc_export_*` dispatcher through the shared `ffc-batched-export.js` driver instead of a synchronous nonce link. The `admin_init` `handle_csv_export` handler + its `ffc_export_logs` entry point are retired. Gated by the same `ffc_export_activity_log` cap; `level` / `action` / `search` filters are preserved. **Minor behaviour change:** the export now streams in id-DESC keyset order (stable across concurrent writes) rather than `created_at DESC` — for this sequentially-written table the two coincide. (#772)
- Internal (#772) — migrated the **URL Shortener** list export (`ffc-short-urls`) from the synchronous `CsvStreamer` download to the batched engine, as the first `BatchedExportSourceInterface` beyond submissions. New `UrlShortenerExportSource` + repository `countForExport()` / keyset `findByCursor()` (`WHERE id < cursor ORDER BY id DESC`, sharing a `build_export_where()` helper with `findPaginated()`); the source registers with the shared registry (`type=url_shortener`) and the "Export CSV" button now drives the unified `ffc_export_*` dispatcher through the shared `ffc-batched-export.js` driver instead of a synchronous `admin_post` form. The old `UrlShortenerCsvExporter` + its `admin_post_ffc_export_short_urls_csv` endpoint are retired. Gated by the same `ffc_export_url_shortener` cap; `search` + `status` filters are preserved. **Minor behaviour change:** the export now streams in id-DESC keyset order (stable across concurrent writes) rather than the on-screen `orderby`/`order` — in practice id-DESC matches the default `created_at DESC`.
- Internal (#772) — introduced `Core\BatchedCsvExport`, a shared timeout-safe CSV export engine (start → batch → download over a temp file + transient job), and migrated the certificate-submissions exporter onto it as the first source. All the domain specifics (columns, PII-decrypting row formatting, dynamic-key scan, keyset cursor query, per-phase authorization) move behind a new `Core\BatchedExportSourceInterface` implemented by `Admin\SubmissionsExportSource`; `Admin\CsvExporter` shrinks to a thin façade that wires the source to the engine and keeps the stale-job cleanup cron. Behaviour-preserving — same AJAX endpoints, request/response envelopes, temp-file scheme and hooks, so the front-end is untouched. This is the first step of the batched-export consolidation that de-duplicates the near-identical admin + public batched exporters.
- Internal (#772) — migrated the **public** batched CSV export onto the shared `Core\BatchedCsvExport` engine, the second source (after admin submissions). Its front-end specifics — the layered start-time authorization (IP rate-limit + page nonce + honeypot/CAPTCHA + form-access hash + per-form CPF gate), the download-quota bump, the job-scoped-nonce + IP-hash ownership fence on every batch/download, the shared column layout, the keyset cursor query, and the pre-stream "delivered" audit row — move behind a new `Frontend\PublicFormsExportSource`, using the interface's `on_before_download` seam for that audit row. `Frontend\PublicCsvExporter` shrinks to a façade that wires the source to the engine (keeping the synchronous no-JS `stream_form_csv` fallback) and its constants become aliases of the engine's. Behaviour-preserving — same `ffc_public_csv_*` AJAX endpoints, request/response envelopes (incl. the per-job `nonce_batch`), temp-file scheme and hooks, so the front-end JS is untouched.
- Internal (#772) — unified the two batched-export endpoint trios behind a single `type`-routed dispatcher. `Core\SourceRegistry` maps a source `type` to a lazy factory; `Core\BatchedExportDispatcher` registers one AJAX trio (`ffc_export_start` / `ffc_export_batch` / `ffc_export_download`, priv + nopriv) and routes each request to the registered source, which owns its own authorization. The admin submissions and public forms sources now register with the registry instead of adding their own `ffc_csv_export_*` / `ffc_public_csv_*` endpoints (those are retired); one shared JS driver (`assets/js/ffc-batched-export.js`) replaces the duplicated start→batch→download loops in `ffc-admin.js` and `ffc-csv-download-flow.js`. Behaviour-preserving for end users — same request/response envelopes, per-job nonce, temp-file scheme and download flow; only the internal action names changed (and the stale-job cleanup cron now also sweeps the unified `ffc_export_` transient namespace). This closes the batched-export de-duplication.
- **⚠ Breaking (#772) — CSV export hook rename.** The public filter/action hooks `ffcertificate_csv_export_headers`, `ffcertificate_csv_export_filename`, `ffcertificate_csv_export_data` and `ffcertificate_csv_export_completed` were renamed to `ffc_export_headers`, `ffc_export_filename`, `ffc_export_data` and `ffc_export_completed` respectively, for consistency with the unified export namespace. Their signatures are unchanged. **External integrations hooking the old names must update to the new ones** — the old names no longer fire (no back-compat shim, by design).
- Internal (#772) — introduced the **synchronous** export contract, the bounded-output counterpart to the batched engine: `Core\SyncSourceInterface` (`authorize` / `filename` / `header` / `rows`) + `Core\SyncCsvExport`, which authorizes a source then streams its header + rows through the shared `Core\CsvStreamer`. Migrated the four bounded exporters onto it, each keeping its existing `admin_post` / page-load entry point: the recruitment example template (`RecruitmentExampleCsvSource`), the audience members + audiences exports and the audience import-sample (`AudienceMembersExportSource` / `AudienceAudiencesExportSource` / `AudienceSampleCsvSource` — the sample now shares one source of truth with the on-screen `<pre>` via `AudienceCsvImporter::get_sample_rows()`), and the per-form public-download audit log (`Frontend\Csv\CsvDownloadLogExportSource`, the last hand-rolled `php://output` holdout — CPF still decrypted in memory, never to disk). Behaviour-preserving except one documented edge: when encryption is not configured, the audit log's advisory note now streams as the first data row (the header is always written first) instead of before the header.
- Internal (#764) — migrated the **activity-log** CSV export (`AdminActivityLogPage::handle_csv_export`) to the injectable `Core\CsvStreamer`, the last of the in-scope `php://output` streaming exporters. The materialised rows are handed straight to the streamer, replacing the hand-rolled `header()`/`fopen('php://output')`/`exit`; the existing streaming test moves off the `Csv::writer` alias-mock + throw-on-`close()` trick to assert the captured bytes through a buffered download double. Behaviour-preserving. This closes the CSV consolidation: all seven in-scope exporters (url-shortener, audience-bookings, recruitment sample, audience import/export ×2, reregistration, appointment, activity-log) now flow through the single `CsvDownloadInterface` → `HttpCsvDownload` → `CsvStreamer` pipeline, and their formerly-untestable streaming paths (~269 lines) are unit-tested. The two public front-end exporters (`PublicCsvDownload`, `PublicCsvExporter`) and the file-based batch exporter (`Admin\CsvExporter`, which writes a temp file and `readfile()`s it) stay on their own mechanisms by design.
- Internal (#764) — migrated the **appointment** CSV export (`AppointmentCsvExporter::export_csv`) to the injectable `Core\CsvStreamer`, completing the memory-safe two-pass exporters. The empty-check peek, dynamic-key collection pass and filename building stay in `export_csv`; the second streaming pass (which reuses the already-fetched first page) moves into a `export_rows` generator handed to the streamer, replacing the hand-rolled `header()`/`fopen('php://output')`/`exit`. The formerly-untested streaming path now has a full unit test (fixed header + a row + a dynamic custom_data column) via a buffered download double. Behaviour-preserving — same columns, dynamic-column union, paging and filename.
- Internal (#764) — migrated the **reregistration submissions** CSV export (`ReregistrationCsvExporter::handle_export`) to the injectable `Core\CsvStreamer`: the fixed + dynamic-field header is built up front and the per-submission rows (decrypt sensitive fields, format the instant timestamps, stringify dynamic values) move into a `export_rows` generator handed to the streamer, replacing the hand-rolled `header()`/`fopen('php://output')`/`exit`. The formerly-untested streaming path now has a full happy-path unit test (header + a submission row with a dynamic field) via a buffered download double. Behaviour-preserving — same columns, order, chunked streaming and filename.
- Internal (#764) — migrated the **Audience Import & Export** page's two CSV exports (members + audiences) to the injectable `Core\CsvStreamer`: each export now hands its fixed header + a row generator (`members_rows` / `audiences_rows`) to the streamer instead of hand-rolling `header()`/`fopen('php://output')`/`exit`. Both formerly-untested export paths are now unit-tested via a buffered download double (member dedup + audience-name join; hierarchical parents-then-children with default colour). Behaviour-preserving — same columns, order and filenames.
- Internal (#764) — migrated the **recruitment CSV-example** template download (`RecruitmentNoticeEditPage::handle_download_csv_example`) to the injectable `Core\CsvStreamer`, replacing its hand-rolled `header()`/`fopen('php://output')`/`exit` block with a header row + example rows handed to the streamer. The formerly-untouched download path is now unit-tested (cap-denied guard + captured header line and both example rows via a buffered download double). Behaviour-preserving — same filename, same example content, same `;` delimiter.
...
### Added
- Internal (#764) — an injectable output boundary for CSV downloads, so the streaming path becomes unit-testable for the first time. `Core\CsvDownloadInterface` (`send_headers` / `open_stream` / `finish`) fronts the otherwise-untestable I/O; `Core\HttpCsvDownload` is the single adapter that touches `header()` / `fopen('php://output')` / `exit`; `Core\CsvStreamer` is pure orchestration (headers → header row → each row → finish) taking an `iterable` of rows so a caller can pass a paging generator (memory bounded by one page). Pilot migration: `UrlShortenerCsvExporter` now injects a `CsvStreamer` (defaulting to the live HTTP download) and its export path is covered by capturing the bytes through a buffered download double — the seam the reverted `CsvDownloadTrait` lacked. Behaviour-preserving; groundwork for consolidating the remaining `php://output` exporters one at a time.
...
### Changed
- Internal (#739 follow-up) — extracted `AbstractRecruitmentListTable`, a shared base for the three recruitment catalog tables (Notices / Adjutancies / Reasons) that had duplicated the same fetch-all → search → sort → paginate → bulk-delete mechanics line-for-line (~500 lines of copy). The base owns those mechanics plus the destructive row-action link; each subclass keeps only its columns, row shape and domain hooks. Same-module refactor, behaviour-preserving. It also makes the bulk-delete capability a **required** hook, so no recruitment table can ship a bulk delete without a cap gate (the candidate list table stays separate — it paginates SQL-side and doesn't fit the fetch-all template).
...
- FFC role **display labels** normalized to one grammar (#739 follow-up): `FFC <Module> - <Tier>` for every module role — e.g. "FFC Certificates Admin" → "FFC Certificates - Administrator", the un-prefixed "Recruitment Manager" → "FFC Recruitment - Manager" — with the admin tier spelled out consistently (`Admin` → `Administrator`). pt_BR translations were re-keyed to the new source strings (most already matched the target grammar). Only the display labels change — role **slugs** and capabilities are untouched, so existing grants and any integration keying on a slug are unaffected.
- Both admin surfaces that list FFC roles — the "Blocked Roles" checkboxes and the role → capability editor dropdown (Settings → User Access) — now present the roles in one canonical order (#739 follow-up): the three cross-cutting roles (Administrator, Read-Only, End User) first, then each module ladder alphabetically by module name, each from least to most powerful (viewer → operator → manager → administrator). The order derives from the translated label, so it reads alphabetically in whatever language the admin is viewing.

### Fixed
- **Audience Bookings** (`ffc-scheduling-bookings`) "Export CSV" button did nothing — no request, no console error (#772 regression). The batched-export click handler had been added at the tail of `ffc-audience-admin.js`'s `initCalendarPermissions()`, which early-returns when `#ffc-permissions-table` is absent — as it is on the bookings list — so the handler was never registered there (the driver, nonce and button were all present, which is why nothing errored). Moved it to its own `initBookingsExport()` method called unconditionally from `bindEvents()`. A regression test now clears leaked `document` handlers before asserting registration, so this can't pass on a leaked delegate again.
- Public operator **CSV download** failed with "Security check failed. Refresh the page and try again." at the download step (#772 regression). When the public export was migrated onto the shared `ffc-batched-export.js` driver, the driver merged the routing `type` into the caller's `startData` with `$.extend({ type }, startData)`. The public caller passes `startData` as a **serialized form string** (`$form.serialize()`), and `$.extend` iterates a string's character indices — so the string was spread into `{0:'…',1:'…'}` keys and every real field (including the `_ffc_pcd_nonce` page nonce) was dropped, tripping the start-phase nonce check. The driver now appends `&type=` to a string `startData` (and object-merges otherwise), mirroring how `FFC.request` already accepts both shapes; the admin export (which passes an object) is unaffected. The info screen was never affected because it posts the string straight through `FFC.request`.
- Appointments (`ffc-appointments`) **Export CSV** button did nothing when clicked (#758): it was a dead `<a href="#">` with no form or JS behind it (the screen's script was removed in an earlier cleanup), while the working `AppointmentCsvExporter` (`admin_post_ffc_export_appointments_csv`) was never wired to it. The button is now a real `POST` to `admin-post.php` with the export action + nonce, shown only to holders of `ffc_export_appointments`, and carries the current calendar/status filters so it exports exactly what is on screen.
- Self-scheduling booking returned **403 Forbidden** on submit (#755): `submitBooking()` sent the serialized form (which carries its own `ffc_self_scheduling_nonce` field) with no explicit nonce, and `FFC.request` appends `&nonce=<fallback>` to a string payload — the fallback being the *frontend* nonce (or empty). As the last `nonce=` param it overrode the correct one, so `check_ajax_referer()` rejected every booking. The submit now passes the live `ffcCalendar.nonce` (refreshed by `ffc-dynamic-fragments` on cached pages) and strips the form-baked `nonce`/`action` from the payload, matching the slot/month read calls. Loading the calendar and time slots was never affected (those already passed the nonce explicitly).
- FFC role labels showed untranslated (English) in the admin — users list "Role" column, the role dropdown and the "Blocked Roles" list — even with a complete translation (#739 follow-up). WordPress freezes a role's label (English source) in `wp_user_roles` at registration; `RoleRegistrar::relabel_ffc_roles()` re-translates it, but it was only hooked on `wp_roles_init`, which usually fires during early auth (`plugins_loaded`, when `current_user_can()` first builds `WP_Roles`) — before that hook is added at `init:1` — so it never ran. The relabel is now also applied directly at `init:1`, where the plugin textdomain resolves via just-in-time loading, so the translated labels reach every screen. The "Blocked Roles" list additionally renders each role name through `translate_user_role()` (so core WordPress roles translate too).
