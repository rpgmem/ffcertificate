...
- **Geofence country-block pinned to the unspoofable IP resolver** (#931): the geofence deny-decision (and its audit log) always resolve the client IP via the trusted-proxy `secure` strategy, regardless of the global legacy/secure toggle — the one deliberate exception to the unified toggle, because a geofence is the only IP-based hard-deny with no secondary defence layer, so a forged `CF-Connecting-IP` can never bypass a country block. Behind a non-Cloudflare reverse proxy, declare it as trusted on the IP Diagnostics tab so the real client (not the proxy) is geolocated.
