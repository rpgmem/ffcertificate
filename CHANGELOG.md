...
### Fixed
- **Reregistration settings-tab icon back to monochrome** (#951): the new **Recadastramento** tab used the `ffc-icon-id` badge (🆔), which renders as a solid colour and broke the flat/mono icon row in the Settings nav; switched to the monochrome `ffc-icon-user` glyph. CSS-class-only change; the `ffc-icon-id` badge stays where an ID connotation fits (CPF rate-limit heading, CPF-split migration card).
