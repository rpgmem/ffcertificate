...
- **Varredura de emissão de classe CSS** (#1170): renomear uma classe só é seguro quando se acha quem a emite, e um grep não acha — o nome costuma ser montado em runtime (`'ffc-dashboard-status-' + status`). A varredura conhece sete formas, cada uma achada por ter falhado antes, e deixa 26 classes sem emissor conhecido como lista de perguntas em aberto.
