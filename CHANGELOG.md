...
### Changed
- **Email polish follow-ups** (#976): the recruitment convocation **subject** now follows the shared `Event: {{ref}}` pattern (`Recruitment Call: {{notice_name}}`) instead of the terse `Call - {{notice_code}} - {{adjutancy}}`; the pt_BR translation of the reregistration emails now uses the established term **"Recadastramento"** consistently (five strings introduced during the email standardization read "Rerregistro"); and the read-only **"All plugin emails"** directory table fills its panel width instead of being capped at 820px.
- **All email subjects standardized** (#976): the audience **booking** and **cancellation** emails, which shipped with no subject line (empty in the Email texts hub), now carry proper subjects (`New scheduled activity: {{schedule_name}}` / `Activity cancelled: {{schedule_name}}`); and five outlier subjects are normalized to the shared `Event: {{ref}}` shape (colon separator, no bracketed `[{{site_name}}]` prefix, no trailing dashes): certificate-to-user, appointment reminder, reregistration reminder, access-granted, and the calendar-deletion cancellation. The calendar-deletion subject now references `{{calendar_title}}` instead of the site name. Bodies are unchanged; this is subject-line consistency only.
- **Email texts: edit one email at a time** (#976): the Email texts hub no longer stacks all ~15 rich editors on the page at once. A feature-grouped **selector** picks the email to edit and only that one's editor is shown, with its TinyMCE **initialized on demand** (`wp.editor.initialize` / `.remove`) instead of booting fifteen on load — much lighter and less overwhelming. Every email's field is still part of the one form, so a single "Save email texts" persists them all; "Restore Default Text" and the Global/Custom clear-on-equal behaviour are unchanged. Degrades gracefully to plain textareas without the block-editor API.
- **Email settings split into three focused tabs** (#976): the overloaded **SMTP** tab is divided into a **SMTP** tab (transport only — the enable-emails switch + server config), a new **Email Model** tab (the shared chrome editor + live preview + test-send) and a new **Email texts** tab (the per-email subject/body hub + the "All plugin emails" directory). The three sit together in the Communication nav group in that deliberate order (SMTP → Email Model → Email texts) via a new explicit intra-group ordering, instead of the default alphabetical order. Save flows are unchanged (the Email Model still posts `ffc_email_template`, the hub still posts `ffc_email_bodies`); this is a relocation, and every deep-link in the directory now points at the right new tab. First step of the #976 email-settings UX rework (the per-email selector lands next).
- **In-plugin documentation refreshed for the email/template rework** (#976): the Documentation tab is brought back in line with the current plugin. Emails & Delivery now describes the three email tabs (SMTP / Email Model / Email texts) and all editable emails instead of "only three"; Forms → Email documents the per-form Global/Custom toggle; Certificates, Ficha and Personal Calendars describe pool-backed templates (and Personal Calendars gains custom calendars, waitlist/promotion, the per-user block cap and the `ffc_bypass_appointment_capacity` cap); the Developer page adds the `ClientIpResolver` filters, the `ffcertificate_ficha_template_html` filter and the shared batched-CSV-export engine. Ten legacy "Section N / #141" partial docblocks are migrated to the current style. Documentation-only.
- **One consistent visual + copy standard across all default emails** (#976): every plugin email now follows a single skeleton — an `<h2>` event title (no emoji, semantic colour: green = confirmed/approved, red = cancelled, amber = reminder, purple = waitlist, blue = informational), a `Hello {{name}},` greeting, one context line, and a semantic details box — replacing the four divergent patterns that had accumulated (emoji-heading, greeting-as-heading, plain-heading, no-heading). The two admin notifications follow the same visual pattern (they stay system-default), audience emails drop their body-level "Best regards, {{site_name}}" sign-off in favour of the shared Email Model footer, and CTAs are harmonised to the brand blue. No token changes; unconfigured installs simply render the new consistent look. ⚠️ Because the shipped defaults changed, a per-form / per-schedule / per-calendar body still holding the *old* shipped default text is now detected as **Custom** (keeps its old text) until you hit "Restore Default Text"; blank (Global-tracking) entities render the new standard automatically.

### Added
- **Two documentation pages for the hubs** (#976): the Documentation tab gains an **Email texts hub** page (the Global/Custom override model, the shared subject/body standard, the "All plugin emails" directory, the gating capability) and a **Document Templates hub** page (the certificate template pool, its document kinds, and where each feature selects its template). Both are registered in the recursive doc tree so the nav and page order stay in sync. Documentation-only.
- **Access-granted & calendar-deletion emails join the email-body hub** (#965, phase 2): the two remaining echo-partial emails — the **"access granted to a user"** notice (sent when a user is granted plugin capabilities) and the **"calendar deleted → appointment cancelled"** notice — are now editable body + subject strings in **Settings → SMTP → Email texts**. Access-granted uses `{{user_name}}` / `{{context_label}}` / `{{site_name}}` plus a pre-rendered `{{dashboard_button}}` token (empty when no dashboard URL); calendar-deletion uses `{{calendar_title}}` / `{{appointment_date}}` / `{{appointment_time}}` (the subject references `{{calendar_title}}`). The **"All plugin emails"** index (#963) is updated so every migrated row now reads **"Editable text (global)"** with an **Open →** into the hub, and a new **Account access** group surfaces the access-granted email. Completes the phase-2 tokenization; unconfigured installs render identically. The access-granted dashboard button now uses the shared `ffc_email_button()` renderer (centred, consistent with every other email CTA — a small cosmetic change from its former left-aligned inline link).
- **Appointment lifecycle emails join the email-body hub** (#965, phase 2): the five self-scheduling notification emails — **approval, cancellation, reminder, waitlist-promotion and waitlisted** — previously code-built echo partials with conditional buttons, are now single editable body strings in **Settings → SMTP → Email texts**. Each uses pre-rendered tokens: `{{receipt_button}}` / `{{cancel_button}}` / `{{waitlist_button}}` (empty when their URL is absent), `{{cancellation_reason_block}}`, and `{{status_message}}`, alongside `{{calendar_title}}` / `{{appointment_date}}` / `{{appointment_time}}`. Subjects are now templated too. These are global-only (no per-calendar override); unconfigured installs render identically. Extends the phase-2 tokenization to the whole appointment set.
- **Self-scheduling booking confirmation joins the email-body hub** (#965, phase 2): the appointment booking-confirmation email — previously a code-built echo partial (details box + conditional receipt/cancel buttons) — is now a single editable body string in **Settings → SMTP → Email texts**, using pre-rendered button tokens `{{receipt_button}}` / `{{cancel_button}}` (empty when their URL is absent) plus `{{status_message}}` / `{{status_label}}` / `{{user_notes_block}}`. The per-calendar confirmation editor keeps working and can now place those tokens too; its "Restore Default Text" restores the effective global. New reusable `EmailHelperTrait::ffc_email_button()` renders the buttons. No regression to the buttons; unconfigured installs render identically. First validation case of the phase-2 echo-partial tokenization; `#963` index row updated to "Editable text (global)".
- **Per-form certificate email — Global/Custom toggle** (#964, phase 1): the form editor's Email tab gains an **Email content: Global / Custom** switch. On **Global** (the new default) the form follows the shared hub text and the per-form subject/body fields are hidden behind a short note linking to **Settings → SMTP → Email texts**; flip to **Custom** to reveal them, pre-seeded with the current global so you edit from real text. Switching back to Global clears the per-form copy (behind a confirmation) so the form tracks the global again; "Restore Default Text" now restores to the **global** default. The editor is no longer pre-seeded with the shipped body — a blank form stays on the global. Mode is a migration-free runtime heuristic (a subject/body edited away from the shipped default = Custom), so existing forms open in the right state with no data change.
- **Email-body hub on Settings → SMTP** (#964, phase 1): a new **Email texts** section below the Email Model lets an admin edit the **global default subject + body** of the seven token-based plugin emails (certificate-to-user, recruitment convocation, the three reregistration emails, audience booking + cancellation) once, in one place — each a `wp_editor` with its own `{{token}}` help and a "Restore Default Text" button. Saves to the `ffc_email_bodies` store (`EmailTemplates::save_global()`); a subject + body left equal to the shipped default clears the override so the email keeps tracking the file default. Gated by `ffc_manage_email_templates` (hidden entirely without it). The self-scheduling confirmation is intentionally deferred to phase 2 (its real default is a rich echo-partial). Send sites are wired to `effective_body()` per feature in later increments — so this increment adds the editor UI + store; e-mails not yet re-wired still render their file default until then.
- **Dedicated `ffc_manage_email_templates` capability** (#964, phase 1): a new admin capability that will gate editing the global default body/subject of the plugin emails (the upcoming Settings → SMTP email-template hub), carved out so email-copy editing can be delegated — or withheld — independently of the blanket `ffc_manage_settings`. A one-shot, option-flagged migration (`ffc_email_templates_cap_v1`) seeds it onto every current `ffc_manage_settings` holder (users **and** roles), so no one loses access on upgrade; admins restrict by removing it from a role. Registered in `CapabilityManager` + `CapabilityCatalog` (Settings group); no gate consumes it yet (that lands with the hub).
- **Global email-body override layer — foundation** (#964, phase 1): `Core\EmailTemplates` gains a global-override store (option `ffc_email_bodies`) plus an **opt-in cascade** — `global_body()` (the stored admin override), `effective_body()` (override if set, else the shipped file default), and `save_global()` / `clear_global()` for the upcoming hub. `body()` is **unchanged** (still the file default, reads no options), so every existing caller stays byte-for-byte the same; send sites migrate from `body()` to `effective_body()` per feature increment. Dormant by default (empty option ⇒ `effective_body()` equals `body()`); no change on upgrade. Groundwork for the #964 email-body hub.
- **"All plugin emails" directory on the SMTP tab** (#951): a read-only index at the bottom of **Settings → SMTP** (below the Email Model) lists every email the plugin can send, grouped by feature, each with a one-line purpose, a **personalisation** state (Editable text / On/off only / System default) and an **Open →** deep-link to where it's configured. Discoverability only — the email *pipeline* is already centralized (#662: one chrome, one transport, default bodies in `templates/emails/`); this surfaces the otherwise-opaque email landscape without moving any control (so no capability or module coupling changes). Each feature group is gated by its own view cap; the panel hides when none apply. Answers "should email models be centralized too?" in the same spirit as the template "Current assignments" panel — an index, not a move.
- **"Current assignments" overview on the Document Templates tab** (#951): a read-only table at the top of **Settings → Document Templates** shows which pool template each feature is using right now — appointment receipt (Regular / Custom) and the reregistration ficha — each with a "Change →" link into that feature's own settings where the assignment is set. Display-only: the selection controls (and their capabilities) stay per-feature, so no control or cap moves into the hub and no new module coupling is introduced. Each row is gated by its feature's view cap; the panel is hidden when none apply.
- **Editable, pool-backed reregistration ficha template** (#951, phase 2): the reregistration **ficha** PDF now draws from the certificate **template pool** (#865) via a new `ficha` kind, so its layout is admin-editable/duplicable in the Document Templates hub instead of being a bundled file only reachable through the `ffcertificate_ficha_template_file` dev filter. A shipped default seeds automatically; a new **Reregistration** settings tab globally selects which ficha template the PDF uses (falling back to the shipped default when unset). Resolves via the new `ffcertificate_ficha_template_html` filter — no `Reregistration → Admin` coupling. No data migration; unconfigured installs render exactly as before.
- **Editable, pool-backed appointment-receipt templates (per mode)** (#945): the appointment receipt (comprovante) PDF is no longer a single hardcoded layout — it now draws from the certificate **template pool** (#865), reused via a `_ffc_template_kind` discriminator (absent = certificate, fully retrocompatible). Two receipt defaults ship (Regular + Custom), editable/duplicable like any pool template, with a Category column + filter in the Document Templates hub. A new **Receipt** tab under **Scheduling → Settings** selects, globally per scheduling mode, which template each calendar type uses (Regular vs Custom), falling back to the shipped default when unset; templates are created/edited/duplicated in the Document Templates hub (the tab links to it). Adds `{{appointment_end}}` / `{{appointment_time_range}}` receipt tokens. No data migration; unconfigured installs render exactly as before.
- **Self-scheduling per-user block cap + occupancy report** (#941, phase 3): custom calendars gain `max_blocks_per_user` — a cap on how many distinct blocks a single user may hold (booking validation rejects a user over the limit, counting their active **and** waitlisted bookings; the admin overbook bypass skips it; `0` = no limit). Custom calendars also get a read-only **Occupancy Report** meta box pairing each block with its live booked/capacity, occupancy % and waitlist length. Off/empty by default; existing calendars unchanged.
- **Self-scheduling waitlist with automatic FIFO promotion** (#941, phase 2): a per-calendar waitlist (both scheduling modes) — when a slot/block is full, users can join a queue (`waitlist_enabled`, optional `waitlist_capacity`) instead of being turned away. Queued bookings carry a new `waitlist` status excluded from every capacity count. When a spot frees up (a cancellation or an admin rejection), the oldest queued booking is promoted automatically and transactionally (`FOR UPDATE`) to confirmed or pending per the calendar's approval setting, with a promotion email; the front-end offers a "Join waitlist" action on full slots and reflects the queued status. Off by default, so existing calendars are unchanged.
- **Custom (irregular) self-scheduling calendars** (#941, phase 1): a calendar can now use a `custom` schedule type — explicit date/time blocks with per-block vacancies (e.g. 03/09 08h–13h, 40 vagas) — instead of the `regular` weekday working-hours pattern. Blocks are configured in the calendar editor (date + start/end + capacity + optional label), stored as JSON alongside the existing config; the front-end calendar exposes only the configured dates and lists each block with its real time range and remaining vacancies. Booking enforces each block's own capacity via the existing `FOR UPDATE` slot check; a new `ffc_bypass_appointment_capacity` capability lets an authorised operator overbook a full block. The mode locks to `regular`/`custom` once a calendar has bookings.
...
### Changed
- **Audience & reregistration emails become globally editable** (#964, phase 1): the audience booking/cancellation default bodies and the three reregistration campaign emails (invitation/reminder/confirmation) now resolve through `EmailTemplates::effective_body()`, so their text is editable once in **Settings → SMTP → Email texts** — an audience schedule's own custom body still overrides the global. The **"All plugin emails"** index (#963) is updated to match: the hub-editable rows are marked **"Editable text (global)"** and their **Open →** now points to the Email texts box instead of the feature screen. Completes the phase-1 hub wiring; unconfigured installs render identically.
- **Recruitment convocation email absorbed into the SMTP hub** (#964, phase 1): the convocation email's subject + body are now edited in **Settings → SMTP → Email texts** with every other plugin email, instead of the bespoke editor on **Recruitment → Settings** (which is retired, replaced by a link to the hub). The dispatcher resolves both through `EmailTemplates::effective_body('recruitment-convocation', …)`, and `email_subject`/`email_body_html` are removed from the `ffc_recruitment_settings` option (From address/name + send-mode stay). A one-shot migration (`ffc_recruitment_email_hub_v1`) moves any customised subject/body into the hub; a default (unchanged) email migrates nothing and renders identically. ⚠️ The convocation body now passes through `wp_kses_post` (it previously allowed arbitrary admin HTML) — an accepted, admin-cap-gated tightening.
- **Certificate email tracks the SMTP hub global** (#964, phase 1): the per-form certificate email now resolves its subject + body through the global email-body hub — a form that hasn't customised the text (empty, or still carrying the shipped default) follows the hub's global default, so an admin edit in **Settings → SMTP → Email texts** reaches every such form; a form with a genuinely custom subject/body still wins. Resolution is a migration-free runtime heuristic (`EmailTemplates::overrides_global()`, comparing normalised visible text against the shipped **file** default), so unconfigured installs render identically. The shipped certificate subject moved into the `certificate-user` template file so subject + body share one source with the hub. The per-form editor UI (an explicit Global/Custom toggle) lands next; this increment wires the send path only.
- **Settings-nav hover uses a soft blue** (#951): the tab-nav item hover/focus background is now the theme-aware `--ffc-primary-light` tint (soft blue on light, deep navy on dark) instead of the neutral card colour, so pointing at an item reads as interactive at a glance. CSS-only.
- **Settings nav grouped into domain sections** (#951): the 17-item Settings side-nav (`page=ffc-settings`) — previously a flat alphabetical list — now renders tabs under seven subheadings (General · Content · Communication · Security & Access · Tools · Go to · System), staying alphabetical within each group. Each tab picks its group via a new `SettingsTab::get_group()`; the group order + labels live centrally in `Settings::nav_group_labels()`, and the module-settings links (Scheduling / Recruitment) form the "Go to" group. No tab content changes; the previously-dead `tab_order` is untouched (group order supersedes it). The nav stays a single ARIA `tablist` (group headers are `role="presentation"`), verified violation-free with axe.
- **Document-template hub reachable from a Settings tab** (#951): the template pool CPT (`ffc_cert_template`) — which holds certificate **and** appointment-receipt models — is moved out of the Certificates menu (where it misleadingly read as certificate-only) and relabelled kind-neutral (**“Document Templates”**). It no longer carries a standalone menu item; instead a new **Document Templates** tab inside **FFC Settings** launches the native management screens (Manage all / per-kind / + New), so you stay within the Settings tabbed UI. The list/edit screens, caps and storage are unchanged. First step of the #951 Direction 1 consolidation (one management hub + per-feature selectors).
- **Receipt tab is now a pure selector** (#951): the **Scheduling → Settings → Receipt** tab keeps its per-mode (Regular / Custom) template dropdowns but drops the inline CodeMirror editor + “duplicate to edit”; it now links into the Document Templates hub to **Manage / Edit / + New** (the “+ New template” link creates a receipt-kind template directly, via the hub’s `?ffc_kind` Add-New preset). Editing/creating/duplicating a receipt template happens in the one hub — no second editor. Second step of the #951 Direction 1 consolidation.
- Internal (#950) — **wall-clock date/time validation consolidated on `DateFormatter`**: new `DateFormatter::is_valid_time()` (HH:MM, optional :SS) and `is_valid_date()` (strict Y-m-d real-calendar date) replace the hand-rolled time regex (~6 copies — self-scheduling validator, extend-end / schedule-exception frontend actions, plus DateFormatter's own two private copies via a shared parser) and the Y-m-d regex+`checkdate`/round-trip copies (certificates-calendar REST controller, self-scheduling save handler, reregistration validator). The lenient `normalize_block_time()` normalizer is left as-is. No module-boundary edge; behaviour unchanged; tests added.
- Internal (#949) — **code-reuse sweep across the admin JS/PHP** (follow-up to the #945 duplication audit): extracted the CodeMirror flush/write/insert DOM idiom into `window.FFCCodeEditor` (form-editor + admin-pdf); folded the Email Model logo picker into the generic `ffc-branding-media.js` handler; migrated three hand-rolled `$.post` callers (receipt templates, cert-template visibility, url-shortener backfill) to the shared `FFC.request`; and added `RequestInput::is_truthy()`, `SettingsTab::should_enqueue_on()` (11 tabs) and `Core\Utils::read_file_contents()`, each collapsing 3+ duplicate copies. Behaviour unchanged; tests added.
- Internal (#945) — **the appointment-receipt tab's HTML editors reuse the shared CodeMirror initializer** instead of a parallel copy: a new `assets/js/ffc-code-editor-core.js` (`window.FFCCodeEditor.init()`) owns the `{{placeholder}}` overlay, theme wrapper class, change/submit → textarea sync and the Syntax-Highlighting-off fallback notice; the form-editor/cert-template wrapper (`ffc-admin-code-editor.js`) and the receipt tab (`ffc-receipt-templates.js`) both call it, and `AdminAssetsManager::enqueue_code_editor_base()` provides the shared enqueue/theme foundation. Restores the placeholder overlay, dark theme and disabled notice on the receipt editors, which the duplicated initializer had lost. No user-facing change beyond the recovered editor affordances.
...
- Internal (#935) — **namespace-migration `Phase` codename dropped from class docblocks**: the `(Phase 2)` / `(Phase 4)` tag the v3.2.0/v4.0.0 namespace migration left on ~50 `@version`/`@since` headers and the `ffcertificate.php` bootstrap docblock is removed (the version anchor + "Migrated to namespace" already carries the fact); issue-anchored phase refs (#563, #865, #899, #901) untouched. Comments only; no runtime change.
- Internal (#935) — **`DocumentFormatter::validate_phone()` uses the `PHONE_REGEX` constant** instead of an inline duplicate of the same pattern (single source of truth). Behaviour identical.
- Internal (#935) — **auto-generated duplicate docblock summary lines removed**: 18 class files carried repeated filler summary lines (e.g. `* Get profile.` ×5, `* Constructor.` ×41) left by an earlier doc-gen pass; each collapsed to its real first line + tags (492 lines). Comments only; no runtime change.

### Removed
- **Dead audience booking/slot AJAX endpoints** (#935): `wp_ajax_ffc_audience_create_booking` and `wp_ajax_ffc_audience_get_schedule_slots` were placeholders that always returned "Not implemented yet." with no JS/template caller — booking creation runs through `AudienceRestController::create_booking` and slot retrieval through `AudienceScheduleService`. Removes the two handlers and their registrations.
- **Dead-code sweep — orphaned AJAX handlers removed** (#935): eight admin-ajax actions with zero JS/template caller (each superseded by a live path) are dropped — the five URL-shortener row actions `ffc_{delete,toggle,trash,restore,empty_trash}_short_url(s)` (the working UI is the GET-link `handle_actions()` path), `ffc_audience_check_conflicts` (live path is the REST `/audience/conflicts` route; its handler referenced a never-defined `AudienceConflictService`), `ffc_generate_tickets` (superseded by `ffc_generate_codes`; also drops its private code-generation helpers), and `ffc_preview_date_format` (no client-side date preview exists). No user-facing behaviour change.
- Internal (#935) — **two unused helper methods removed**: `CsvExportTrait::extract_dynamic_values` (row-value extraction superseded by the per-source formatters; no production caller) and `SettingsTab::get_tab_url` (no subclass ever called it). Both were exercised only by their own tests, which are removed too. No runtime change.
- Internal (#935) — **dead constants and a no-op method removed**: three never-read global consts (`FFC_MIN_WP_VERSION`, `FFC_MIN_PHP_VERSION`, `FFC_JQUERY_UI_VERSION` — redundant with the plugin-header `Requires`/enqueue that WP core enforces), `RateLimitChecker::WEAK_SIGNALS` (documentary only; the matcher derives "weak" dynamically), the two literal-bypassed `ActivityLog::ACTION_SCHEDULE_OVERRIDE_CREATED` / `ACTION_OPERATOR_IP_BYPASS` class constants, and the vestigial no-op `SelfSchedulingEditor::display_save_errors()` + its `admin_notices` registration (the functional twin is `FormEditorSaveHandler::display_save_errors()`). No runtime change.
