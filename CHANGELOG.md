...
- Self-scheduling: appointment times no longer shift by the site's UTC offset on display. `appointment_date` / `start_time` / `end_time` are wall-clock (Category B) values, but they were being rendered through the instant API (`strtotime()` + `format_date`/`format_time`), which parses them as UTC and then re-applies `wp_timezone()` — so a 9h–17h calendar showed as 6h–14h on a UTC-3 site. Added `DateFormatter::format_wallclock_date()` / `format_wallclock_time()` (render the literal value, no timezone conversion) and routed every self-scheduling display through them: the frontend time-slot picker, the booking receipt, confirmation/notification emails, the admin appointments list, the CPT notice, and the cancellation page. The instant API is unchanged.
- Scheduling settings → Holidays: a saved holiday date no longer displays one day earlier (e.g. 05/06/2026 listed as 04/06/2026 on a UTC-3 site). Holiday `date` values are wall-clock DATE strings; the global-holidays list (Scheduling Settings → General) and the per-calendar holidays list both rendered them through `format_date()`, which parsed the literal date as UTC midnight and then re-applied the site offset, rolling it back a day. Both lists now use `format_wallclock_date()`.
- Certificate preview (form editor + public operator page) no longer crops the certificate's borders. The preview now renders the certificate at the **real PDF page size** (A4 — landscape by default, matching the generator's treatment of an unset orientation) inside the iframe and scales the whole frame down to fit the modal, preserving the page aspect. Previously the iframe took the modal's own aspect ratio, so `background-size: cover` cropped whatever didn't fit. The scale recomputes on window resize.
- Form editor → Time tab: the End date no longer blocks the whole form from saving when "Multiple days" is off. The start+1 `min` floor (added in the multi-day End-date change) was being emitted even while the End field is hidden and mirrors Start, so its value (= Start) fell below `min` and native validation rejected the hidden control with "An invalid form control … is not focusable". The `min` is now applied only while multi-day is on — both on render and live as the toggle flips.
- Form editor → Operator (Public Operator Access): a blank **Download Limit** now **inherits the global default** (Settings → Advanced → Default Download Limit) instead of force-saving the resolved value. An empty field deletes the per-form meta so the runtime falls back to the global; the field renders empty with an "Inherit from global" placeholder and its description shows the current global value in the same subtle `.ffc-global-default` chip. Mirrors the device-fingerprint inherit behavior.
- Form editor → Restriction → Device Fingerprint: a blank "Max submissions per device" now **inherits the global default** (Settings → Rate Limit → Device Fingerprint) instead of being force-saved as `2`. Leaving the field empty deletes the per-form meta so the read path falls back to the global value, matching the threshold/message fields. Each field's description now **shows the current global value it will inherit** (max / match threshold / block message), highlighted in a subtle `.ffc-global-default` chip. (Reverts the 6.3.11 hard-default-to-2 behavior.)
