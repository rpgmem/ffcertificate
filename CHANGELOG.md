...
### Added
- Documentation tab (#697) — **Troubleshooting** area (item 7): extended the guide with feature-grounded rows — a form hidden by the geofence time/geolocation gates (and the admin bypass), a short URL returning 404 (flush permalinks / link disabled), a submission blocked by the rate limit (and the allowlist), and an admin missing a plugin menu (the gating capability). Completes the #697 functional reorganization.
- Documentation tab (#697) — **Developer** area (item 6): rewrote the Hooks / REST / Forms API page against the code — a real inventory of the extension **filters** (the `ffcertificate_email` last-mile hook, plain-text, scheduling-email, certificate/QR, Ficha, form-schema) and lifecycle **actions** (submission / appointment / audience-booking / short-redirect / CSV-export), the REST surface across **both** namespaces (`ffc/v1` plus the Recruitment `ffcertificate/v1`), and the Forms API corrected to `per_page`/`page` (the old `limit` param was removed in 6.6.1). The Shortcodes reference was already accurate.
- Documentation tab (#697) — **Short URLs** area (item 5): reviewed the URL-shortener page against the module — corrected that it is a **top-level** admin menu (not nested under FFC Forms), and documented manual + auto-create-on-publish, the `/go/{code}` redirect, the per-link QR (which encodes the short URL so scans are counted), the counter-only click tracking (no per-click timestamp/referrer/IP), the settings and the three capabilities.
- Documentation tab (#697) — **Recruitment** area (item 4): reviewed the Recruitment page against the module and corrected staleness — the notice status is `definitive` (the old `active` value was renamed away), `closed` notices are still shown publicly with a banner, the grouping unit is the **adjutancy** (there is no separate "Cargos"/roles entity), the capability list is now complete (adds delete / reasons / settings / PII caps), and `[ffc_recruitment_my_calls]` matches by the candidate record linked to the account (not live CPF/RF). Added the CSV import headers and the call-up outcome flow.
- Documentation tab (#697) — **Reregistration** area (item 3): rewrote the **Campaigns** page against the module — the member form is delivered on the personal dashboard over AJAX (there is no reregistration shortcode), with corrected campaign statuses (`draft`/`active`/`expired`/`closed`) and submission statuses (the draft state is `in_progress`), the actual read-only integration endpoints (replacing stale POST REST routes that are really dashboard AJAX), the capability list, and cross-links to Audience Custom Fields and Ficha PDF. The Ficha PDF token reference is unchanged.
- Documentation tab (#697) — **Scheduling** area (item 2): expanded **Personal Calendars** (calendar configuration, working hours, booking rules, the `[ffc_self_scheduling]` booking flow, the status lifecycle, emails and capabilities, plus the receipt-PDF tokens) and added **Audience Calendars** (audiences and their hierarchy, schedules and environments/"spaces", the free-form `[ffc_audience]` booking with hard/soft conflict handling, per-schedule notifications + ICS, and CSV import/export). The former Audiences page is now the **Audience Custom Fields** reference. Verified against the self-scheduling and audience modules.
- Documentation tab (#697) — completed the **Certificates & Forms** area with its **Submissions** and **Configuration** subtrees: Submissions (admin list, edit screen, CSV export, and the capabilities that gate each; downloading the certificate via the magic link vs the public validation page, regenerated on demand), and Configuration pages for **General**, **Cache**, **Rate Limit**, **Geolocation** and **Advanced** (the existing Emails, User Dashboard, Capabilities and Maintenance pages were regrouped under Configuration). Content verified against the Settings tabs and the rate-limit / submissions subsystems.
- Documentation tab (#697) — first expansion tranche of the functional reorganization, the **Certificates & Forms → Forms** subtree: new pages for **Dynamic Fields (Form Builder)**, **Schedule (open/close window)**, **Geolocation / Geofence**, **per-form Email**, and **Public Operator Access**, plus a rewritten **Security & Restrictions** page (password / allow- & denylist / single-use tickets, automatic reprints, and the per-device fingerprint limit). Pages are cross-linked (tokens, validation URL, emails). Content verified against the form-editor metaboxes and submission pipeline.
...
### Fixed
- Documentation tab — removed the whole-card collapse behavior from the Quick Navigation entirely. The card no longer folds into a thin strip on scroll or on a stray click, and the disclosure triangle is gone — it stays a fully-expanded sticky panel, and only the tree branches open and close (one at a time). This drops the `ffc-doc-toc.js` script and its sentinel/IntersectionObserver. (#697)
...
### Changed
- Documentation tab — the Quick-Navigation branches now default to **collapsed** and are styled like the leaf items (same pill look, with a disclosure triangle); typing in the search box opens the branches that contain a match and re-closes them when the query is cleared. (#697)
- Documentation tab — rewrote the **Overview** page as a concise introduction to the whole plugin (certificates + validation, anti-fraud, scheduling, reregistration, recruitment, short URLs, LGPD/security, the email pipeline and the developer API), linking into the functional tree. (#697)
- Removed six unused legacy emoji icon CSS rules (`ffc-icon-celebrate` / `-edit` / `-note` / `-pin` / `-tag` / `-wrench`) left dead after the documentation dashicons migration. A wider audit confirmed the other "unused" selectors are dynamically generated (status/capability/banner classes built in PHP) or come from libraries (CodeMirror, WP list tables), so they were kept. (#697)
- Documentation tab (#697) — the Quick-Navigation is now a **collapsible tree that mirrors the plugin's functional areas** (Overview · Certificates & Forms · Scheduling · Reregistration · Recruitment · Short URLs · Developer · Troubleshooting) instead of the flat content-type sections. The tab is driven by a single **recursive registry** (nodes with optional `children`), so nesting a page or adding a functional area is a one-line edit and the nav + render order stay in sync. Foundation step: the existing pages are only re-grouped under the new tree — page files, anchors and content are unchanged; each area is fleshed out in follow-up PRs.
- Documentation tab (#674) — the Quick-Navigation and section headings now use **WordPress dashicons** (the same icon set as the admin menus) instead of emoji, with a distinct, non-redundant glyph per page — each feature reuses the icon of its own admin menu (Reregistration `update-alt`, Recruitment `groups`, Audiences `calendar-alt`, URL Shortener `admin-links`, …) and Developer reads as `editor-code`. **Ficha PDF** is now shown as a nested sub-item under **Reregistration** (it generates a reregistration-submission PDF). Removed the now-unused emoji `ffc-icon-code` helper.
- Documentation tab (#674) — Tranche 5 (Features, batch 5): retired the global "Complete Template Examples" page; its certificate template examples now live in a collapsible **Examples** accordion at the bottom of the Certificates & Forms feature page (per-feature examples, not a separate cookbook).
