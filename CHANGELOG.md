...
### Changed
- Internal (#711) — introduced `Core\SettingsPersistence`, a single capability-driven **authorization chokepoint** for settings writes: `authorize()` verifies the CSRF nonce then the capability in one audited place, and `save()` layers required per-field sanitisation + caller-owned persistence on top of it. Roles never enter the gate (it resolves via `current_user_can_admin_or`). The Scheduling settings (self-scheduling / audience visibility and global holidays) adopt `authorize()`; other writers follow incrementally. Mirrors the `Core\EmailService` chokepoint. Behavior-preserving.
