...
- IP geolocation debug logging — hardened to never store a raw client IP, even with `debug_geofence` enabled. A security audit found `IpGeolocation::get_location()` wrote the raw IP into the ActivityLog `LEVEL_DEBUG` trail on the cache-hit and service-success paths (and via the returned `location.ip` field echoed into the log), inconsistently with the already-hashed `fetch_from_service` path and the salted-hash treatment in `PreflightTelemetry`. All debug-log call sites now emit a truncated SHA-256 (`hash_ip()`) and redact the `ip` field of any logged `location` array via a new `redact_location_ip()` helper; the value returned to callers and the cache entry are unchanged (the contract `location['ip']` is preserved). Low severity — the path is gated behind the off-by-default debug toggle and an IP is low-sensitivity operational data — but it removes a residual LGPD/GDPR exposure and aligns the module with the plugin's IP-hashing convention. No behavior change for callers.

### Changed

- Internal refactor (#563 B3-A — retire delegating repository façades, one PR / one commit per façade) — completed the call-site migration the A6/D1/D2 façade tech-debt notes pointed to: the nine **static** repository façades (`RecruitmentCallRepository`, `RecruitmentCandidateRepository`, `RecruitmentNoticeRepository`, `RecruitmentAdjutancyRepository`, `RecruitmentReasonRepository`, `ReregistrationSubmissionRepository`, `AudienceRepository`, `AudienceBookingRepository`, `CustomFieldRepository`) were deleted and every caller now depends on the `*Reader` (reads) / `*Writer` (writes) class directly, so read-vs-write intent is explicit at the call site. Each façade's `@phpstan-type` row-shape and public constants (e.g. `CustomFieldReader::{FIELD_TYPES,DISPLAY_ONLY_TYPES,VALIDATION_FORMATS,USER_META_KEY}`, `RecruitmentNoticeReader::DEFAULT_PUBLIC_COLUMNS_CONFIG`, `RecruitmentReasonReader::{DEFAULT_COLOR,APPLIES_TO_VALUES}`, `ReregistrationSubmissionReader::STATUSES`, the adjutancy `DEFAULT_COLOR`) moved onto the reader as the canonical home; cross-module `@phpstan-import-type` consumers, `class_exists()` availability guards (dashboard asset-manager/shortcode, verification-handler, user-reregistrations REST controller, admin user-capabilities, sensitive-field registry, …) and stale `use` imports were repointed accordingly. The three **instance** repository façades (`AppointmentRepository`, `SubmissionRepository`, `UrlShortenerRepository`) are intentionally kept — they extend `AbstractRepository` and act as the transactional aggregate root (`begin_transaction()` → `FOR UPDATE` read → write → `commit()` on one `$wpdb`), not dead delegators. Tests repointed: repository tests now `@covers` the reader/writer (with pcov preloads to hold the coverage floor); read-only alias mocks swapped to the reader, mixed read+write mocks split into paired reader/writer alias mocks. Behavior-preserving — cross-module dependency graph unchanged (the #593 module-boundary guard stays at 130 edges), full suite green (5513 tests), PHPStan L8 + WPCS clean. No `FFC_VERSION` bump (develop PR).
- Internal architecture (#563 / #591 B3 — enforce module boundaries) — added a dependency-free module-boundary guard (`tests/Unit/ModuleBoundaryTest.php`) that scans `includes/`, derives the cross-module dependency graph (a "module" is the first namespace segment after `FreeFormCertificate\`; an edge is module A referencing `FreeFormCertificate\B\…`), and asserts it against a committed baseline (`tests/fixtures/module-boundary-baseline.php`, 130 edges at introduction). New cross-module coupling fails CI via the existing PHPUnit gate; removing coupling requires tightening the baseline — so the graph can only shrink. Regenerate after an intentional change with `FFC_UPDATE_BOUNDARY_BASELINE=1 vendor/bin/phpunit --filter ModuleBoundary`. This is the enforcement step of B3 (no facades required yet); per-module public facades and façade-retirement can now proceed incrementally without regression.
- Internal refactor (#591 phase-3 roadmap — one PR, one commit per sprint) — continuation of #589. **E5 (remaining admin/handler god-classes)**, all behavior-preserving: `FormEditorSaveHandler` (853→630) → extracted `FormEditorSaveValidator` + broke `save_form_data` into per-section savers; `AudienceAdminAudience` (899→196) → controller + `AudienceAdminAudienceRenderer`; `PrivacyHandler` (878→156) → `PrivacyExporters` + `PrivacyErasers` (WP exporter/eraser callbacks repointed — invoked by callable, identical); `AdminAssetsManager` (899→511) → `AdminConditionalAssets` for the per-screen enqueues; `SubmissionHandler` (871→624, critical path) → `SubmissionLifecycleService` for the trash/restore/delete/bulk/move/reset/cleanup ops, kept behind thin same-signature delegators so all 24 callers stay untouched and `process_submission` is unchanged. **G2′:** removed the lone truly-dead shim `DateFormatter::flush_cache()` (a no-op kept only for pre-#244 tests); the other "migration-tail shims" (`Encryption` defensive plaintext fallback, geolocation pre-presets, ficha `generation_date`) are *not* dead code and stay, evidence-gated like G3. **D2 (extend A6):** read/write split of the remaining smaller repositories — `RecruitmentNoticeRepository`, `RecruitmentAdjutancyRepository`, `RecruitmentReasonRepository`, `RecruitmentCallRepository`, `UrlShortenerRepository`, `ReregistrationSubmissionRepository` — each behind a delegating façade. **F3:** extracted `SettingsActionHandler` from `Settings` (1078→545; thin delegators keep hooks/tests intact). **C3:** removed dead back-compat entry points (`render_box_geofence`, `FormEditorGeofenceMetabox::render`, `AudienceAdminImport::render_page`), tests repointed to the live methods. New collaborator classes carry `@covers` + pcov-preloads so the coverage floor holds; markup/output byte-identical, hooks/queries/return-shapes unchanged.
- Internal refactor (#589 phase-2 roadmap — one PR, one commit per sprint) — continuation of #563. **C1/C2 (retire aliases):** removed the backward-compat delegation shims left by earlier splits — `UserManager`→`CapabilityManager`/`RoleRegistrar` (capability methods + `CONTEXT_*`/`*_CAPABILITIES` constant aliases) and `ActivityLog`→`ActivityLogQuery` (get/count/cleanup/run_cleanup/get_stats/get_submission_logs); every call site repointed to the real owner and the obsolete delegation/constant test assertions removed. **D1 (extend A6):** `RecruitmentCandidateRepository` split into `RecruitmentCandidateReader` + `RecruitmentCandidateWriter` behind the existing delegating façade. **E3 (public CSV):** extracted `CsvDownloadFlash`, `CsvDownloadAuditLog` (behind the kept public `PublicCsvDownload::get_audit_log_summary` contract), and `PublicCsvRowFormatter` from the two public-CSV god-classes; AJAX hooks unchanged. **E1/E2 (admin god-classes):** `ReregistrationAdmin` (1101→451) and `RecruitmentAdminPage` (1090→378) split into controller + `*Renderer` (byte-identical markup). **F1/F2 (templates):** moved the large inline HTML blocks of `RecruitmentNoticeEditPageRenderer` (1405→1209) and `RecruitmentCandidateEditPage` (1049→900) into `templates/admin/recruitment/{notice,candidate}-edit/` partials (methods that interleave private calls inside loops left inline). **E4 (PDF):** `PdfGenerator` (901→501) split — HTML/placeholder/QR rendering moved to `PdfHtmlRenderer`, data-assembly (incl. the untouched `cpf_rf_encrypted` legacy fallback) stays. Behavior-preserving throughout (markup/output byte-identical, query order and hooks unchanged); new collaborator classes carry `@covers` + pcov-preloads so the coverage floor holds. E5 (remaining admin god-classes), G2 (migration-tail shims) and the low-value cleanups are deferred to a dedicated phase-3 PR — see #589.
- Internal refactor (#563 backlog — A6 + A7 + B4, one PR, per-item commits) — closed the opportunistic backlog of the modular-monolith roadmap. **A6 (repositories read/write split):** the five large repositories were split into dedicated `*Reader` + `*Writer` classes behind a thin delegating façade that keeps the existing public API (and constants/static accessors) so the ~200 call sites are untouched — `CustomFieldRepository`, `AudienceRepository`, `AudienceBookingRepository` (static, via `StaticRepositoryTrait`, sharing each repo's cache group so writes still invalidate read caches; the booking `FOR UPDATE` create-transaction kept on the writer), and `AppointmentRepository`, `SubmissionRepository` (instance-based — reader/writer extend `AbstractRepository`, composed by the façade; all three bind the same global `$wpdb`, so transactions and `FOR UPDATE` locks stay coherent; the generic inherited CRUD stays on the façade). **A7 (view/logic separation):** `RecruitmentNoticeEditPageRenderer`, `RecruitmentCandidateEditPage`, and `Settings::display_settings_page()` had their data-gathering/decisions (bulk fetches, pagination math, PCD/empties maps, decryption, tab/permission/message resolution) lifted into named private prep methods, leaving the render methods to emit byte-identical markup from the prepared structs — query order and conditional guards preserved. **B4 (per-option-group readers):** new typed `GeolocationSettingsReader` and `RateLimitSettingsReader` (extending the `SettingsReader` pattern) centralize the ad-hoc scattered reads of `ffc_geolocation_settings` (Geofence, IpGeolocation, geofence metabox) and `ffc_rate_limit_settings` (RestController); the already-encapsulated `get_settings()` helpers keep their own defaults/merge strategies and only swap the literal option-key string for the new `OPTION_KEY` constant. Each façade carries a tech-debt note (#563 B3) pointing to the future call-site migration. Behavior-preserving throughout — full suite green (5517 tests); PHPStan L8 + WPCS clean.
- Internal refactor (#563 Sprint 7, A5) — began breaking up the 1085-line monolithic `Activator` by giving two more modules their own installer (one PR, per-module commits), matching the existing per-module activator convention (`AudienceActivator`, `RecruitmentActivator`, …): the reregistration schema (campaigns, audiences junction, submissions + auth_code/magic_token back-fill, audience→junction migration) → `FreeFormCertificate\Reregistration\ReregistrationActivator`; and the user-dashboard activation (base `ffc_user` role + admin cap grant, `ffc_user_profiles` and `ffc_custom_fields` tables, the "My Dashboard" page) → `FreeFormCertificate\UserDashboard\UserDashboardActivator`. The core `Activator` now delegates to both via the same `class_exists` guard pattern it already uses for the other modules; genuinely cross-cutting core work (submissions table, activity-log table, verification page, composite indexes, cross-table foreign keys, the `auth_code` UNIQUE upgrade, cron) stays in the orchestrator. `Activator` drops from ~1085 to ~760 lines. Behavior-preserving — activation order unchanged; ActivatorTest green.
- Internal refactor (#563 Sprint 5 phase 2, B1) — continued dismantling the `Core\Utils` god-utility, moving four more concerns to focused homes (one PR, per-concern commits; no back-compat wrappers): the asset pipeline (`asset_suffix` / `enqueue_dark_mode`) → new `FreeFormCertificate\Core\AssetHelper`; the client-IP resolver (`get_user_ip`) → the existing `FreeFormCertificate\Core\RequestInput` (it reads `$_SERVER`); the submissions-table-name accessor (`get_submissions_table`) → `FreeFormCertificate\Repositories\SubmissionRepository::get_submissions_table()` (the table's owning repository, public static); the `wp_kses` allow-list (`get_allowed_html_tags`) → new `FreeFormCertificate\Core\HtmlPolicy`; and the success-card renderer (`generate_success_html`) → new `FreeFormCertificate\Frontend\Submission\SuccessHtmlRenderer`, beside its sole caller `SuccessResponder`. All ~120 call sites repointed; test alias-mocks repointed accordingly (split out where a single Utils alias also stubbed other methods); dedicated tests relocated to the new classes (`AssetHelperTest`, `RequestInputTest`, `HtmlPolicyTest`, `SubmissionRepositoryTest`). `Core\Utils` now holds only four generic primitives (`format_bytes`, `truncate`, `get_day_of_week_number`, `sanitize_username_slug`) — the B1 end state. Behavior-preserving.
...
- Internal refactor (#563 Sprint 2) — split the 1359-line `CapabilityManager` god-class into three single-responsibility classes under `FreeFormCertificate\UserDashboard\`: `CapabilityMigrator` (the one-shot, option-flagged migrations — legacy renames, the 6.9.0 taxonomy renames, and the delete/export/import/reasons grant back-fills) and `RoleRegistrar` (the FFC role lifecycle — register/upgrade/remove the `ffc_user`, recruitment-manager, and per-module roles, plus role relabeling). `CapabilityManager` keeps the runtime surface (capability granting, access checks, the `*_CAPABILITIES`/`CONTEXT_*` registry, and `module_roles_definition()`), dropping from ~1359 to ~673 lines. All call sites (loader, activator, user-manager, recruitment-loader, role-capability-editor) and the `wp_roles_init` callback are repointed. Behavior-preserving — the `CapabilityCatalog` ↔ `CapabilityManager::get_all_capabilities()` invariant holds and the full capability/role suites stay green.
- Internal refactor (#563 Sprint 3, PR 3a) — began dismantling the `Core\Utils` god-utility (364 inbound refs) by extracting the filename builders into a focused `FreeFormCertificate\Core\FilenameHelper`: `sanitize_filename()`, `build_pdf_filename()`, and `get_export_filename()`. All call sites repointed (no back-compat wrappers). Pure string helpers, behavior-preserving; the `Core\Utils` request-input accessors follow in PR 3b.
- Internal refactor (#563 Sprint 3, PR 3b) — extracted the request-input accessors from `Core\Utils` into `FreeFormCertificate\Core\RequestInput`: `get_post_array()`, `get_post_string()`, `get_get_string()`, `get_post_int()`, `get_post_bool()`. All 207 call sites across ~50 files repointed (no back-compat wrappers). Behavior-preserving — typed `$_POST`/`$_GET` readers moved verbatim; nonce verification stays the caller's responsibility. `Core\Utils` drops from ~708 to ~430 lines across phase 1.
- Internal refactor (#563 Sprint 4, PR 4a) — began splitting the `RateLimitChecker` god-class into per-dimension strategy classes behind a `RateLimitStrategy` interface with an injected `RateLimitSupport` (settings + message formatter): `IpLimiter`, `EmailLimiter`, `CpfLimiter` (all in `FreeFormCertificate\Security\`). `RateLimitChecker::check_{ip,email,cpf}_limit()` now delegate to the strategies; the `RateLimiter` facade and every external call site are unchanged. First sprint to adopt instance-deps + interface (the B2/B3 direction). Behavior-preserving (full suite green); the `DeviceLimiter` follows in PR 4b. Also dropped four now-unused `Core\Utils` imports left in the submission guards after Sprint 3.
- Internal refactor (#563 Sprint 4, PR 4b) — completes the `RateLimitChecker` device split: the two-tier device-fingerprint matcher, the per-form effective-settings resolver, and the signal-row writer move to `FreeFormCertificate\Security\DeviceLimiter` (constructed with the injected `RateLimitSupport`). `RateLimitChecker::check_device_limit()` / `get_device_effective_settings()` / `record_device_signals()` now delegate; the `STRONG_SIGNALS` / `WEAK_SIGNALS` registry stays on `RateLimitChecker`, and the `RateLimiter` facade + all external call sites are unchanged. `DeviceLimiter` is keyed by `(form_id, signals[])` so it does not implement `RateLimitStrategy`. Behavior-preserving — `RateLimitChecker` drops from ~1119 to ~790 lines across Sprint 4.
- Internal refactor (#563 Sprint 5, PR 5a) — extracted the current-user permission gate from `Core\Utils` into a `FreeFormCertificate\Core\Capabilities` facade (B3): `current_user_can_manage()` and `current_user_can_admin_or()`. All ~100 call sites across ~48 files repointed (no back-compat wrappers); the `AjaxTrait` `class_exists` guards now reference `Capabilities`. Behavior-preserving thin facade over `current_user_can()`. (`CLAUDE.md` inline-gate guidance updated accordingly.)
- Internal refactor (#563 Sprint 6, PR 6a) — began splitting the 1554-line `RecruitmentCsvImporter` by extracting its pure string→rows layer into a stateless `FreeFormCertificate\Recruitment\CsvParser`: `parse()`, `build_row()`, `parse_pcd_flag()`, `normalise_id()`, plus the `REQUIRED_HEADERS` / `OPTIONAL_HEADERS` contract (now public consts). The importer's public `parse()` delegates to the new class and every internal call site is repointed; none of these helpers touch the database or WordPress globals, making them the safest, most isolated slice to lift first. Behavior-preserving — the existing parse/normalise characterization tests stay green and a new `CsvParserTest` pins the previously-untested `parse_pcd_flag()` / `build_row()` leaves. The validator (PR 6b) and staging/commit services (PR 6c) follow.
- Internal refactor (#563 Sprint 6, PR 6b) — extracted the §6 row-validation layer from `RecruitmentCsvImporter` into a stateless `FreeFormCertificate\Recruitment\CsvValidator`: the `validate()` rule set (CPF/RF presence + width, score / time-points format, rank, adjutancy membership, and the cross-row physical-candidate duplicate / field-divergence detection) plus the shared `line_error()` formatter (now a public static, since the staging ingest path uses it too). The importer's `run()` and staging paths delegate to `CsvValidator::`; the result-envelope builders (`failure()` / `failure_with_messages()`) stay on the importer as orchestrator-level concerns. Behavior-preserving — the existing validate characterization suite (repointed to the now-public method) stays green. The staging / commit services (PR 6c) follow.
- Internal refactor (#563 Sprint 6, PR 6c) — completes the `RecruitmentCsvImporter` split (closes the Sprint 6 modular decomposition). The staged batched-import flow (`ingest_job` → `validate_job` → `promote_batch` → `commit_job`, plus the stale-job sweep) moves to `FreeFormCertificate\Recruitment\CsvStagingService`, and the shared candidate-persistence cluster (`upsert_candidate`, `refresh_pcd_hash`, `maybe_promote_candidate`, `build_adjutancy_map`) moves to `FreeFormCertificate\Recruitment\CandidatePersister` — both the synchronous `run()` flow and the staging `promote_batch` flow now write candidates through that one class, eliminating the duplicated lookup-then-create / encryption / wp_user-promotion logic. `RecruitmentCsvImporter` becomes a thin façade: the four public staging entry points (referenced by the REST controller + the batched test-suite) and `parse()` delegate to the extracted classes, while `run()` / `import_preview()` / `import_definitive()` / the `failure*()` envelopes and `BATCH_SIZE_DEFAULT` stay as orchestration. No back-compat wrappers beyond the public façade; behavior-preserving — the full importer + batched characterization suites stay green. The importer drops from ~1153 to ~310 lines (≈1240 lines lifted into focused single-responsibility classes across the sprint).
