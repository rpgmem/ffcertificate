...
- **Certificate template pool — management screen** (#865): the `ffc_cert_template` pool is now editable in wp-admin as a **Templates** submenu under Certificate — a native list table with **Type** (Default/Custom) and **Visible** columns plus a nonce-protected **Show / Hide** row action that toggles whether a template appears in the form editor's "Load" list (via a new `CertTemplateWriter`). Shipped defaults are protected from deletion (the seeder re-creates them). Gated by `ffc_manage_forms` (read-only for `ffc_view_forms`).
