...
- **`RecruitmentNoticeEditPage` (sprint B).** Dedicated edit screen at `?action=edit-notice&notice_id=N` with five metabox-style sections: General (code read-only per §3.2 stable identifier rule, name + public_columns_config editable; UI-level guard rejects `rank=false`/`name=false`), Status (current state badge + transition buttons gated by `NoticeStateMachine::transition_to`; `closed → final` requires a reason; `draft → preliminary` shows a "this publishes the candidate list" confirm; `preliminary → final` opens a dual-path UI — A: snapshot preview as definitive, B: scroll to import section to upload a new definitive CSV; the dual-path hides itself when a definitive list already exists), Adjutancies (attach/detach UI relocated from the row inline), CSV import (radio-selectable target list — preliminary writes to preview via `/notices/{id}/import`, final writes to definitive via `/promote-preview` mode=definitive_import; disabled for final/closed notices per §5.1), Classifications (preview + definitive list tables with rank/candidate/adjutancy/score/status, candidate name + adjutancy slug resolved via in-memory lookup map to avoid N+1).
...
- **Notice status `active` renamed to `final`.** The `active` enum value was a holdover from the §5.1 plan vocabulary; "final" lines up with what the operator and candidates think of as the published, no-longer-preliminary list (and matches the existing "Final classification" copy on the candidate dashboard). Renames cascade through the state machine, the promotion service, the REST controller's transition target, the notice edit-screen UI labels, and the test suite. The error code `recruitment_active_to_preliminary_blocked_by_calls` is renamed to `recruitment_final_to_preliminary_blocked_by_calls` — REST consumers should update accordingly.
  - Schema migration: `RecruitmentActivator::maybe_migrate()` runs on `plugins_loaded` (option-versioned via `ffc_recruitment_schema_version=2`). Three-step ALTER widens the enum, UPDATEs every `status='active'` row to `'final'`, then narrows the enum back. Idempotent across reloads. Fresh installs skip this entirely (the new `CREATE TABLE` already uses the canonical 6.1.0 enum).
...
- **Public shortcode status branching rebalanced.**
  ...
  - `final` → renders the definitive list with a "Final classification." banner. The two-section split (não-chamados / chamados) is unchanged.
