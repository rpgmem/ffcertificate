...
- **Two legacy columns on `ffc_activity_log` could stop the audit trail under a strict `sql_mode`** (#1458): production carries `action_type varchar(50) NOT NULL` with no default — declared by no statement in this plugin, so `dbDelta` can neither drop it nor reach it — and `submission_id NOT NULL` against the `DEFAULT NULL` the CREATE declares. Nothing is broken there today only because that host has no `STRICT_TRANS_TABLES`: the column silently takes `''` on every insert. Under a strict mode — which the CI runner's MariaDB already uses, so CI is stricter than production — every activity-log write would fail. Both are now relaxed by an explicit `ALTER`, idempotent by reading the server rather than by a flag. **It relaxes and never drops**: no consumer reads those columns, but *nobody reads it* is not *it is empty*, and destroying audit history needs a count from a real install first.

- **Three columns nothing writes are dropped from `ffc_activity_log`, where they are provably empty** (#1458): `action_type`, `action_details` and `user_agent` are pre-6.6.4 leftovers that no `CREATE` in this plugin declares and `dbDelta` can never drop. Both halves of the condition were measured before the code was written — no consumer reads them anywhere in the tree, and production holds **14,062 rows with not one carrying a value in any of the three**. The evidence does not stay in the pull request: the migration **counts each column immediately before dropping it and refuses on a single row that carries anything**, so an install whose `action_details` holds audit history keeps it, relaxed and inert. `submission_id` is not a candidate — it is live, and its `NOT NULL` is the relax's to fix.
...
- **A capped category counted as though the cap were the total** (#1466): `LIMIT` is 100 per check, so a category holding more showed 100 and its counter read `1 of 100` — a floor in the grammar of a total. The cap was already detected and reported, but only in the page-level banner, which speaks about the *scan*, once, while the number is per category and further down. Each counter now says `of at least` when the checks behind **its** panel were capped. The relation is derived from each finding's own check rather than from a tier→check table, which is what makes one capped check qualify all three of the tiers it feeds — a hand-written map gets that wrong, and the test that proves the derivation is the one that fails when it is replaced by one.

- **The identity card's action column was sized by how long the card's sentence is** (#1468): `.ffc-identity-card-main` was `flex: 1 1 auto`, whose basis is the paragraph's max-content — so the shared-mailbox card, with the longest sentence on the screen, gave **1093px to the prose and 303px to the controls** and wrapped its two verb groups into a card 657px tall, while the decision card got 661px of controls in 226px. Measured, not eyeballed: crossing the two differences between those cards shows it is the prose, not the acknowledgement checkbox. The prose now claims no width of its own and takes the remainder, with a floor so it cannot become a letter-per-line column. Two further tidies in the same column, both of which read as size differences and neither of which is a height: every control already measures 44px, but a `<button>` and an `<input>` align on the text baseline rather than on their boxes (`Buscar…` sat 5px low), and the two fields of one group rendered 199px and 215px because `size` is a character count the UA ignores entirely on `type="number"`.

- **The shared-mailbox panel told the operator that no action was offered, above the two it offers** (#1368): the tier's note ended by saying there was no verb here and to decide with HR. That was true while the tier was classified and then refused every verb; #1461 gave it move and split, and the sentence stayed for four releases — so the panel denied the buttons rendered beside it. Found in a screenshot of the shipped screen, with the denial and the two buttons in one frame, which is the only way it could be found: the tests asserted the controls, and the prose describing them was nobody's. `with HR` survives, because the move still makes the operator tick an acknowledgement that HR confirmed whose the records are; what goes is the claim that there is nowhere to put that answer.

- **The identity card's action column was one width per tier** (#1468): it was sized `flex: 0 1 auto` — its own max-content — and the two tiers that reach that rule do not hold the same content. The shared-mailbox move form carries an acknowledgement, which pushes its button onto a second line, so its widest line is the field plus Search while the decision tier's is the field plus Search plus Move: **719px against 614px, 105px apart and stable across viewport widths**. The same grey column, two widths, decided by whether a checkbox happens to be in the form. With a declared basis both negotiate from the same number and come out equal at every width, shrinking together — 338/338 at 800px, 538/538 at 1000, 723/723 from 1200 up.
