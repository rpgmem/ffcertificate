...
- Form editor → Restriction → Device Fingerprint: a blank "Max submissions per device" now **inherits the global default** (Settings → Rate Limit → Device Fingerprint) instead of being force-saved as `2`. Leaving the field empty deletes the per-form meta so the read path falls back to the global value, matching the threshold/message fields; the placeholder/help text now say "inherit from global". (Reverts the 6.3.11 hard-default-to-2 behavior.)
