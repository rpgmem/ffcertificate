...
### Added
- **Editable, pool-backed appointment-receipt templates (per mode)** (#945): the appointment receipt (comprovante) PDF is no longer a single hardcoded layout — it now draws from the certificate **template pool** (#865), reused via a `_ffc_template_kind` discriminator (absent = certificate, fully retrocompatible). Two receipt defaults ship (Regular + Custom), editable/duplicable like any pool template, with a Category column + filter and a kind-aware editor. A new **Scheduling → Appointment Receipt** settings page selects, globally per scheduling mode, which template each calendar type uses (Regular vs Custom), falling back to the shipped default when unset. Adds `{{appointment_end}}` / `{{appointment_time_range}}` receipt tokens. No data migration; unconfigured installs render exactly as before.
