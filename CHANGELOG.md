...
- **Guarda: um id que o JavaScript procura tem de ser emitido por alguém** (#1220): três vezes numa sessão um teste passou verde sobre marcação que o produto nunca renderizou — o painel do usuário sem carregar painel (#1204) e o grupo de acúmulo que nunca se ocultava (#1219). `JsSelectorEmitterTest` cruza os 184 ids que o JS busca com quem os emite em PHP e em JS, e congela 7 sem emissor (todos código morto guardado por checagem de comprimento) mais 2 exceções do WordPress.
