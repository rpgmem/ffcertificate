...
- **Sete folhas de admin não usavam token nenhum e ignoravam o modo escuro** (#1126): calendário, editor de calendário, horário de trabalho, campos personalizados, encurtador, recrutamento e recadastramento pintavam com hex fixo — badges de status, modais, tabelas de transferência e o balão de toast ficavam claros dentro da tela escura. Todas passam a pintar pela paleta.
- **Rótulo do botão de sucesso reprovava o contraste no tema claro** (#1126): branco sobre `#00a32a` dá 3,35:1, abaixo do mínimo, desde sempre. O verde do botão escureceu para `#00801e` (5,11:1) e ganhou par `on-success` que inverte no tema escuro, como o par da primária.
- **Duas regras do `ffc-common.css` liam token inexistente** (#1126): a pista do interruptor e o badge de autosave usavam `var(--ffc-border-medium, …)` e `var(--ffc-bg-subtle, …)`, tokens que a paleta nunca declarou — então o literal de reserva é que pintava, nos dois temas. É o mesmo defeito que o #1116 fechou uma vez, sobrevivendo por dentro de um fallback.
- **Toda folha que lê token passa a depender de `ffc-common`** (#1126): uma custom property não declarada não cai para o literal anterior — ela invalida a declaração inteira, e o elemento fica sem cor nenhuma. Dezesseis enfileiramentos liam `var(--ffc-*)` sem declarar a dependência; quatro telas não tinham a paleta na página de forma alguma, entre elas a lista de usuários e o shortcode público de recrutamento.
...
- **O modo escuro nunca chegou ao frontend, e a causa era um bloco de tokens local** (#1126): o dashboard do usuário e a lista pública de editais redeclaravam `--ffc-*` dentro do próprio componente, o que sombreia a paleta para tudo ali dentro — então `:root.ffc-dark-mode` não tinha como alcançá-los. Removidos; os dois passam a ler a paleta compartilhada.
- **Quatro regras de modo escuro do frontend nunca dispararam** (#1126): casavam `[data-ffc-theme="dark"]`, atributo que nada no plugin define — nem PHP, nem template, nem JS. Tratamento de tema inerte, com aparência de tratamento existente; removidas, e os alertas passam a seguir a paleta.
- **Rótulo do botão de aviso reprovava o contraste** (#1126): branco sobre `--ffc-warning` dá 3,04:1 no tema claro. Ganhou par `on-warning`, que não inverte entre os temas porque o aviso é tom médio-claro nos dois.
- **Caixa de funções, dashboard, editais, calendário e caixa informativa do formulário ignoravam o modo escuro** (#1126): cinco folhas pintavam com hex fixo. Todas passam a pintar pela paleta; o que segue literal é decisão com motivo no código — o botão branco do interruptor, o véu translúcido sobre cor arbitrária e o papel branco da pré-visualização do certificado.
...
- **Dezoito controles de formulário ficavam com texto quase preto no modo escuro** (#1126): um `<button>`, `<input>`, `<select>` ou `<textarea>` **não herda `color`** — o navegador dá o próprio quase-preto — então a regra de base não os alcança. Eram as setas de mês e o botão "Hoje" do calendário, e também todos os campos do formulário público de recadastramento e da edição de perfil, que ninguém tinha aberto. Guarda nova falha quando uma regra pinta o fundo de um controle sem pintar o texto.
- **Texto das tarjas informativas do admin continuava ilegível no modo escuro** (#1126): a regra pinta a caixa e os filhos herdam — o que se sustenta isoladamente e mede 13:1 contra o CSS real do wp-admin. Numa instalação com outros plugins não se sustenta: qualquer declaração direta vence um valor herdado. Os nós de texto da tarja passam a ser nomeados, o que supera qualquer regra de terceiro sem `!important`.
