...
### Added
- Internal (#997) — **vacuous-test ratchet**: a dependency-free `AssertionCoverageTest` that fails CI when a *new* test method verifies nothing — no mock expectation and no assertion beyond a literal `assertTrue( true )`, so it passes whether or not the code works. The 108 existing cases (of 7,285) are frozen in a baseline that can only shrink, matching the module-boundary guard; a sweep of them was measured and deliberately deferred, since 94 sit in admin/render tests where a mocked WordPress can honestly assert little more. Tests only.
- Internal (#994) — **fresh-install gate in CI**: a new job installs a throwaway WordPress on a MariaDB service, activates the plugin into an empty database and asserts every table `uninstall.php` declares was created — then deletes the plugin with the Danger Zone opt-in on and asserts nothing is left behind. Because a fresh install has no legacy residue, the comparison runs both ways, so an object the plugin creates but never cleans up now fails CI. Complements the post-deploy smoke (#990), which alone sees the provider's real stack but only an established install. Adds a dependency-free `ActivatorSqlTest` for the same defect class without a database. CI + tests only.
...
### Fixed
- Internal (#997) — **three untested degraded paths in the admin JS**: the shared code-editor guard is `!settings || wp undefined || !wp.codeEditor`, but its only test passed `settings = null` and short-circuited before the two `wp` probes ran; and of the two `wp.media` guards in the PDF layout screen only the insert-image one was covered — the media-library button was not even in the fixture. All three now have a test, each verified to fail when its guard is neutered. Same class as the #989 email-editor bug: a test that supplies the global can never see the code fail without it.
- Internal (#997) — **thirteen tests that verified nothing now do**: the vacuous methods in the schema/activation and security areas — six migration routines, the audience and dashboard capability grants, the role sweep, the profile-role snapshot, the privacy-policy no-op and both user-access enqueue paths — each asserted only `assertTrue( true )`. Each now pins its actual contract: the one-shot completion flag for a migration, the exact set of roles a grant touches, the autosave bundle the tab must enqueue. Several assert what the test's own comment already claimed but never checked. Baseline 108 → 95.
- Internal (#997) — **dbDelta hygiene in the activators**: `dbDelta()` reads every line of a `CREATE TABLE` field list as a column definition, so the 46 blank lines and 35 SQL comments inside twelve statements were each issued as a malformed `ALTER` against any table that already existed. Measured against a real MariaDB, the three self-scheduling tables produced 41 database errors before the cleanup and 1 after. Latent rather than live (every affected activator guards on `table_exists()`), but it would fire the moment a column was added the normal way. Documentation moved to PHP comments above each statement, and `ActivatorSqlTest` now fails on any non-definition line. The appointments CREATE also now declares `UNIQUE KEY validation_code`, matching what `ensure_unique_validation_code_index()` has enforced on every activation since it was written — the stale plain `KEY` was the last remaining error. Zero database errors after.
- **Self-scheduling had no calendars table on a fresh install** (#994): an SQL comment and a column `COMMENT` string inside the `ffc_self_scheduling_calendars` CREATE each contained a semicolon, and `dbDelta()` splits its input on that character before executing — so the statement was truncated mid-column and the table was never created. Sites that already had the table from an earlier release were unaffected, which is why it went unseen. Same family as #358 and the 6.0.1 COMMENT-clause failure.
- **Uninstall left two tables and 21 options behind** (#994): `ffc_recruitment_import_jobs` / `ffc_recruitment_import_staging` and every one-shot capability, role and migration marker written during activation were absent from `uninstall.php`, so deleting the plugin never removed them. Found by the new fresh-install check, which compares what activation actually writes against the delete list — most of these are written through a constant or variable, so no static scan could see them.
- Internal (#993) — **settings defaults that disagreed with themselves**: `obsolete_shortcode_days` (90 declared, 30 read), `qr_default_size` (200 / 256) and `public_csv_default_limit` (1 / 100, plus 0 at two frontend sites) restated different defaults at their read sites. Three of the five lived in the `SettingsReader` typed accessors — the path the project's conventions tell callers to prefer — and all three were caller-less, so nothing was broken in practice; the first caller would have inherited a public-download quota of 100 where the declared default is 1. All aligned to the declared value, and a new `SettingsDefaultsTest` fails CI on any future drift.
