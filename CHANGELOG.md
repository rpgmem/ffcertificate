...
- **Auth-code "virtual prefix" letter from `DocumentFormatter` is now embedded in the filename**: certificates get `C-`, appointment receipts get `A-`, fichas get `R-` — mirroring the same letters used at `/valid` and in emails. The helper detects already-prefixed and pre-formatted codes (e.g. caller passing `C-MLQQ-Z9UX-9MWF` from a display rendering) and **does not double-prefix**, also stripping inner dashes from the body for filesystem compactness. Synthetic `S{id}` fallbacks for draft fichas stay un-prefixed since they are not verifiable auth codes. Example shapes: `certificado_666_C-MLQQZ9UX9MWF.pdf`, `recibo_42_A-7K3M9P2XQRST.pdf`, `ficha_99_R-ABCDEF123456.pdf`, `ficha_99_S12345.pdf` (draft).
- **Prefix is translatable** via `_x()` with the `pdf filename prefix - …` context. Default labels: `certificado` (certificate), `recibo` (appointment receipt), `ficha` (reregistration record). PT-BR sites get those literal slugs; sites in other locales get whatever the translator provides (e.g. `certificate` / `receipt` / `record` for EN). The prefix is then passed through `sanitize_filename`'s safety rules (ASCII alnum + dash, lowercase) so non-ASCII translations still produce a safe filename.
- **Appointment receipt now has its own filter** `ffcertificate_appointment_receipt_filename( $filename, $calendar_id, $validation_code, $appointment )`. Pre-6.6.11 this PDF was the only one without a per-type filter — a gap that made customisation impossible for sites that wanted to keep the old `Appointment_Receipt_{CODE}.pdf` shape. Now matches the parity of `ffcertificate_certificate_filename` and `ffcertificate_ficha_filename`, which continue to fire with their original argument shapes for back-compat.
- **Ficha filename now uses `auth_code` (when populated) or `S{submission_id}` fallback** instead of `{Title}_{Display Name}`. Approved reregistration submissions already carry a real `auth_code` (generated by `AuthCodeService::generate_globally_unique_auth_code()` at status transition to `approved`); submitted-but-not-yet-approved fichas fall back to the synthetic `S{id}` slot so the filename stays unique and stable across re-downloads. When a future schema migration makes `auth_code` mandatory for all states, the same code path keeps working unchanged — only the fallback branch stops being exercised.

### Added

- **New central hook `ffcertificate_pdf_filename( $filename, $type, $entity_id, $code )`** fires for every PDF the plugin generates, BEFORE the per-type hooks. Documented escape-hatch for automation-driven sites that need stable, locale-independent filenames.

### Tests

- 9 new PHPUnit cases in `UtilsTest::test_build_pdf_filename_*` covering: the three known types, code uppercase-normalisation, unsafe-char stripping, empty-code segment skipping, negative-id clamping, unknown-type fallback, locale-translatable prefixes, and the central filter as escape hatch.

### How to pin a stable shape (for DMS / automation sites)

Drop this single hook into `functions.php` or a small mu-plugin — locks the prefix to stable EN slugs regardless of WP locale:
...
- This is the only release on `release/6.6.x` that intentionally changes user-visible filenames. The forward-port to `main` rolls into 6.7.x as a non-breaking addition (6.7.0 already shipped with the old shapes; the forward-port PR will document the same banner there).
- No JS asset rebuild — the change is server-side. The browser receives the new `filename` over the existing AJAX contract.
