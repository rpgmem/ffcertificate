...
- **A guard that a deprecation cannot outlive its own removal release** (#1309): `DeprecationDueTest` reads `@removal X.Y.Z` — a tag on the code that is still there — and fails once, in the release that must act. A second direction requires the tag on every `apply_filters_deprecated()` / `_deprecated_*()` call, so a cycle cannot ship with no end date. Blocks at zero on 6.25.0.
- **A guard that every declared table has a declarer the runtime can reach** (#1311): `ActivatorSchemaGuardTest` fails when a `CREATE TABLE` under `includes/` resolves to a table no class the `Loader` names declares. Stated per table rather than per class, which is what lets it block at zero with no allowlist — a one-shot migration may stay unwired while its tables are creatable elsewhere. Reverting the fix makes it name all seven.
...
- **Two guards that close #1313's shape rather than its instances** (#1313): `IdentityConvergenceGuardTest` fails when a file hands a value straight to `Encryption::hash()` outside the registry, and when anything but `UserCreator` calls `wp_create_user()` / `wp_insert_user()`. Both read the token stream, not lines — the migration registry's own description contains the text `Encryption::hash()` inside a translated string, and `wp_create_user_request()` is an unrelated GDPR call — with a canary that fails if either distinction stops working.
- **The identity-index invariant, checked against a real database** (#1313): a `fresh-install` step plants an orphaned submission carrying a CPF hash, asks the plugin's own resolver to resolve it, and asserts the row was adopted and the index carries the hash — then sweeps every linked row in every module table. It plants first because on a fresh install the invariant would otherwise hold over nothing, and it is the one of the issue's three guards no static scan can make: it is a statement about rows.
- **The link audit now covers every store, and reports the conflict count** (#1313): Settings → Migrations gains three checks across submissions, appointments, recruitment candidacies and the identity index — one identifier held by two accounts, one account holding two identifiers, and identifiers the index does not yet carry. The first is the number this arc needs before anyone designs a merge policy, and the one the backfill deliberately declines to resolve by picking; the third reads zero once the backfill has finished. Still report-only: nothing is changed.
...
- **The removal release is now a tag, not a sentence** (#1309): the first version of the guard read the prose already in the tree and reported two `Security` docblocks that say the opposite of a promise — past-tense history about an argument removed in 6.24.0. `until X` is a promise or a finished story depending on a verb several clauses away, so the nine live sites carry `@removal` and the prose stopped restating the number.
- **Identity resolution asks the indexed profile first, and every link feeds it** (#1313): `UserCreator` consulted `ffc_submissions` alone while its own orphan-adoption happily claimed appointment rows, so the plugin declined to recognise someone it would adopt a record for moments later. It now asks `ffc_user_profiles` first and keeps submissions as the fallback — the index is not yet a superset on an existing install, so replacing the query outright would create duplicates. Linking a record now records that user's identifiers, filling empty columns only: a value already on record is a conflict to count, not to overwrite.
- **The user's identifier hashes moved to an indexed column** (#1313): `ffc_user_profiles` gains `cpf_hash` and `rf_hash`, each indexed, and `UserProfileService` writes them there instead of beside the ciphertext in `wp_usermeta` — which indexes `meta_key` and never `meta_value`, so the question the hash exists to answer was a scan. The ciphertext stays in the meta; existing hashes are copied into the columns on upgrade, and the key rotation rebuilds both halves.
- **An e-mail is now hashed in lowercase everywhere** (#1313): `sanitize_email()` strips invalid characters and does not lowercase, so an appointment booked as `Joao@Escola.gov.br` wrote a hash no other module's lookup could find. The rule ships with the migration that rewrites the rows already stored — alone it would make every lookup canonicalise against rows that did not.
- **Forty-four sites asked the same three questions and each answered for itself** (#1314): *seven digits is an RF* had nine copies, the canonical form of an address had three spellings with two of them wrong, and `normalize_cpf_rf()` — which has existed since 6.6.1 — had its body reproduced inline at twenty sites rather than called. All of them now go through `DataSanitizer`. The four entry validators keep their own length test on purpose, because rejecting a length is not routing one.
- **Certificates and appointments resolve users through the identity index** (#1313): both were still on the legacy single-hash entry point, which queries `ffc_submissions` alone and writes to no index — so the invariant this arc exists to create, *whenever a record gains a link to a user that user's identity index receives the identifiers*, was false for the two highest-volume writers. They now use the dual entry point, which asks `ffc_user_profiles` first and feeds it. The lookup itself is unchanged: one `cpf_rf` field means exactly one hash is non-null.
- **An imported audience member is created under the audience context** (#1313): the CSV import passed `CONTEXT_CERTIFICATE`, which #1321 preserved rather than decide in passing. It grants nothing the person needs — the capability that shows a member their own bookings is granted by `AudienceWriter::add_member()` (#1302) — and nothing is taken away, since every grant path adds what the holder lacks and none removes.
- **The user profile hashes through the boundary, not past it** (#1313): `UserProfileService` normalised and then called `Encryption::hash()` directly, which was correct because it remembered to — the exact shape this arc removes. It calls `hash_identifier()` now; the normalisation is idempotent, so the value is unchanged and the agreement is enforceable instead of promised in a comment.
- **The audience CSV import resolves people by identifier, not by address alone** (#1313): `cpf` and `rf` join the members template as optional columns, hashed through the registry and asked of the identity index before the e-mail. A person already registered under a different address was given a second account when creation was on, and their row was dropped as *User not found* when it was off. The members export deliberately does not gain the columns — that would put decrypted CPFs in a downloaded file, which is a separate decision.
- **The campaign invitation stops telling people to complete a reregistration that is already complete** (#1300): `get_awaiting_invitation()` selects on `invited_at IS NULL` whatever the status, so the invitation reaches everyone imported by CSV, and anyone who used the dashboard banner before the operator pressed *Send invitations*. The sentence and the button label now come from the submission's status as two tokens — they move together, since a line saying the reregistration is recorded under a button saying *Complete Reregistration* makes the e-mail contradict itself. The destination is unchanged, and the pending wording is what it always was. The import panel says the same thing at import time, which is the only moment before the invitation goes out.
...
- **Loading a certificate template with an empty name threw** (#1309): the success notice read `displayName || filename || ''`, and `filename` was a variable of the by-filename branch the `html/` removal made unreachable in 6.23.0 — so a pool template with a blank title hit a `ReferenceError` on the happy path. A truthy name hides it, because `||` never evaluates its right side.
- **The template modal still described the `html/` folder that went in 6.23.0** (#1309): "Template file not found. Check if file exists in html/ folder." pointed operators at a directory the plugin no longer has, and a 403 from `admin-ajax.php` advised checking file permissions — advice that belonged to the direct `fetch()` this path replaced. Both now say what actually happened; the JS branch, its test and three stale comments are gone.
- **The reregistration CSV import shipped with no tables on every upgraded install** (#1311): `ReregistrationActivator::create_tables()` had one caller, `Activator::activate()`, and neither a WordPress plugin update nor the testes rsync activates anything — so the two tables #1214 added existed on fresh installs only. It now heals from `Loader` behind an `FFC_VERSION` gate, like its four siblings. The post-deploy smoke reported it on twelve consecutive deploys.
- **`UserDashboardActivator` had the same gap, still latent** (#1311): its schema also only ever existed after an activation. It heals **schema only** — re-running `create_tables()` would resurrect a dashboard page an administrator deleted and re-register a role the orchestrator already owns, so those two steps stay at activation where they belong.
...
- **The per-address submission limit counted nothing for an address typed with a capital** (#1314): `RateLimitRepository` hashed the value exactly as the visitor typed it and compared it against a hash written from the canonical form, so `Joao@…` matched no prior submission of `joao@…` and the daily, weekly and monthly caps all read zero. Both branches canonicalise first, and the configured one goes through the hash boundary.
