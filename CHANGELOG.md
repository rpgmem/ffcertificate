...
### Changed
- Internal (#711) — introduced `Core\SettingsPersistence`, a single capability-driven chokepoint for settings writes (CSRF nonce verification → capability authorization → required per-field sanitisation → caller-owned persistence), mirroring the `Core\EmailService` transport chokepoint. The plugin's settings writers adopt it incrementally in follow-up work; no behavior change yet.
