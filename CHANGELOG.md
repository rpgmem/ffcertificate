...
- **Documentation tab (Settings → Documentation): floating "Back to top" button.** The page is long — a TOC card followed by 21 sections — so a discreet circular link now sits at the bottom-right of the viewport and jumps back to the top. Zero JS: a pure HTML anchor (`#ffc-doc-top`) paired with `scroll-behavior: smooth` scoped via `:has(.ffc-doc-back-to-top)` to the Documentation tab only. Honours `prefers-reduced-motion`, has an accessible `aria-label`, and degrades to an instant jump in browsers without `:has()` support.
