...
- **A screen for the identity queue, under the Certificate menu** (#1368): the audit card lists only *the findings that name an account*, so a mistyped RF on a recruitment candidacy — which carries no WordPress user until promotion — was counted there and shown nowhere. The new screen reads those rows and the row ids that are the only handle on them. Read-only; correcting a value comes next.
- **A confirmed RF can be corrected from the queue** (#1368): rewrites the ciphertext and the hash across every store holding the old value, repoints the identity index and grants the owning account the capabilities that read it — as one transaction, so the index and the rows can never disagree. Refuses a value failing the check digit, a finding naming two accounts, and a correction that collides with existing records, because that is a merge.
...
- **The identity screen stops calling an unread scan clean** (#1368): the check-digit scan returns failures, so it returns none when no store carries the columns it needs, when nothing decrypts, and when the data is genuinely fine — and the screen reported all three as "every stored RF satisfies its check digit". It now says what it read, and both unread states say plainly that they are not a clean result.
- **The identity screen's empty state had a fourth case, and it read as reassurance** (#1368): a scan that found no stored RF at all fell into the "every one satisfies its check digit" branch, which then rendered *0 values checked and all of them sound*. Zero examined is now its own state, saying no row in any scanned store carries both a hash and a ciphertext — ordinary on an install that never captured an RF, a signal on one that did.
