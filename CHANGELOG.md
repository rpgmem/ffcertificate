...
- Admin tab/menu ordering is now **alphabetical by the translated label**, locale-aware (accents collate correctly) with stable anchors, via a new shared `Core\LabelSorter`: the Settings tabs (General first; Advanced/Migrations/Documentation last), the Recruitment sub-tabs (Notices first, Settings last) and the Scheduling Settings sub-tabs (General first). The #787 capability editor now shares the same helper (#PR).
- The audience/reregistration custom-field type `<select>` now shows **translated** labels instead of a raw `ucfirst()` of the slug (e.g. "Dependent_select" → "Dependent Select"), ordered A→Z with the display-only `acknowledgment` type last. Slugs are unchanged — display-only (#PR).
