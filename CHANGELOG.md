...
### Added
- Recruitment convocation email now has a **send mode** (#794): Settings → Recruitment offers Always / Never / Ask. "Ask" prompts per call — single and bulk — whether to notify the candidate, defaulting to send; the global "disable all emails" switch still wins. Gated in `RecruitmentEmailDispatcher::should_send()`.
