...
**Bugfix release.** Fixes around the per-device submission limit (added in v6.3.0) plus a polish pass on reprint UX and i18n coverage. The server-side gate now correctly lets the reprint flow through; the form gains a friendly client-side hint when the current device has already submitted before; the reprint success message surfaces the existing certificate's authentication code; and several JS-only fallback strings introduced over v6.3.6–v6.3.10 are now translatable through the standard `wp_localize_script` pipeline.
...
- **Reprint success message now shows the authentication code.** When `ReprintDetector` returns an existing submission, the success response includes the formatted code in the message itself: *"Certificate previously issued (Reprint). Authentication code: C-XXXX-XXXX-XXXX."* (The same code is also already rendered in the success card via `Utils::generate_success_html`; this just makes it copy-pasteable from the inline message too.) Implemented by surfacing `auth_code` from the reprint result (or, in the quiz reprint branch, from the existing submission row) into `$submission_data` before rendering, so both the message and the card render against a single, reprint-aware data shape. New translatable string: *"Certificate previously issued (Reprint). Authentication code: %s"*.
