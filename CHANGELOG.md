...
- **Encryption Key Health card — one-click `FFC_ENCRYPTION_KEY` suggestion** (#851, on #846/S7a): whenever FFC's encryption key still derives from the WordPress salts (`encryption_decoupled === false`, i.e. any status while not decoupled — `ok`/`weak`/`placeholder`/`empty`), the card now offers a ready-to-paste `define( 'FFC_ENCRYPTION_KEY', '…' )` snippet in a read-only field with **"Generate another"** and **Copy** buttons. The 64-char key is generated **client-side** (`crypto.getRandomValues`) over a charset safe for a single-quoted PHP literal (excludes `' " \ $`) and is **never saved or sent** — the plugin never writes wp-config.php. Carries the mandatory rotation caveat (new install only; a site with data needs a planned rotation, S7b). Hidden once already decoupled. New `ffc-encryption-key-suggest.js`, enqueued only on the Advanced tab.
