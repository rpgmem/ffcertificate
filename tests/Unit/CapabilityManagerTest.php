...
class CapabilityManagerTest extends TestCase {
    ...
    public function test_grant_context_sends_chromed_access_email_when_enabled(): void {
        ...
        // The dashboard link is a pre-rendered button token (#965).
        $this->assertStringContainsString( 'Go to your dashboard', $captured['body'] );
        // Wrapped in the shared configurable chrome.
        $this->assertStringContainsString( '<!DOCTYPE html>', $captured['body'] );
    }

    public function test_grant_context_access_email_tracks_the_hub_override(): void {
        $mock_user = Mockery::mock( 'WP_User' );
        $mock_user->shouldReceive( 'has_cap' )->andReturn( false );
        $mock_user->shouldReceive( 'add_cap' );
        $mock_user->ID          = 41;
                ...
                if ( 'ffc_email_bodies' === $key ) {
                    return array(
                        'access-granted' => array(
                            'body' => '<p>Global grant body for {{context_label}}.</p>{{dashboard_button}}',
                        ),
                    );
                }
                if ( 'ffc_email_template' === $key ) {
        ...
        CapabilityManager::grant_context_capabilities( 41, 'certificate' );

        $this->assertNotNull( $captured, 'access-granted email should be sent' );
        $this->assertStringContainsString( 'Global grant body for Certificates.', $captured['body'] );
        // The button token still resolves inside the override.
        $this->assertStringContainsString( 'Go to your dashboard', $captured['body'] );
        // The shipped default greeting is gone — the override replaced the body.
        $this->assertStringNotContainsString( 'Hello Grantee,', $captured['body'] );
    }
