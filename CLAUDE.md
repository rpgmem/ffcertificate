...
On/off **toggle switches** (`.ffc-toggle`, rendered via `AdminUI::render_toggle()`) auto-save on flip: mark the input `data-ffc-autosave-key="<key>"`, add the key to `SettingsAjaxEndpoint::allowlist()` (`{option, path?, type, cap}`), and have the tab call `enqueue_autosave_infra()`. This is the standard for atomic, side-effect-free boolean settings. **Two autosave endpoints exist and stay separate on purpose** (do NOT unify — bad-façade trap): `SettingsAjaxEndpoint` (`ffc_update_setting`) writes WP **options** under a global cap; `FormMetaAjaxEndpoint` (`ffc_update_form_meta`, attribute `data-ffc-autosave-form-key`) writes per-post **meta** gated on `edit_post` of that exact form. **The JS is NOT shared, despite how the pair reads** — measured in #1114: `data-ffc-autosave-key` is bound by `bootAutoSaveFields()` and saved by `FFC.Admin.autoSaveField` (`assets/js/ffc-admin-autosave.js`, debounced, `change`+`input`, `.ffc-autosave-badge`), while `data-ffc-autosave-form-key` has its own delegated handler inside `assets/js/ffc-admin.js` (`change` only, no debounce, `.ffc-form-meta-autosave-status`, whose CSS hardcodes hex instead of the `--ffc-*` tokens, so it ignores dark mode). The cost is not theoretical: a fix written in one path silently misses the other, which is exactly what happened to the #1114 validity guard — it had to be written twice. Treat any change to autosave behaviour as two edits until #1116 converges them; four of the six divergences (action, extra payload, nonce source, badge class) are parameters, and of the rest the delegated binding and the token-based badge are the better halves to keep.
