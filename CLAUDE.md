...
- **User-deletion integrity is two layers over a *deliberate subset*, not every `user_id` column.** The primary is the app-layer `UserCleanup` hook (`deleted_user`), which anonymises `user_id` on the user-owned tables (submissions, appointments, audience members/booking-users/schedule-perms, user-profiles). `MigrationForeignKeys` adds a DB-level `user_id → wp_users` FK backstop on *those same* tables + the `activity_log` audit table (FK-only), so integrity holds even if the hook is bypassed. This FK migration is **not** legacy: `dbDelta()` cannot emit FKs, so it is the sole path that creates them (absent on a fresh install until it runs), guarded by `ffc_foreign_keys_db_version`. Tables outside the subset are app-layer only — recruitment by design (a candidate isn't a WP user until promotion). **Known gap:** `ffc_reregistration_submissions` + the custom-field value tables carry a `user_id` covered by neither layer (orphaned on deletion) — tracked separately; when touching those, decide retain-vs-anonymise rather than assuming coverage.
