...
It blocks against `.github/scripts/dbdelta-drift-baseline.php`, a ratchet that can only shrink (a change not listed fails; a listed change that stops happening also fails). **There is no local way to regenerate it** — it needs a live MariaDB, which only that job has — so on failure the gate prints the exact block to paste, and dumps `SHOW CREATE TABLE` for statements whose disagreement is not a type spelling.

Three things the first measurement taught, all of them counter-intuitive. **Write the display width.** `int unsigned` looks cleaner and is wrong here: MariaDB stores `int(10) unsigned` and compares textually, while WP core's `dbDelta` ignores a width-only difference on MySQL 8.0.17+ and *explicitly not on MariaDB*. So `int(10)` is correct on both servers, and it is what WP core writes in its own schema — do not "tidy" it away. **`json` is not stored by MariaDB**, which implements it as `LONGTEXT` plus a CHECK, so a `json` column drifts forever there; `longtext` would fix MariaDB and lose native validation on MySQL 8, which is why the seven remaining entries are a deliberate open question rather than a bug to sweep. **Declare the index the code actually creates:** `Activator::upgrade_auth_code_unique_constraints()` converts `auth_code` to `UNIQUE INDEX uq_auth_code` on two tables, so a declared plain `KEY auth_code` never survives — the same shape as `validation_code` in the self-scheduling tables.
