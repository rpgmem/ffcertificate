...
_The `html/` layout fallback (#865 phase-4) was removed in 6.23.0 (#1087), leaving this inventory empty. Its exit condition was met: the pool seeds on every install since the `CertTemplateSeeder::pool_has_defaults()` retry in 6.22.0, and `import_legacy_templates` had long read 0 pending. Both removed sites were internal render paths — the `html/*.html` glob in `AdminAssetsManager` and `FormEditor`'s by-filename load — plus the three `html/default_certificate_*.html` files, which the seeder never used (it reads `templates/certificate-defaults/`)._

**The `html/*.png` files stay, and not as a shim.** `rewrite_html_image_refs` side-loads them into the Media Library for installs whose templates still carry a v1 `html/` reference, and it reads them **off disk** to do it — so deleting them would strand any install that has not finished that migration. That is a separate exit condition (`rewrite_html_image_refs` reading 0 pending), measured separately, and the removal above deliberately did not touch it.
