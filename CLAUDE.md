...
**Vacuous-test ratchet (#997).** `tests/Unit/AssertionCoverageTest.php` fails when a *new* test method verifies nothing — no Mockery/Brain\Monkey expectation and no assertion beyond a literal `assertTrue( true )`. The 108 existing cases are frozen in `tests/fixtures/vacuous-tests-baseline.php`, a ratchet that can only shrink: a new entry fails (write a real assertion), and a removed one also fails (a test was fixed — lock the win in). Regenerate after an intentional change with `FFC_UPDATE_VACUOUS_BASELINE=1 vendor/bin/phpunit --filter AssertionCoverage` and review the diff.
